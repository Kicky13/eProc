...
class MY_Model extends CI_Model
{
    ...
    protected $timestamps_format = 'd-M-y g.i.s A';
    ...
    public function where($field_or_array = NULL, $operator_or_value = NULL, $value = NULL, $with_or = FALSE, $with_not = FALSE, $custom_string = FALSE)
    {
    	if($this->soft_deletes===TRUE)
    	{
    		...
    		if($temp[1]['function']!='force_delete')
    		{
    			$this->_where_trashed();
    		}
    	}
    ...
    /**
     * Where raw. WARNING! This query is not escaped by CI.
     * @param String query
     */
    public function where_raw($query) {
        $this->_database->where($query, null, false);
        return $this;
    }
    ...
    */
    ...
    private function get_current_id() {
    	$this->db->select($this->table."_SEQ.CURRVAL AS NEXTID", FALSE);
    	$this->db->from('DUAL');
    	$query = $this->db->get();
    	$row = $query->row();
    	return $row->NEXTID;
