...
class ec_approval_invoice extends MY_Model {	
    ...
    public function getData($role){
        $sql = "
            SELECT AI.*,IH.*,VH.VENDOR_NAME,TO_CHAR(IH.INDATE,'DD-MM-YYYY') AS CREATE_DATE, EGS.BSART 
...
//        echo $this->db->last_query($sql);die();
    ...
    public function getRoleUser($user){
        $sql = "
            SELECT AA.USERNAME,AA.ROLE_AS,SUM(COMPANY) COMPANY, SUM(STATUS_A) STATUS  FROM (
                SELECT RU.*,
                    CASE WHEN OBJECT_AS = 'COMPANY'
                        THEN VALUE
                    ELSE '' END COMPANY,
                    CASE WHEN OBJECT_AS = 'STATUS'
                        THEN VALUE
                    ELSE '' END STATUS_A
                FROM EC_ROLE_USER RU
                JOIN EC_ROLE_ACCESS RA
                    ON RU.ROLE_AS = RA.ROLE_AS
                WHERE RU.ROLE_AS LIKE '%APPROVAL INVOICE%' AND RU.STATUS = 1 AND USERNAME = '$user'
            ) AA
            GROUP BY AA.USERNAME,AA.ROLE_AS ORDER BY STATUS,COMPANY ";
