...
class ec_penawaran_vnd extends CI_Model
{
    ...
    public function get_data_produk($vendorno, $mins = 0, $max = 12)
    {
        ...
        if ($mins == 0) {
            $SQL = "SELECT * FROM (select inner_query.*, rownum rnum FROM (SELECT  DT.STOK_COMMIT, SM.MAKTX, SM.MEINS, SM.PICTURE, SM.DRAWING, EML.TDLINE, ASS.*,
            ...
            $SQL = "SELECT * FROM (SELECT  DT.STOK_COMMIT, SM.MAKTX, SM.MEINS, SM.PICTURE, SM.DRAWING, EML.TDLINE, ASS.*,
                ...
                LEFT JOIN (SELECT SUM(CRT.STOK_COMMIT) STOK_COMMIT,CRT.MATNO FROM EC_T_CHART CRT 
                                        ...
                                        ON DT.KODE_DETAIL_PENAWARAN=CRT.KODE_PENAWARAN GROUP BY CRT.MATNO) DT
                        ON DT.MATNO=ASS.MATNO JOIN EC_M_LONGTEXT EML ON SM.MATNR=EML.MATNR
                WHERE ASS.VENDORNO='" . $vendorno . "' GROUP BY DT.STOK_COMMIT, SM.MAKTX, SM.MEINS, SM.PICTURE, SM.DRAWING, EML.TDLINE, ASS.KODE_ASSIGN, ASS.MATNO, ASS.VENDORNO, ASS.START_DATE,ASS.END_DATE,ASS.INDATE,ASS.KODE_PENAWARAN,ASS.KODE_UPDATE,ASS.DAYS_UPDATE,ASS.CURRENCY) WHERE ROWNUM <='" . $max . "' ORDER BY INDATE DESC NULLS LAST";
        }
