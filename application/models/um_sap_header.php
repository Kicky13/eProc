<?php
class um_sap_header extends CI_Model {

	protected $table = 'UM_SAP_HEADER', $hist_po = 'HISTORY_PERPANJANGAN_PO';
	...
	public function join_vnd() {
		...
		$this->db->join('VND_HEADER', "VND_HEADER.VENDOR_NO=PO_HEADER_SPB.VENDOR", 'inner');
	}
	...
	public function where_po_null(){
		$this->db->where('PO_NUMBER IS NOT NULL', null, false);
	}
	...
	public function where_creator($id) {
		$id = strtoupper($id);
		$this->db->where('("FULLNAME" like '. "'$id%'".' OR '.'"FULLNAME" like '. "'%$id'".' OR '.'"FULLNAME" like '. "'%$id%')", null, false);
	}
	...
	function ptm($ptm) {
		...
		return $this->get(array('PO_HEADER.PTM_NUMBER' => $ptm));
	}
	...
	public function get_id_log() {
		$this->db->select_max('ID','MAX');
		$max = $this->db->get($this->hist_po)->row_array();
		return $max['MAX']+1;
	}

	public function get_log($where = NULL) {
		$this->db->select('*');
		if (!empty($where)) {
			$this->db->where($where);
		}
		$this->db->from($this->hist_po);
		$result = $this->db->get();
		// echo $this->db->last_query();
		return $result->result_array();
	}
	...
	public function insert_log($data) {
		$this->db->insert($this->hist_po, $data);
	}

	public function update_log_po($data) {
		$this -> db -> set('DOC_DATE', "TO_DATE('" . $data['DOC_DATE'] . "', 'YYYY-MM-DD HH24:MI:SS')", FALSE);
		$this -> db -> set('DDATE', "TO_DATE('" . $data['DDATE'] . "', 'YYYY-MM-DD HH24:MI:SS')", FALSE);
		$this -> db -> where('ID', $data['ID']);
		$this -> db -> update('HISTORY_PERPANJANGAN_PO');
	}
}