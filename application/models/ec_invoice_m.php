...
class ec_invoice_m extends CI_Model {
	protected $table_INVOICE = 'EC_INVOICE_HEADER', $table_GR = 'EC_GR';
	public function __construct() {
		parent::__construct();
		$this -> db = $this -> load -> database('default', TRUE);
	}
        // 1->Draft,2->Submited,3->Approved,4->Rejected,5->Posted,6->Paid
	public function get_Invoince($status = 1) {		
		$this -> db -> from($this -> table_INVOICE);
		$this -> db -> join('EC_M_PO_HEADER', 'EC_M_PO_HEADER.PO_NUMBER = EC_INVOICE_HEADER.NO_SP_PO', 'inner');		
                if(is_array($status)){
                    $this->db->where_in("STATUS_HEADER", $status, TRUE);
                }else{
                    $this->db->where("STATUS_HEADER", $status, TRUE);
                }		
	...
	public function getINV($INVNO) {
		$this -> db -> from($this -> table_INVOICE);
		$this -> db -> where("ID_INVOICE", $INVNO, TRUE);
		$result = $this -> db -> get();
		return (array)$result -> result_array();
	}
	...
	public function get_InvoinceDetail($INVOICE_NO) {
        ...
        public function detailInvoice($invoice){
            $this -> db -> from($this -> table_GR);            
            $this -> db -> where("INV_NO",$invoice);	
	...
	function updateData($data) {
		$this -> db -> where("INVOICE_NO", $data['INVOICE_NO'], TRUE);
		$this -> db -> update($this -> table_INVOICE, array('ALASAN_REJECT' => $data['ALASAN_REJECT'], 'STATUS_HEADER' => '2', 'CHANGE_ON' => date("Ymd")));

		$this -> db -> where("INV_NO", $data['INVOICE_NO'], TRUE);
		$this -> db -> update($this -> table_GR, array('STATUS' => '2', 'CHANGE_ON' => date("Ymd")));
	}
        
        function updateInvoice($invoice,$data) {
		$this -> db -> where("ID_INVOICE", $invoice, TRUE);
                $this -> db -> set("CHDATE", "sysdate", FALSE);
		return $this -> db -> update($this -> table_INVOICE, $data);
	//	$this -> db -> where("INV_NO", $data['INVOICE_NO'], TRUE);
	//	$this -> db -> update($this -> table_GR, array('STATUS' => '2', 'CHANGE_ON' => date("Ymd")));
	}
	...
	function updateHeader($data) {
		$this -> db -> where("INVOICE_NO", $data['INVOICE_NO'], TRUE);
		$this -> db -> update($this -> table_INVOICE, array('DOC_DATE' => $data['DOC_DATE'], 'POST_DATE' => $data['POST_DATE'], 'PAYMENT_BLOCK' => $data['PAYMENT_BLOCK'], 'NOTE_APPROVE' => $data['NOTE_APPROVE'],  'CHANGE_ON' => date("Ymd")));
	}

	function updateHeaderSukses($data) {
		$this -> db -> where("INVOICE_NO", $data['INVOICE_NO'], TRUE);
		$this -> db -> update($this -> table_INVOICE, array('STATUS_HEADER' => '3', "INVOICE_NO_SAP" => $data['INVOICE_NO_SAP'], 'CHANGE_ON' => date("Ymd")));
		
		$this -> db -> where("INV_NO", $data['INVOICE_NO'], TRUE);
		$this -> db -> update($this -> table_GR, array('STATUS' => '3', 'CHANGE_ON' => date("Ymd")));
