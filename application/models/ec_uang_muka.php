...
class ec_uang_muka extends CI_Model {
    ...
    protected $table = 'EC_GR', $tableHeader = 'EC_UM_HEADER', $tableDenda = "EC_T_DENDA_INV", $tableDoc = "EC_T_DOC_INV", $tableCOMPANY = 'EC_C_COMPANY', $tablePURC_ORG = 'EC_C_PURC_ORG', $tableDOC = "EC_C_DOCTYPE", $tableTracking = "EC_TRACKING_INVOICE", $tableUm = 'EC_UM_HEADER';
...
public function getManUmVer($PSS){
    $this->db->from($this->tableUm);
    $this->db->join('EC_TRACKING_UM', 'EC_TRACKING_UM.ID_UM = EC_UM_HEADER.ID_UM', 'left');
    $this->db->where("(STATUS_TRACK=2
                       AND POSISI = 'VENDOR')", NULL, FALSE);
    $this->db->or_where("(STATUS_TRACK=3
                       AND POSISI = 'EKSPEDISI')", NULL, FALSE);
    $this->db->or_where("(STATUS_TRACK=3
                       AND POSISI = 'VERIFIKASI')", NULL, FALSE);
    $this->db->or_where("(STATUS_TRACK=5
                       AND POSISI = 'VERIFIKASI')", NULL, FALSE);
    $result = $this->db->get();
    return (array) $result->result_array();
}
