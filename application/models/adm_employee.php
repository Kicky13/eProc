...
class adm_employee extends CI_Model
{
	...
	function getListApp($idemp){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_APPROVAL.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_APPROVAL', 'ETOR_APPROVAL.ID_EMP = ADM_EMPLOYEE.ID', 'inner');
		$this->db->where('ETOR_APPROVAL.CREATED_BY = '.$idemp, null, false);
		$this->db->where('ETOR_APPROVAL.ID_TOR is null', null, false);
	...
	function getListAppMail($idemp, $idtor){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_APPROVAL.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_APPROVAL', 'ETOR_APPROVAL.ID_EMP = ADM_EMPLOYEE.ID', 'inner');
		$this->db->where('ETOR_APPROVAL.CREATED_BY = '.$idemp, null, false);
		$this->db->where('ETOR_APPROVAL.ID_TOR = '.$idtor, null, false);
		$this->db->order_by("ORDER_APPRV", "asc");
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
	...
	function getListAppKomentar($idtor){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_KOMENTAR.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_KOMENTAR', 'ETOR_KOMENTAR.ID_EMP = ADM_EMPLOYEE.ID', 'inner');
		$this->db->where('ETOR_KOMENTAR.ID_TOR = '.$idtor, null, false);
		$this->db->order_by("ID_KOM", "asc");
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
		}
	...
	function getListAppEdit($id){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_APPROVAL.*, ETOR_MAIN.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_APPROVAL', 'ETOR_APPROVAL.ID_EMP = ADM_EMPLOYEE.ID', 'inner');
		$this->db->join('ETOR_MAIN', 'ETOR_MAIN.ID_TOR = ETOR_APPROVAL.ID_TOR', 'inner');
	...
	function getListAppMain($idemp){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_MAIN.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_MAIN', 'ETOR_MAIN.CREATED_BY = ADM_EMPLOYEE.ID', 'inner');
		$this->db->where('ETOR_MAIN.CREATED_BY = '.$idemp, null, false);
		$this->db->order_by("ETOR_MAIN.ID_TOR", "desc");
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
		}
	...
	function getListApproval($idemp){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_MAIN.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_MAIN', 'ETOR_MAIN.CREATED_BY = ADM_EMPLOYEE.ID', 'inner');
		$this->db->join('ETOR_HOLDER', 'ETOR_HOLDER.ID_TOR = ETOR_MAIN.ID_TOR', 'inner');
		$this->db->where('ETOR_HOLDER.ID_EMP = '.$idemp, null, false);
		$this->db->where('ETOR_MAIN.IS_SUBMIT = 1', null, false);
		$this->db->where('ETOR_MAIN.STATUS <> 0', null, false);
		$this->db->order_by("ETOR_MAIN.ID_TOR", "desc");
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
	...
	function getListGambar($idemp){
		$this->db->select("ADM_EMPLOYEE.ID, FULLNAME, EMAIL, ETOR_UPLOAD.*");
		$this->db->from($this->table);
		$this->db->join('ETOR_UPLOAD', 'ETOR_UPLOAD.CREATED_BY = ADM_EMPLOYEE.ID', 'inner');
		$this->db->where('ETOR_UPLOAD.CREATED_BY = '.$idemp, null, false);
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
		}
	...
	function getListCari($FULLNAME){
		$this->db->select("ID, FULLNAME");
		$this->db->from($this->table);
		//$this->db->like('FULLNAME', $idemp);
		$this->db->where("LOWER(FULLNAME) LIKE '%". strtolower($FULLNAME)."%'");
		$this->db->limit(10); 
		$result = $this->db->get();
		if ($result->num_rows() > 0) {
			return $result->result_array();
