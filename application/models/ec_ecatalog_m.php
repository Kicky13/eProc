...
class ec_ecatalog_m extends CI_Model
{
    ...
    public function get_data_checkout_pl($ID_USER)
    {
        ...
        $this->db->join('EC_PL_PENAWARAN', 'EC_T_CHART.KODE_PENAWARAN = EC_PL_PENAWARAN.KODE_PENAWARAN','LEFT OUTER');
        $this->db->join('EC_T_DETAIL_PENAWARAN', 'EC_T_CHART.KODE_PENAWARAN=EC_T_DETAIL_PENAWARAN.KODE_DETAIL_PENAWARAN', 'left');
        $this->db->join('VND_HEADER', 'EC_T_DETAIL_PENAWARAN.VENDORNO = VND_HEADER.VENDOR_NO', 'RIGHT');        
        ...
        $this->db->where('EC_T_CHART.CONTRACT_NO', 'PL2018', TRUE);
        $this->db->where("(EC_T_CHART.STATUS_CHART ='8' OR EC_T_CHART.STATUS_CHART ='0' ) ");
...
//        echo $this->db->last_query();die();        
    ...
    {
        $this->db->from($this->ccPl);
        $this->db->join('ADM_EMPLOYEE', $this->ccPl.'.ID_USER = ADM_EMPLOYEE.ID');
        $result = $this->db->get();
        return (array)$result->result_array();
    }
...
//        $this->db->trans_complete();*/
    ...
    function POsuccessOnePL($id, $cont, $mat, $kmbl, $idcart, $qty, $CC = "", $data = "", $curr, $alasan)
    {
//        $qty *= 300000;
        $this->db->where("ID_USER", $id, TRUE);
        $this->db->where("CONTRACT_NO", $cont, TRUE);
        ...
        $SQL = "INSERT INTO EC_PL_APPROVAL VALUES ( 
            '" . $kmbl[1] . "', '" . $CC . "', '" . $data['VENDORNO'] . "', '" . ($data['PRICE'] * $qty) .
            "', '" . $curr . "', TO_DATE('" . date("Y-m-d H:i:s") . "', 'yyyy-mm-dd hh24:mi:ss'), " .
            " TO_DATE('" . date("Y-m-d H:i:s") . "', 'yyyy-mm-dd hh24:mi:ss'), 1, 1,'" . $PROGRESS_CNF . "','1','0')";
...
//        }
    ...
    public function getVndConfirm2($matno = '603-200850', $plant = '2702', $kode_detail)
    {
        $SQL = "SELECT
                TB2.MATNO,
                SM.MAKTX,
            ...
            WHERE
                TB2.MATNO = '" . $matno . "'
                AND TB2.STOK > 0 AND DT3.KODE_DETAIL_PENAWARAN = '" . $kode_detail . "'
            ORDER BY
                DT3.PRICE ASC";
        $result = $this->db->query($SQL);
        return (array)$result->result_array();
    ...
    public function get_data_checkout_PO_One_PL($ID_USER, $contract_no, $matno)
    {
        ...
        $this->db->where("EC_T_CHART.CONTRACT_NO", $contract_no, TRUE);
        $result = $this->db->get();
        return (array)$result->result_array();
    }
    ...
    public function addCart($MATNO, $CNT, $USER, $DATE, $KODE_PENAWARAN, $COSCENTER, $QTY = 0, $ONE = 0, $STS = 0, $alasan, $funcloc, $ket, $file)
    {
        if ($ONE == 0) {
            ...
            $this->db->insert($this->tableCart,
                array('QTY' => $QTY, 'CONTRACT_NO' => $CNT, "STATUS_CHART" => $STS, 'BUYONE' => $ONE, 'MATNO' => $MATNO,
                    'ID_USER' => $USER, 'DATE_BUY' => $DATE, 'KODE_PENAWARAN' => $KODE_PENAWARAN,
                    'COSCENTER' => $COSCENTER,'ALASAN_BELI' => $alasan,'FUNCLOC' => $funcloc,'KETERANGAN' => $ket,'FILE_KORIN' => $file));
        }
        if ($CNT != 'PL2017')
            $this->db->join('EC_T_CONTRACT',
    ...
    public function addCart_pl($MATNO, $VENDORNO, $USER, $DATE, $KODE_PENAWARAN, $COSCENTER, $QTY = 0, $ONE = 0, $STS = 0, $alasan, $funcloc, $ket, $file)
    {
        if ($ONE == 0) {
            $this->db->select('COUNT(*)')->from($this->tableCart)->where(array('VENDORNO' => $VENDORNO, 'BUYONE' => 0, 'MATNO' => $MATNO, 'ID_USER' => $USER));
            $this->db->where("(STATUS_CHART = 0 or STATUS_CHART = 8 )");
            $query = $this->db->get();
            $result = $query->row_array();
            $count = $result['COUNT(*)'];
            if ($count > 0) {
                $this->db->set('QTY', 'QTY+(1)', FALSE);
                $this->db->where(array('VENDORNO' => $VENDORNO, 'BUYONE' => 0, 'MATNO' => $MATNO, 'ID_USER' => $USER));
                $this->db->update($this->tableCart);
            } else {
                $this->db->insert($this->tableCart,
                    array('CONTRACT_NO' => 'PL2018', "STATUS_CHART" => $STS, 'BUYONE' => $ONE, 'MATNO' => $MATNO,'QTY' => $QTY,
                        'ID_USER' => $USER, 'DATE_BUY' => $DATE, 'KODE_PENAWARAN' => $KODE_PENAWARAN,
                        'COSCENTER' => $COSCENTER, 'VENDORNO' => $VENDORNO));
            }
        } else {
            $this->db->insert($this->tableCart,
                array('QTY' => $QTY, 'CONTRACT_NO' => 'PL2018', "STATUS_CHART" => $STS, 'BUYONE' => $ONE, 'MATNO' => $MATNO,
                    'ID_USER' => $USER, 'DATE_BUY' => $DATE, 'KODE_PENAWARAN' => $KODE_PENAWARAN,
                    'COSCENTER' => $COSCENTER,'ALASAN_BELI' => $alasan,'FUNCLOC' => $funcloc,'KETERANGAN' => $ket,'FILE_KORIN' => $file, 'VENDORNO' => $VENDORNO));
        }
        $query = $this->get_data_checkout_pl($USER);
        $count = count($query);
        return $count;
    }
...
//         var_dump($SQL);die();
    ...
    public function getDealsVendor($matno, $plant, $comp = '2000')
    {
        ...
        ,TB3.STOK,TB3.DESKRIPSI_ITEM, 
                        SM.MATNR,SM.PICTURE,SM.MAKTX,SM.MEINS,SM.ID_CAT, CAT.\"DESC\",TB1.*
                        FROM (SELECT ASS.MATNO, CURRENCY, ASS.KODE_PENAWARAN FROM EC_PL_ASSIGN
         ASS
                        GROUP BY ASS.MATNO, ASS.KODE_PENAWARAN,CURRENCY) TB1
                        INNER JOIN EC_M_STRATEGIC_MATERIAL SM ON TB1.MATNO=SM.MATNR
                        INNER JOIN EC_M_CATEGORY CAT ON SM.ID_CAT=CAT.ID_CAT
...
//        return $this->db->last_query();
    ...
    public function get_m_plant($plant)
    {
        $this->db->from('EC_M_PLANT');        
        $this->db->where("PLANT", $plant);        
        $result = $this->db->get();
        return $result->result_array();
    }
    
    function POsuccessCartPL($id, $cont, $mat, $po, $idcart, $qty, $CC = "", $data = "", $curr, $total,$gudang,$korin)
    {
        $this->db->trans_start();
        $this->db->where("ID_CHART",$idcart,TRUE);
        $this->db->where("ID_USER", $id, TRUE);
        ...
        $this->db->where("STATUS_CHART", '0', TRUE);
        $this->db->where("BUYONE", '0', TRUE);        
        $this->db->update($this->tableCart, array('STATUS_CHART' => '1', 'STOK_COMMIT' => $qty, 'PO_NO' => $po, 'FILE_KORIN'=>$korin));        
        
        $this->db->select('PROGRESS_CNF')->from("EC_PL_CONFIG_APPROVAL");
        $this->db->where("VALUE_FROM<=", $total, false);
        $this->db->where("VALUE_TO>=", $total, false);
        $this->db->where("UK_CODE", $CC, TRUE);
        $query = $this->db->get();
        // var_dump($query);
        $result = $query->row_array();
        ...
        $cek=$this->db->where('PO_NO',$po)->get()->num_rows();         
//        var_dump($cek);
        if($cek < 1){
            if($gudang == '1'){
                $SQL = "INSERT INTO EC_PL_APPROVAL VALUES ( 
            ...
            $SQL = "INSERT INTO EC_TRACKING_PO VALUES ( 
            '" . $po . "', TO_DATE('" . date("Y-m-d H:i:s") . "', 'yyyy-mm-dd hh24:mi:ss'), 1,null,'" . $id . "', '" .
            $this->session->userdata['FULLNAME'] . "')";
            $this->db->query($SQL);
        }        
        $this->db->trans_complete();                        
