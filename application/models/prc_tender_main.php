...
class prc_tender_main extends CI_Model {
	...
	public function kondisi_evatek() {
		$this->db->select('TRUNC(SYSDATE) - TRUNC(PTC_END_DATE) AS SELISIH');
		$this->db->where("PTM_COMPANY_ID",4000)->where("(NAMA_BARU='Evatek' OR NAMA_BARU='Approval Evatek')");
	}
	...
	public function join_latest_activity($cron_job = null) {
		...
		$this->db->join('(SELECT LISTAGG(S.PPI_PRNO, \', \') WITHIN GROUP (ORDER BY S.PPI_PRNO) AS PPI_PRNO, LISTAGG(S.PPI_MRPC, \', \') WITHIN GROUP (ORDER BY S.PPI_MRPC) AS PPI_MRPC , S.PTM_NUMBER
			FROM (
			SELECT distinct PRI.PPI_PRNO, PRI.PPI_MRPC, S.PTM_NUMBER
	...
	public function orderNameEvatek(){
		$this->db->distinct();
		$this->db->select('ADM_EMPLOYEE.ID,ADM_EMPLOYEE.FULLNAME,ADM_EMPLOYEE.EMAIL');
		$this->db->join('PRC_PROCESS_HOLDER', 'PRC_TENDER_MAIN.PTM_NUMBER = PRC_PROCESS_HOLDER.PTM_NUMBER', 'inner');
		$this->db->join('ADM_EMPLOYEE', 'ADM_EMPLOYEE.ID = PRC_PROCESS_HOLDER.EMP_ID', 'inner');
		$this->db->where('PRC_TENDER_MAIN.PTM_STATUS >', '0');
	}
	...
	public function join_pr_item() {
		$this->db->join('V_HEADER_PR', 'PR.PPI_PRNO = V_HEADER_PR.PPR_PRNO', 'inner');
	}

	public function join_mrpc() {
		$this->db->join('ADM_MRPC', 'ADM_MRPC.MRPC = PR.PPI_MRPC AND ADM_MRPC.PLANT = V_HEADER_PR.PPR_PLANT', 'inner');
	}
	...
	public function where_app_proses() {
		$this->db->where('APP_PROCESS.NAMA_BARU', 'Kirim RFQ dan Input Penawaran');
	}
