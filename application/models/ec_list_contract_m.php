...
class ec_list_contract_m extends CI_Model {
	protected $table = 'EC_T_CONTRACT', $tableCOMPANY = 'EC_C_COMPANY', $tablePURC_ORG = 'EC_C_PURC_ORG', $tableDOC = "EC_C_DOCTYPE";
	...
	function insert_sap($dataLC) {
		...
		if ($count < 1) {
			...
			$this -> db -> update($this -> table, array("del" => $dataLC['del'], "inco1" => $dataLC['inco1'], "inco2" => $dataLC['inco2'], "pgrp" => $dataLC['pgrp'], "exrate" => $dataLC['exrate'], "validstart" => $dataLC['validstart'], "validend" => $dataLC['validend'], "procstat" => $dataLC['procstat'], "relind" => $dataLC['relind'], "shortext" => $dataLC['shortext'], "t_qty" => $dataLC['t_qty'], "netprice" => $dataLC['netprice'], "grossprice" => $dataLC['grossprice']));
		}
	}
	...
	function ubahStatDraft($data) {
		$this -> db -> where("contract_itm", $data['contract_itm'], TRUE);
		$this -> db -> where("contract_no", $data['contract_no'], TRUE);
		$this -> db -> update($this -> table, array('published' => $data['published']));
	}
