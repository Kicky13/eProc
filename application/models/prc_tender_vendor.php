<?php
class prc_tender_vendor extends CI_Model
{
	...
	public function join_pqi_item_retender() {
		$this->db->join('RETENDER_QUO_ITEM', 'PRC_TENDER_QUO_MAIN.PQM_ID = RETENDER_QUO_ITEM.PQM_ID', 'left');
		$this->db->join('RETENDER_ITEM', 'RETENDER_ITEM.TIT_ID = RETENDER_QUO_ITEM.TIT_ID', 'left');
	}
	...
	public function join_app_proses() {
		$this->db->join('APP_PROCESS', 'PRC_TENDER_MAIN.PTM_STATUS = APP_PROCESS.CURRENT_PROCESS AND APP_PROCESS.KEL_PLANT_PRO = PRC_TENDER_MAIN.KEL_PLANT_PRO AND APP_PROCESS.JUSTIFICATION = PRC_TENDER_MAIN.JUSTIFICATION AND APP_PROCESS.TIPE_SAMPUL = PRC_TENDER_MAIN.SAMPUL', 'left');
	...
	public function where_ptv_pc_code($pccode) {
		$this->db->where(array('PRC_TENDER_VENDOR.PTV_PC_CODE' => $pccode));
	}
	...
	public function order_by_pqi_tech_val() {
		$this->db->order_by('PQI_TECH_VAL', 'DESC');
	}
