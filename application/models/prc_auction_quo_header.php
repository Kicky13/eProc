<?php
class prc_auction_quo_header extends CI_Model
{
	...
	function list_auction($status){
		...
		foreach ($result as $key => $row)
		{
			...
			$PAQH_ID = $row['PAQH_ID'];
			$sql = "SELECT COUNT(PRC_TENDER_ITEM.TIT_ID) as jumlah FROM PRC_TENDER_ITEM WHERE (PRC_TENDER_ITEM.TIT_STATUS = '4' OR PRC_TENDER_ITEM.TIT_STATUS = '5') AND PRC_TENDER_ITEM.PAQH_ID = ? AND PRC_TENDER_ITEM.PTM_NUMBER = ?"; 
		...
		foreach ($data as $key => $row)
		{
			...
			if($status == 'proses' && strtotime($now) < oraclestrtotime($row['PAQH_AUC_END'])){
					$output[] = $data[$key];
				// }												
			} else if($status == 'tutup' && strtotime($now) > oraclestrtotime($row['PAQH_AUC_END']) && $row['IS_BREAKDOWN']==0){
				// if(($row['PAQH_OPEN_STATUS'] == 1 || $row['PAQH_OPEN_STATUS'] == 2) && strtotime($now) > oraclestrtotime($row['PAQH_AUC_END'])){
				$output[] = $data[$key];
			} else if($status == 'history' && $row['IS_BREAKDOWN']==1){
				// if(($row['PAQH_OPEN_STATUS'] == 1 || $row['PAQH_OPEN_STATUS'] == 2) && strtotime($now) > oraclestrtotime($row['PAQH_AUC_END'])){
				$output[] = $data[$key];
			} 
		}
