...
class ec_pen_shipment_m extends CI_Model
{
    ...
    public function getItemShipment($USERID){
        $this->db->select('TC.MATNO, TC.DATE_BUY, TC.PO_NO, BT.KODE_SHIPMENT, DTS.STATUS, TO_CHAR (DTS.SEND_DATE, \'dd-mm-yyyy hh24:mi:ss\') DATE_SEND, DTS.GR_NO, TO_CHAR (BT.IN_DATE_GR, \'dd-mm-yyyy hh24:mi:ss\') DATE_GR,DTS.KODE_DETAIL_SHIPMENT,DTS.KODE_DETAIL_SHIPMENT,DTS.QTY',
            false);
    ...
    public function updateStatus($kode_shipment, $gr_no, $gr_year){
        $this->db->trans_start();

        $this->db->where("KODE_DETAIL_SHIPMENT", $kode_shipment, TRUE);
...
//        $this->db->where('EC_PL_CONFIG_APPROVAL.UK_CODE', $userid, TRUE);
    ...
    function detail($PO)
    {
        $this->db->select('EC_T_CHART.MATNO,SM.MAKTX,EC_T_CHART.QTY,SM.MEINS,DP.PRICE,(EC_T_CHART.QTY*DP.PRICE) TOTAL,DP.PLANT,PL.PLANT_NAME,DP.DELIVERY_TIME',
            false);
