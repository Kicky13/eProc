...
*/
	...
	public function reminder(){ // function untuk melakukan peringatan (tombol peringatan) dengan mengrimim email ke vendor
		...
		foreach ($input as $key => $value) {
			...
			if ($opco == '7000' || $opco == '5000' || $opco == '2000') {
				$this->email->subject("Expired Document for eProcurement PT. Semen Indonesia (Persero) Tbk.");
			} else if ($opco == '3000') {
				$this->email->subject("Expired Document for eProcurement PT. Semen Padang.");
			} else if ($opco == '4000') {
				$this->email->subject("Expired Document for eProcurement PT. Semen Tonasa.");
			}
