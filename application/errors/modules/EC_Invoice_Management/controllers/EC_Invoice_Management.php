...
class EC_Invoice_Management extends MX_Controller {
    ...
    public function index($cheat = false) {
        ...
        $this->layout->add_js('pages/EC_inv_mngement.js');
    ...
    public function get_data() {
        ...
        $venno = $this->session->userdata['VENDOR_NO'];
        //	$venno = "0000112709";
        $_dataa = $this->ec_open_inv->getMan($venno);
    ...
    public function get_data_invoice() {
        ...
        $venno = $this->session->userdata['VENDOR_NO'];
        //$venno = "0000112709";
        $dataa = $this->ec_open_inv->get_Invoice(' where EIH.VENDOR_NO = \'' . $venno . '\'', ' order by EIH.CHDATE DESC');
        $kirim = array();
    ...
    public function createInvoice() {
        ...
        $FILE_BAST = $this->input->post('fileBast');
        $FILE_BAPP = $this->input->post('fileBapp');
        $VENDOR_NO = $this->session->userdata['VENDOR_NO'];
        $FILE_POMUT = $this->input->post('filePotMutu');
        ...
        if (!empty($uploaded) && $_FILES['fileInv']['name'] && $this->input->post('invoice_date') != "" && $this->input->post('total') != "") {
            $header_insert = array("NO_INVOICE" => $NO_INVOICE,
            ...
            if (!empty($FILE_BAST)) {
                $header_insert['BAST_PIC'] = $FILE_BAST;
            }

            if (!empty($FILE_BAPP)) {
                $header_insert['BAPP_PIC'] = $FILE_BAPP;
            }
            if (!empty($FILE_POMUT)) {
                $header_insert['POTMUT_PIC'] = $FILE_POMUT;
            }

            if (!empty($uploaded)) {
                if (isset($uploaded['filePotMutu'])) {
                    $header_insert['POTMUT_PIC'] = $uploaded['filePotMutu']['file_name'];
                }
                if (isset($uploaded['fileInv'])) {
                    $header_insert['INVOICE_PIC'] = $uploaded['fileInv']['file_name'];
                }
                if (isset($uploaded['filePO'])) {
                    $header_insert['PO_PIC'] = $uploaded['filePO']['file_name'];
                }
                if (isset($uploaded['fileBapp'])) {
                    $header_insert['BAPP_PIC'] = $uploaded['fileBapp']['file_name'];
                }
                if (isset($uploaded['fileBast'])) {
                    $header_insert['BAST_PIC'] = $uploaded['fileBast']['file_name'];
                }
                if (isset($uploaded['fileKwitansi'])) {
                    $header_insert['KWITANSI_PIC'] = $uploaded['fileKwitansi']['file_name'];
                }
                if (isset($uploaded['filespbyr'])) {
                    $header_insert['SPMHONBYR_PIC'] = $uploaded['filespbyr']['file_name'];
                }
                if (isset($uploaded['fileK3'])) {
                    $header_insert['K3_PIC'] = $uploaded['fileK3']['file_name'];
                }
            ...
            /* $i = 0;
              ...
              $_tmpgr = explode('#', $_gr);
              if($itemCat != '9'){
              $this->gr_sap->update(array('STATUS'=>1,'INV_NO' => $ID_INVOICE),array('BELNR'=>$_tmpgr[0],'BUZEI'=>$_tmpgr[1], 'GJAHR' => $_tmpgr[2],'STATUS' => 0));
              }else{
              $this->gr_sap->update(array('STATUS'=>1,'INV_NO' => $ID_INVOICE),array('LFBNR'=>$_tmpgr[0],'LFPOS'=>$_tmpgr[1], 'LFGJA' => $_tmpgr[2],'STATUS' => 0));
              }
              $i++;
              } */
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $pesan = "Invoice " . $NO_INVOICE . ", PO " . $NO_SP_PO . " created";
            $this->session->set_flashdata('message', $pesan);
            redirect("EC_Invoice_Management");
        }
    }
    ...
    public function update_invoice($ID_INVOICE) {
        ...
        //$venno = $this -> session -> userdata['VENDOR_NO'];
        //$venno = "0000112709";
        $this->ec_open_inv->setStatus_Invoice($ID_INVOICE, $this->uri->segment(4), $this->session->userdata['VENDOR_NAME']);
        $this->load->model('invoice/ec_invoice_header', 'eh');
        $dataInv = $this->eh->get(array('ID_INVOICE' => $ID_INVOICE));
        $pesan = "Invoice " . $dataInv->NO_INVOICE . ", PO " . $dataInv->NO_SP_PO . " Submitted";
        $this->session->set_flashdata('message', $pesan);
        //$json_data = array('data' => $dataa);
    ...
    public function tracking($ID_INVOICE = '') {
        header('Content-Type: application/json');
        $this->load->model('ec_open_inv');
        $datainvoice = $this->ec_open_inv->tracking($ID_INVOICE);
...
//                    echo $this->db->last_query();exit;
        ...
        return array('poHeader' => $poHeader, 'poAddress' => $poAddress, 'poSchedules' => $poSchedules, 'detailRR' => $detailRR, 'barcode' => $barcode, 'landed_cost' => $landed_cost);
    }
