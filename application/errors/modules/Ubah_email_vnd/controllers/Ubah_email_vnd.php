...
class Ubah_email_vnd extends CI_Controller {
	...
	{
		$data['title'] ='Ubah_email_vnd';
		$this->layout->set_table_js();
		$this->layout->set_table_cs();
		$this->layout->add_js('pages/ubah_email.js');
	...
	public function updateEmail() 
	{
		...
		$set_edit['EMAIL_ADDRESS'] = $this->input->post('EMAIL_ADDRESS');

		//--LOG MAIN--//
			$this->log_data->main($this->session->userdata['ID'],$this->session->userdata['FULLNAME'],
			$this->authorization->getCurrentRole(),'Update Email Vendor','SAVE',$this->input->ip_address());
			$LM_ID = $this->log_data->last_id();
		//--END LOG MAIN--//

		$this->ubah_vendor->updateEmail($set_edit, $where_edit);
		$this->hist_vnd_header->update($set_edit,$where_edit);
		//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Ubah_email_vnd/updateEmail','vnd_header','update',$set_edit, $where_edit);
		//--END LOG DETAIL--//

		//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Ubah_email_vnd/updateEmail','hist_vnd_header','update',$set_edit, $where_edit);
		//--END LOG DETAIL--//
		$data['status'] = 'success';
		$data['post'] = $this->input->post();
		//echo json_encode($data);

		redirect('Ubah_email_vnd/');
	}
}
