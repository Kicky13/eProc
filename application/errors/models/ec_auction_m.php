...
class ec_auction_m extends CI_Model {
	protected $tableHeader = 'EC_AUCTION_HEADER', $tableItem = 'EC_AUCTION_ITEM', $tablePeserta = 'EC_AUCTION_PESERTA', $tableItemTemp = 'EC_AUCTION_ITEM_temp', $tablePesertaTemp = 'EC_AUCTION_PESERTA_temp', $tableLog = 'EC_AUCTION_LOG';
	...
	public function get_hps($ID_USER) {
		$SQL = 'SELECT SUM(jumlah * price) as HPS from "EC_AUCTION_ITEM_temp" WHERE ID_USER=\'' . $ID_USER . '\'';
		$result = $this -> db -> query($SQL);
		return $result -> row_array();
	}
	...
	public function get_list_Peserta($NO_TENDER) {
		...
		$this -> db -> join("(SELECT MAX(LG.CREATED_AT) AS TGL, LG.VENDOR_NO FROM EC_AUCTION_LOG LG WHERE LG.NO_TENDER='". $NO_TENDER ."' AND LG.VENDOR_NO IN (SELECT PES.KODE_VENDOR FROM EC_AUCTION_PESERTA PES WHERE PES.ID_HEADER='". $NO_TENDER ."') GROUP BY LG.VENDOR_NO ORDER BY TGL ASC) TB1", 'TB1.VENDOR_NO=EC_AUCTION_PESERTA.KODE_VENDOR', 'INNER');
	...
	public function edit($data = '') {
		...
		$this -> db -> update('EC_AUCTION_HEADER');
	}
	...
	public function save($data, $user) {
		$SQL = "INSERT INTO EC_AUCTION_HEADER  
				values
				('" . $data['NO_TENDER'] . "','" . $data['DESC'] . "', '" . $data['LOCATION'] . "', TO_DATE('" . $data['TGL_BUKA'] . "', 'dd/mm/yyyy hh24:mi:ss'),
				 ...
				 '" . $data['HPS'] . "',1,'" . $data['NO_REF'] . "','" . $data['NOTE'] . "')";
		$this -> db -> query($SQL);
		// $this -> db -> insert($this -> tableHeader, $data);

		$SQL = 'INSERT INTO EC_AUCTION_ITEM
				SELECT *
				FROM "EC_AUCTION_ITEM_temp"
				where ID_USER=\'' . $user . '\'';
		$this -> db -> query($SQL);

		$SQL = 'DELETE from "EC_AUCTION_ITEM_temp" where ID_USER=\'' . $user . '\'';
		$this -> db -> query($SQL);
		$this -> db -> set('ID_HEADER', $data['NO_TENDER'], FALSE);
		$this -> db -> where('ID_HEADER', $user);
		$this -> db -> update('EC_AUCTION_ITEM');
		...
		$SQL = 'DELETE from "EC_AUCTION_PESERTA_temp" where ID_USER=\'' . $user . '\'';
		$this -> db -> query($SQL);
		$this -> db -> set('ID_HEADER', $data['NO_TENDER'], FALSE);
		$this -> db -> where('ID_HEADER', $user);
		$this -> db -> update('EC_AUCTION_PESERTA');
		...
		for ($i=0; $i < sizeof($dataa); $i++) { 
			$SQL = "INSERT INTO EC_AUCTION_LOG   
				values
