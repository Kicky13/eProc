...
class ec_list_auction extends CI_Model {
	protected $table_aunction = 'EC_AUCTION_HEADER', $tableItem = 'EC_AUCTION_ITEM', $tablePeserta = 'EC_AUCTION_PESERTA', $tableItemTemp = 'EC_AUCTION_ITEM_temp', $tablePesertaTemp = 'EC_AUCTION_PESERTA_temp', $tableLog = 'EC_AUCTION_LOG';
	...
	public function get_list_auction($KODE_VENDOR = '') {
		...
		$SQL = "SELECT
				...
				FROM
					EC_AUCTION_HEADER AH
				inner JOIN EC_AUCTION_PESERTA AP ON AH.NO_TENDER = AP.ID_HEADER
				WHERE
	...
	public function bid($KODE_VENDOR, $no_tender = '', $HARGATERKINI = '') {
		...
		$SQL = "INSERT INTO EC_AUCTION_LOG   
				...
				('" . $KODE_VENDOR . "',TO_DATE('" . date('d/m/Y h:i:s') . "', 'dd/mm/yyyy hh24:mi:ss'),'" . $HARGATERKINI . "', 
				 CASE 
				 WHEN (SELECT MAX(ITER) FROM EC_AUCTION_LOG WHERE VENDOR_NO='" . $KODE_VENDOR . "' AND NO_TENDER='" . $no_tender . "')IS NULL THEN 0
				 ELSE (SELECT MAX(ITER) FROM EC_AUCTION_LOG WHERE VENDOR_NO='" . $KODE_VENDOR . "' AND NO_TENDER='" . $no_tender . "')+1
				 END, '0', 
				 '" . $no_tender . "')";
		$this -> db -> query($SQL);
