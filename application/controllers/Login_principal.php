...
class Login_principal extends CI_Controller {
	...
	function loadLoginPrincipal() {
		...
		$this->layout->render('auth/login_principal',$data);
	}
	...
	function doLoginPrincipal() {
		...
		if ($username != '' && $password != '' && $companyid != '' && $captcha == $this->session->userdata('captchaWord')) {
			$loginStatusQuery = $this->user_principal_model->getUser($username, $password, $companyid);
			$loginStatus = $loginStatusQuery[0];
			//var_dump($loginStatusQuery);exit();
			if ($loginStatus) {
				// if ($loginStatus->STATUS == '4') {
					$this->session->set_userdata('loadPrincipalDashboard', TRUE);
					foreach ($loginStatus as $key => $value) {
							$this->session->set_userdata($key, $value);
						}
						$this->session->set_userdata('logged_in', TRUE);
						$this->session->set_userdata('is_principal', TRUE);
						$this->session->set_userdata('vendor_table', 'VND');
						$this->session->set_userdata('FULLNAME', $loginStatus->PC_NAME);
					redirect(base_url());
				// }
			}
			// else {
			// 	$loginStatusQuery = $this->user_principal_model->getTempUser($username, $password, $companyid);
			// 	$loginStatus = $loginStatusQuery[0];
			// 	// var_dump("masuk");
	...
	function doLogout() {
		if ($this->session->userdata('is_vendor')) {
			...
			redirect(site_url('Login_principal/loadLoginPrincipal'), 'refresh');
		}
		else {
			$this->session->sess_destroy();
			redirect(site_url('Login_principal'),'refresh');
		}
	}
}
