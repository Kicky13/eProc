...
class Register extends CI_Controller {
	...
	public function index() {
		...
		$data['company_name'] = $this->adm_company->as_dropdown('COMPANYNAME')->get_all(array('ISACTIVE' => 1));
		
		$this->layout->add_js('pages/registration.js');
		$this->layout->render('registration/form_start_registration',$data);
	}
	...
	public function initialize()
	{
		...
		if ((strtotime($today) - strtotime($startDate)) >= 0 and (strtotime($today) - strtotime($endDate)) <= 0) {
			...
			$this->layout->render('registration/form_create_user',$data);
		}
		else {
	...
	function get_prefix(){
		...
		$data = $this->m_vnd_prefix->get_child($id);
		...
		$data_company = $this->vendor_employe->join_company($companyid,$vendor_id);
	...
	function do_create_vendor2() {
		...
		$cekuser = $this->tmp_vnd_header->get_cek_user($username,$email);
	...
	function checkNPWPHasTaken($companyid) {
		$this->load->model(array('tmp_vnd_header', 'vnd_header'));
		$npwp = $this->input->post('npwp_no');
		$flag[0] = $this->vnd_header->get(array('NPWP_NO' => $npwp, 'COMPANYID' => $companyid));
		$flag[1] = $this->tmp_vnd_header->get(array('NPWP_NO' => $npwp, 'COMPANYID' => $companyid));
		$return = array('valid' => FALSE);
		if ($flag[0] || $flag[1]) {
			$return['message'] = 'NPWP sudah digunakan';
		}
		else {
			$return['valid'] = TRUE;
		}
