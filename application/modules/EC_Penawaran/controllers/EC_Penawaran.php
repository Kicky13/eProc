...
class EC_Penawaran extends CI_Controller
{
    ...
    public function index($kode = '-')
    {
        ...
        $data['plant'] = $this->ec_penawaran_m->getPlant();
        ...
        $this->layout->add_js('pages/EC_penawaran.js');
    ...
    {
        $this->load->model('ec_penawaran_m');
        $dataCurr = $this->ec_penawaran_m->get_MasterCurrency();
        $dataProduk = $this->ec_penawaran_m->get_data_produk($this->session->userdata['VENDOR_NO']);

        $json_data = array('curr' => $dataCurr, 'data' => $this->getALL($dataProduk));
    ...
    {
        $this->load->model('ec_penawaran_m');
        $dataPlant = $this->ec_penawaran_m->getPlant();
    ...
    public function insertHarga($matno)
    {
        ...
        $this->load->model('ec_penawaran_m');
        $tglAssign = $this->ec_penawaran_m->getData($venno, $matno);
...
//		if ($d1 <= $d2) {
        ...
        $this->load->model('ec_penawaran_m');
        $this->ec_penawaran_m->insertData($venno, $matno, $harga, $curr, $deliverytime, $stok, $in_date);
//			echo "ya";
//		}
//		echo "tdk";
...
//		if ($d1 <= $d2) {
        ...
        $in_date = date("Y-m-d H:i:s");
        $this->load->model('ec_penawaran_m');
        $this->ec_penawaran_m->insertStok($venno, $matno, $harga, $curr, $deliverytime, $stok, $in_date, $stokc);
//			echo "ya";
//		}
//		echo "tdk";
        //echo json_encode($tglAssign);
    ...
    public function getDetail($MATNR)
    {
        ...
        header('Content-Type: application/json');
        $this->load->model('ec_penawaran_m');
        $data = $this->ec_penawaran_m->getDetail($MATNR, $venno);
        echo json_encode($data);
    }
...
//            var_dump($matno);
    ...
    public function SaveHarga($matno)
    {
        ...
        header('Content-Type: application/json');
        $this->load->model('ec_penawaran_m');
        $data = $this->ec_penawaran_m->saveDetail($matno, $plant, $harga, $deliverytime, $venno, $curr);
//        var_dump($this->input->post());
//         foreach ($this->input->post() as $plant => $val) {
// //            var_dump($plant . " harga:" . $val[0] . " deliv:" . $val[1]);
// //            var_dump($matno);
