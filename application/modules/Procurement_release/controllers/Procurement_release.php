...
class Procurement_release extends CI_Controller {
	...
	public function save_bidding() {
		...
		if ($submit == "accept") {				
			...
			// thithe nambah delete pratender sap 28 desember 2017
			if(!empty($data[0]['PTM_PRATENDER']) || $data[0]['PTM_PRATENDER']!="" || $data[0]['PTM_PRATENDER']!=null){
				$this->deletePratender($data[0]['PTM_PRATENDER']);
			}
			// end
	...
	// thithe nambah delete pratender sap 28 desember 2017
	public function deletePratender($id) {
		error_reporting(E_ALL);
		$this->load->library('sap_handler');
		return $this->sap_handler->retenderPaket($id);
	}
	// end
