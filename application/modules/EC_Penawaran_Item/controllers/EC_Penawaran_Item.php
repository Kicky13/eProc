...
class EC_Penawaran_Item extends CI_Controller
{
    ...
    public function index($kode = '-')
    {
        ...
        $this->load->model('ec_penawaran_itm');
        $data['plant'] = $this->ec_penawaran_itm->getPlant();
        ...
        $this->layout->add_js('pages/EC_penawaran_item.js');
    ...
    {
        $this->load->model('ec_penawaran_itm');
        $dataCurr = $this->ec_penawaran_itm->get_MasterCurrency();
        $dataProduk = $this->ec_penawaran_itm->get_data_produk($this->session->userdata['VENDOR_NO'],$this->input->post('limitMin'), $this->input->post('limitMax'));
        $dataCount = $this->ec_penawaran_itm->get_Count_produk($this->session->userdata['VENDOR_NO']);
    ...
    {
        $this->load->model('ec_penawaran_itm');
        $dataPlant = $this->ec_penawaran_itm->getPlant();
    ...
    public function insertHarga($matno)
    {
        ...
        $this->load->model('ec_penawaran_itm');
        $tglAssign = $this->ec_penawaran_itm->getData($venno, $matno);
...
//		if ($d1 <= $d2) {
        ...
        $this->load->model('ec_penawaran_itm');
        $this->ec_penawaran_itm->insertData($venno, $matno, $harga, $curr, $deliverytime, $stok, $in_date);
//			echo "ya";
//		}
//		echo "tdk";
...
//		if ($d1 <= $d2) {
        ...
        $in_date = date("Y-m-d H:i:s");
        $this->load->model('ec_penawaran_itm');
        $this->ec_penawaran_itm->insertStok($venno, $matno, $harga, $curr, $deliverytime, $stok, $in_date, $stokc);
//			echo "ya";
//		}
//		echo "tdk";
        //echo json_encode($tglAssign);
    ...
    public function getDetail($MATNR)
    {
        ...
        header('Content-Type: application/json');
        $this->load->model('ec_penawaran_itm');
        $data = $this->ec_penawaran_itm->getDetail($MATNR, $venno);
...
//            var_dump($matno);
            if (is_array($val) && $val[0] != "" && $val[1] != "")
                $data = $this->ec_penawaran_itm->saveDetail($matno, $plant, $val[0], $val[1], $venno);
        }
        redirect("EC_Penawaran_Item");
    }
    ...
    public function SaveHarga($matno)
    {
        ...
        header('Content-Type: application/json');
        $this->load->model('ec_penawaran_itm');
        $data = $this->ec_penawaran_itm->saveDetail($matno, $plant, $harga, $deliverytime, $venno, $curr);
//        var_dump($this->input->post());
//         foreach ($this->input->post() as $plant => $val) {
// //            var_dump($plant . " harga:" . $val[0] . " deliv:" . $val[1]);
// //            var_dump($matno);
