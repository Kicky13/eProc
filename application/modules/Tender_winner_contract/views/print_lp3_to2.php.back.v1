                ...
                foreach ($vendor_code as $vendor)
                {
                    ...
                    if($i!=1){
                        ...
                        <tr>
                            ...
                            <?php foreach ($pesertavendor as $vnd):?>
                                <?php if ($vendor == $vnd['PTV_VENDOR_CODE']): ?>
                                    <td class="kiri data"><?php echo $vnd['VENDOR_NAME']; ?></td>

                                    <td nowrap class="tengah data"><?php echo $vnd['PTV_RFQ_NO']; ?></td>

                                    <td nowrap class="kanan data">
                                        <?php 
                                        if(isset($ptqi[$val['TIT_ID']][$vnd['PTV_VENDOR_CODE']]['PQI_FINAL_PRICE']) && $k!=1){ ?>
                                        <?php   echo number_format($ptqi[$val['TIT_ID']][$vnd['PTV_VENDOR_CODE']]['PQI_PRICE']); ?>
                                        <?php } 
                            ...
                            <td class="kanan data">
                                ...
                                <?php
                                if(isset($data['DATA']['ITEM'][$val['TIT_ID']][$vendor])){
                                    if(number_format($data['DATA']['ITEM'][$val['TIT_ID']][$vendor]) == "0"){
                                        echo number_format($data['DATA']['ITEM'][$val['TIT_ID']][$vendor]);
                                    } else {
                                        echo number_format($data['DATA']['ITEM'][$val['TIT_ID']][$vendor]);
                                    }
                                } else {
