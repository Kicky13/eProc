...
class Tender_winner_contract extends CI_Controller {
	...
	public function index() {
		...
		$this->layout->add_js('pages/prc_tender_winner_contract_list.js');
	...
	public function listapprovalPO(){ //udah ada filter user.
		...
		$data['data']=$header;		
		$this->layout->add_js('pages/list_contract.js');
	...
	*/
	public function approve() {
		...
		if($is_approve==1){//approve
			...
			if($max_approve == $real_stat){
				...
				$ret = $this->create_contract($id, $ptw);
				// var_dump($ret);
				// die(var_dump($ret));
				$sukses = false;
				$tbody='';
				...
				if($sukses){
					$this->po_approval->update(array('IS_APPROVE' => $is_approve,"CREATED_DATE" => date('d-M-Y g.i.s A')), $where_poapprove);
                		//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','po_approval','update',array('IS_APPROVE' => $is_approve,"CREATED_DATE" => date('d-M-Y g.i.s A')),$where_poapprove);
						//--END LOG DETAIL--//

					$this->po_header->update(array('CONTRACT_NUMBER' => $po_no,'IS_APPROVE'=>$is_approve,'RELEASED_AT' => date('d-M-Y g.i.s A'),'REAL_STAT' => intval($data[0]['REAL_STAT']) + 1 ), array("PO_ID" => $id));			
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','po_header','update',array('PO_NUMBER' => $po_no,'IS_APPROVE'=>$is_approve,'RELEASED_AT' => date('d-M-Y g.i.s A'),'REAL_STAT' => intval($data[0]['REAL_STAT']) + 1, 'PTM_NUMBER'=>$data[0]['PTM_NUMBER']), array("PO_ID" => $id));
						//--END LOG DETAIL--//
					...
					foreach ($po_detail as $key_po => $value_po) {
						...
						$this->prc_tender_item->update(array('TIT_STATUS'=>10),$where_tit);
							//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','prc_tender_item','update',array('TIT_STATUS'=>10),$where_tit);
							//--END LOG DETAIL--//
						...
						$this->prc_pr_item->update(array('PPI_QTY_USED'=>($ppi[0]['PPI_QTY_USED']-($tit[0]['TIT_QUANTITY']-$value_po['POD_QTY'])),'PPI_POQUANTITY'=>$value_po['POD_QTY']),array('PPI_ID'=>$value_po['PPI_ID']));
							//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','prc_pr_item','update',array('PPI_QTY_USED'=>($ppi[0]['PPI_QTY_USED']-($tit[0]['TIT_QUANTITY']-$value_po['POD_QTY'])),'PPI_POQUANTITY'=>$value_po['POD_QTY']),array('PPI_ID'=>$value_po['PPI_ID']));
							//--END LOG DETAIL--//
					}
				...
				$this->po_header->update(array('REAL_STAT' => intval($data[0]['REAL_STAT']) + 1), array("PO_ID" => $id));
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','po_header','update',array('REAL_STAT' => intval($data[0]['REAL_STAT']) + 1, 'PTM_NUMBER'=>$data[0]['PTM_NUMBER']), array("PO_ID" => $id));
					//--END LOG DETAIL--//
			...
			$this->po_header->update(array('IS_APPROVE'=>$is_approve), array("PO_ID" => $id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','po_header','update',array('IS_APPROVE'=>$is_approve,'PTM_NUMBER'=>$data[0]['PTM_NUMBER']), array("PO_ID" => $id));
				//--END LOG DETAIL--//
			...
			$this->prc_process_holder->update($s,$w);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','prc_process_holder','update',$s,$w);
				//--END LOG DETAIL--//
		}
		...
		$this->po_header_comment->insert($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_winner_contract/approve','po_header_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		// 	redirect('Tender_winner_contract/listapprovalPO');
		// }
		// if ($retval['REL_INDICATOR_NEW'] == 'G') {
		// 	redirect('Tender_winner_contract/releasePO/'.$po_no);
		// }
	...
	public function releasePO($id){
		...
		$this->layout->add_js('pages/prc_tender_winner_contract_edit.js');
...
//tambah fungsi ambil semua data PO thithe 16 oktober 2017
	...
	public function saveReleasePO(){		
		...
		// echo json_encode($data);
		redirect('Tender_winner_contract/editReleasePO/'.$data1['PO_ID']);
	}
	...
	public function renego($id){
		...
		$this->po_header->update(array('IS_APPROVE'=>3),array('PO_ID'=>$id));
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_winner_contract/renego','po_header','update',array('IS_APPROVE' => 3,'PTM_NUMBER'=>$ptm),array('PO_ID'=>$id));
			//--END LOG DETAIL--//
		...
		foreach ($po_detail as $key_po => $value_po) {
			...
			$this->prc_tender_item->update(array('TIT_STATUS'=>5),$where_tit);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/renego','prc_tender_item','update',array('TIT_STATUS'=>5),$where_tit);
				//--END LOG DETAIL--//
			$tit = $this->prc_tender_item->get($where_tit);			
		}
		redirect('Tender_winner_contract/listPO');
	...
	public function retender($id){
		...
		// $this->po_header->update(array('IS_APPROVE'=>4),array('PO_ID'=>$id));
			//--LOG DETAIL--//
		// $this->log_data->detail($LM_ID,'Tender_winner_contract/retender','po_header','update',array('IS_APPROVE' => 4,'PTM_NUMBER'=>$ptm),array('PO_ID'=>$id));
			//--END LOG DETAIL--//
	...
	public function cancel($id){
		...
		$this->po_header->update(array('IS_APPROVE'=>5),array('PO_ID'=>$id));
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_winner_contract/cancel','po_header','update',array('IS_APPROVE' => 5,'PTM_NUMBER'=>$ptm),array('PO_ID'=>$id));
			//--END LOG DETAIL--//
		...
		foreach ($po_detail as $key_po => $value_po) {
			...
			$this->prc_tender_item->update(array('TIT_STATUS'=>999),$where_tit); //set status item jadi (999)'dibatalkan'
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/cancel','prc_tender_item','update',array('TIT_STATUS'=>999),$where_tit);
				//--END LOG DETAIL--//
			$tit = $this->prc_tender_item->get($where_tit);			
		}
		...
		if(count($pti)==0){
			$this->prc_tender_main->update(array('PTM_STATUS'=>-999),array('PTM_NUMBER'=>$ptm)); //set status ptm jadi (-999)'dibatalkan'
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/cancel','prc_tender_main','update',array('PTM_STATUS'=>-999),array('PTM_NUMBER'=>$ptm));
				//--END LOG DETAIL--//
		}
		...
		if($holder){
			...
			$this->prc_process_holder->update($s,$w);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','prc_process_holder','update',$s,$w);
				//--END LOG DETAIL--//
		}
	...
	public function detailPO($id,$is_approval = false,$is_print='false'){
		...
		if($is_print=='true'){
			...
			$this->layout->add_js('pages/detail_contract_approve.js');		
  ...
  //       pdf_create($html, $filename, $paper, $orientation,false);
	...
	public function submit() {	
		...
		if($po_id == null){
			...
			foreach ($winner as $key => $each_winner) {
				...
				$this->prc_tender_winner->update($data, array('PTW_ID' => $each_winner));
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','prc_tender_winner','update',$data,array('PTW_ID' => $each_winner));
					//--END LOG DETAIL--//
				...
				if($satu == 0){
					...
					$po = array(
						...
						'LINK' => $idhash,
						'IS_CONTRACT' => 1,
						'START_DATE' => oracledate(strtotime($startdate)),
						'END_DATE' => oracledate(strtotime($enddate))
						);
					$this->po_header->insert($po);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_header','insert',$po);
						//--END LOG DETAIL--//
					...
					if(isset($desc)&&$desc!=''){
						foreach ($desc as $key => $dsc) {
							...
							$this->po_delivery->insert($datas);
								//--LOG DETAIL--//
							$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_delivery','insert',$datas);
								//--END LOG DETAIL--//
						}
					}
				}
				...
				$this->po_detail->insert($pod);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_detail','insert',$pod);
					//--END LOG DETAIL--//
				...
				$this->prc_tender_item->update($data_tit,$where_tit);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','prc_tender_item','update',$data_tit,$where_tit);
					//--END LOG DETAIL--//
			}
				...
				if(!empty($value['BATAS_HARGA']) && $value['BATAS_HARGA']!='NULL'){
					if($total <= $value['BATAS_HARGA']){				
						...
						$this->po_approval->insert($datas);
							//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_approval','insert',$datas);
							//--END LOG DETAIL--//
						$count = $count + 1;
						break;
			...
			$this->po_header->update(array("TOTAL_HARGA" => $total,"RELEASE" => $count),array('PO_ID' => $po_id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_header','update',array("TOTAL_HARGA" => $total,"RELEASE" => $count),array('PO_ID' => $po_id));
				//--END LOG DETAIL--//
			...
			$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_header','update',$poUpdate,array('PO_ID' => $po_id));
				//--END LOG DETAIL--//
			...
			foreach ($po_detail as $key_pod => $value_pod) {			
				...
				$this->po_detail->update($pod,array('POD_ID'=>$value_pod['POD_ID']));	
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_detail','update',$pod,array('POD_ID' => $value_pod['POD_ID']));
					//--END LOG DETAIL--//
			...
			// 		//--LOG DETAIL--//
			// 	$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_approval','update',array('IS_APPROVE'=>0),array('ID'=>$value_po_app['ID']));
			// 		//--END LOG DETAIL--//
			// }
		...
		$this->po_header_comment->insert($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_winner_contract/submit','po_header_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		redirect('Tender_winner_contract');
	}
	...
	public function detail($po_id = null) {
		...
		$data['doctype'] = $this->adm_doctype_pengadaan->get(array('CAT' => 'K'));
		$this->layout->set_datetimepicker();
		// $this->layout->set_table_js();
		// $this->layout->set_table_cs();
		$this->layout->add_js('pages/tender_winner_contract.js');
		$this->layout->add_css('plugins/bootstrap-datepicker/datepicker.css');
		$this->layout->add_js('plugins/bootstrap-datepicker/bootstrap-datepicker.js');
	...
	public function create_po($is_contract,$ptw){
		...
		foreach ($ptw as $key => $each_winner) {			
			...
			$item_text = $data_item[0]['ITEM_TEXT'];
	...
	public function create_contract($id, $ptw){
		$this->load->model('prc_tender_winner');
		$this->load->model('po_detail');
		$this->load->model('po_header');
		$this->load->library('sap_handler');
		...
		$poitem = array();		
		foreach ($ptw as $key => $each_winner) {		
			$ph = $this->po_header->get(array('PO_ID'=>$id));	
			$data = $this->prc_tender_winner->get($each_winner);
			$doc_type = $this->input->post('doctype');
			$po_item = $pod_number;
			$ptm = $data[0]['PTM_NUMBER'];
			$pur_group = $data[0]['PPR_PGRP'];
			$purch_org = pgrp_to_porg($pur_group);
			$docdate = date('d.m.Y', oraclestrtotime($ph[0]['DOC_DATE']));
			$startdate = date('d.m.Y', oraclestrtotime($ph[0]['START_DATE']));
			$enddate = date('d.m.Y', oraclestrtotime($ph[0]['END_DATE']));

			$poheader = compact('doc_type', 'pur_group', 'purch_org', 'docdate', 'startdate', 'enddate');
		...
		// var_dump($poheader);
		// var_dump($poitem);
		$retval = $this->sap_handler->create_contract_strategis($poheader,$poitem);
		// var_dump($retval);
		/*if($is_contract == 1){
			$retval = $this->sap_handler->create_po_contract($poheader,$poitem);
		}else{
