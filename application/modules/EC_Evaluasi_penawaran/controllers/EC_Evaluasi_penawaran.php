...
class EC_Evaluasi_Penawaran extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('user_principal_model');
	...
	public function index($id = '') {
		...
		$this->load->model('prc_tender_quo_eval');
		$this->load->model('prc_tender_vendor');
		$this->load->model('m_tambahan_ub');
		$this->load->model('prc_preq_template_detail');
		$this->load->model('prc_evaluation_template');

		$data['title'] = 'Evaluasi Teknis Baru';
		...
		foreach ($data['vendor_data'] as $key => $val) {
			$data['evatek'][$val['VENDOR_NO']] = $this->prc_tender_quo_eval->ptqe($val['PQM_ID']);
		}
		$data['template_eval'] = $this->get_detail_template($data['ptp']['PTP_EVALUASI_TEKNIS']);
		// var_dump($data);
		$this->layout->add_css('plugins/selectize/selectize.css');
		$this->layout->add_js('plugins/selectize/selectize.js');
		$this->layout->add_js('pages/EC_evaluasi_teknis.js');
	...
	private function template_update($id, $ppt_id){
		...
		$ansi = $this->load->view('EC_Evaluasi_penawaran/evaluasi_template_update', $data, true);
		return $ansi;
	}
	...
	public function save_bidding() {
		...
		if ($template_uraian == false) {
			...
			redirect('EC_Evaluasi_penawaran/index/' . $id);
		}
		...
		for ($i = 1; $i <= $nfiles ; $i++) { 
			...
			if (!empty($uploaded['add_doc'.$i])) {
				...
				$this->prc_add_item_evaluasi->insert($add);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_add_item_evaluasi','insert',$add);
					//--END LOG DETAIL--//
			}
		}
		...
		$this->prc_tender_prep->updateByPtm($id, $ptp);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_tender_prep','update',$ptp,array('PTM_NUMBER'=>$id));
			//--END LOG DETAIL--//
		...
		foreach ($dpet as $dval1) {
			...
			foreach ($dpde as $dval2) {
				...
				$this->prc_tender_quo_item->update(array('PQI_TECH_VAL' => 0), array('PQI_ID' => $dval2['PQI_ID']));
					...
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_do_evatek_uraian','delete',null,array('DET_ID' => $dval2['DET_ID']));
				// $this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_tender_quo_item','update',array('PQI_TECH_VAL' => 0),array('PQI_ID' => $dval2['PQI_ID']));
					//--END LOG DETAIL--//
			}
			$this->prc_do_evatek->delete(array('ET_ID' => $dval1['ET_ID']));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_do_evatek','delete',null,array('ET_ID' => $dval1['ET_ID']));
				//--END LOG DETAIL--//
		}
		$this->prc_evaluasi_uraian->delete(array('PTM_NUMBER' => $id));
		$this->prc_evaluasi_teknis->delete(array('PTM_NUMBER' => $id));
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_evaluasi_uraian','delete',null,array('PTM_NUMBER' => $id));
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_evaluasi_teknis','delete',null,array('PTM_NUMBER' => $id));
			//--END LOG DETAIL--//
			//-- end Delete --//
		...
		$this->prc_evaluasi_teknis->insert_batch($petInsert); 
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_evaluasi_teknis','insert',$petInsert);
			//--END LOG DETAIL--//
			...
			foreach ($pqm as $d) { 
				...
				if(!empty($pqi[0]['PQI_ID'])){
					...
					$this->prc_do_evatek->insert($pdet);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_do_evatek','insert',$pdet);
						//--END LOG DETAIL--//
					...
					foreach (array_unique($pptu) as $uraian) { //proses insert nilai child
						if(isset($valPerChild[$noVnd][$val['TIT_ID']][$val['PPD_ID']][$uraian['PPTU_ID']])){ 
							...
							$this->prc_evaluasi_uraian->insert($new_eu);
								//--LOG DETAIL--//
							$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_evaluasi_uraian','insert',$new_eu);
								//--END LOG DETAIL--//
							...
							$this->prc_do_evatek_uraian->insert($pdeu);
								//--LOG DETAIL--//
							$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_do_evatek_uraian','insert',$pdeu);
								//--END LOG DETAIL--//
						}
					}	
				}
		...
		foreach ($this->input->post('total') as $key => $total) {
			$this->prc_tender_quo_item->update(array('PQI_TECH_VAL' => $total), array('PQI_ID' => $key));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_tender_quo_item','update',array('PQI_TECH_VAL' => $total),array('PQI_ID' => $key));
				//--END LOG DETAIL--//
		}
		...
		foreach ($this->input->post('pqi_note') as $key => $note) {
			$this->prc_tender_quo_item->update(array('PQI_NOTE' => $note), array('PQI_ID' => $key));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_tender_quo_item','update',array('PQI_NOTE' => $note),array('PQI_ID' => $key));
				//--END LOG DETAIL--//
		}
		...
		$this->comment->insert_comment_tender($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if ($next_process == '1') {
			...
			if($ptm_status != '11'){//status proses EVATEK
				...
				$this->prc_chat->update($set, array('PTM_NUMBER'=>$id));
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_bidding','prc_chat','update',$set,array('PTM_NUMBER'=>$id));
					//--END LOG DETAIL--//
			}
	...
	public function assign() {
		...
		if (empty($user)) {
			redirect('EC_Evaluasi_penawaran/index/' . $ptm_number);
		}
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_Penawaran/assign','prc_evaluator','insert',$pe);
			//--END LOG DETAIL--//
	...
	public function save_harga() {
		...
		if ($next_process == '999') { //BATAL
			redirect('Proc_verify_entry/batal/' . $id .'/'.$this->input->post('process_name').'/EC_Evaluasi_penawaran/');
		}
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if ($next_process == '5') { // re evaluasi
			...
			$this->prc_chat->update($set, array('PTM_NUMBER'=>$id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_chat','update',$set,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			$this->prc_process_holder->update($h, array('PTM_NUMBER'=>$id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_process_holder','update',$h,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//

			$t['PTM_STATUS']=11;
			$this->prc_tender_main->updateByPtm($id, $t);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_tender_main','update',$t,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
		...
		foreach ($tit_weight as $key => $val) {
			...
			$this->prc_tender_item->update($set, $whe);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_tender_item','update',$set,$whe);
				//--END LOG DETAIL--//
		}
		...
		foreach ($this->input->post('pqi') as $pqi_id => $value) {
			...
			$this->prc_tender_quo_item->update($set, array('PQI_ID' => $pqi_id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga','prc_tender_quo_item','update',$set,array('PQI_ID' => $pqi_id));
				//--END LOG DETAIL--//
		}	
	...
	public function save_harga_paket() {
		...
		if ($next_process == '999') { //BATAL
			redirect('Proc_verify_entry/batal/' . $id .'/'.$this->input->post('process_name').'/EC_Evaluasi_penawaran/index_harga');
		}
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if ($this->input->post('next_process') == '5') { // re evaluasi
			...
			$this->prc_chat->update($set, array('PTM_NUMBER'=>$id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_chat','update',$set,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			$this->prc_process_holder->update($h, array('PTM_NUMBER'=>$id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_process_holder','update',$h,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//

			$t['PTM_STATUS']=11;
			$this->prc_tender_main->updateByPtm($id, $t);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_tender_main','update',$t,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			$this->session->set_flashdata('success', 'success'); 
			redirect('Job_list');
		}
		...
		foreach ($tit_weight as $key => $val) {
			...
			$this->prc_tender_item->update($set, $whe);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_tender_item','update',$set,$whe);
				//--END LOG DETAIL--//
		}
		...
		foreach ($this->input->post('pqi') as $pqm_id => $value) {
			...
			$this->prc_tender_quo_item->update($set, array('PQM_ID' => $pqm_id));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_harga_paket','prc_tender_quo_item','update',$set,array('PQM_ID'=>$pqm_id));
				//--END LOG DETAIL--//
	...
	public function get_evaluasi_detail(){
		...
		$ans = $this->load->view('EC_Evaluasi_penawaran/evaluasi_teknis_template', $data, true);
	...
	public function save_pesan(){
		...
		if($tndr_main['MASTER_ID']==13){ //evaluasi harga
			...
			$this->prc_chat->update($set, array('VENDOR_NO'=>$no_vendor, 'PTM_NUMBER' => $ptm));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Evaluasi_penawaran/save_pesan','prc_chat','update',$set,array('PTM_NUMBER'=>$ptm));
				//--END LOG DETAIL--//
		}

			//--LOG DETAIL--//
		// $this->log_data->detail($LM_ID,'EC_Evaluasi_Penawaran/save_pesan','prc_chat','insert',$add);
			//--END LOG DETAIL--//
		$data['th']=$th;
	...
	public function viewHistoryPesan($ptm, $vendor_no=null){
		...
		$data['balas'] = true;
		$ans = $this->load->view('EC_Evaluasi_penawaran/history_pesan', $data, true);
	...
	public function search_assign(){
		...
		echo $this->load->view('EC_Evaluasi_penawaran/view_assign', $data, true);
	}
	...
	public function get_detail_template($evt_id){
		// $this->load->model('prc_evaluation_template');
