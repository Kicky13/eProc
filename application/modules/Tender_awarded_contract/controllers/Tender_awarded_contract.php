...
class Tender_awarded_contract extends CI_Controller {
	...
	function index() {
		...
		$this->layout->add_js('pages/tender_awarded_contract.js');
	...
	function cetak_po($id) {
		...
		$ptv = $this->prc_tender_vendor->ptm($data['po']['PTM_NUMBER']);
		$data['ptv'] = $ptv[0];
		// var_dump($data['ptv']);
	...
	function get_tender_awarded_list()
	{
		...
		$po = $this->po_header->get(array('VND_CODE'=>$this->session->userdata('VENDOR_NO'),'RELEASE'=>2,'PO_NUMBER'=>NULL,'IS_CONTRACT'=>1));
		$count = 0;
		$data = array();
		foreach ((array) $po as $key => $value) {	
			...
			foreach ($pod as $key_pod => $value_pod) {
				...
				$data[$count]['START_DATE'] = betteroracledate(oraclestrtotime($value['START_DATE']));
				$data[$count]['END_DATE'] = betteroracledate(oraclestrtotime($value['END_DATE']));
