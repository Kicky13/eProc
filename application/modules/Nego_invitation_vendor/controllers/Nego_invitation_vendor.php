...
class Nego_invitation_vendor extends CI_Controller {
	...
	function index() {
		...
		$this->layout->add_js('pages/nego_invitation_vendor.js');
	...
	function detail_invitation($ptv_id) {
		...
		$data['title'] = "Nego Invitation Detail Vendor";
		...
		$this->layout->set_table_js();
		$this->layout->add_js('pages/nego_invitation_vendor.js');
		$this->layout->add_js('pages/nego_invitation_vendor_detail.js');
	...
	function do_update_tender_invitation($ptv_id) {
		...
		if(strtotime(date('d-M-Y g.i.s A'))>$negoend_time){
			...
			redirect('Nego_invitation_vendor/detail_invitation/' . url_encode($ptv_id));
			exit();
		}
		...
		if(!empty($_FILES['file_negosiasi']['name'])){
			...
			$this->prc_tender_nego_vendor_doc->insert($ptnvd);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Nego_invitation_vendor/do_update_tender_invitation','prc_tender_nego_vendor_doc','insert',$ptnvd);
				//--END LOG DETAIL--//
		}
		...
		foreach ($this->input->post('pqi') as $val) {
			...
			//if ($negodetail[0]['HARGA'] != $valfinalprice) {
				...
				$this->log_data->detail($LM_ID,'Nego_invitation_vendor/do_update_tender_invitation','prc_nego_detail','update',$setnegodetail2,$wherenegodetail);
					//--END LOG DETAIL--//
			//}
		}
		//*/

		if($ptv[0]['PTV_TENDER_TYPE']==1){
			$this->prc_tender_vendor->update(array('PTV_APPROVAL' => '2'),array('PTV_ID' => $ptv_id));
		}
