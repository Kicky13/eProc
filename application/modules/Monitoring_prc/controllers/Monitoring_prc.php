...
class Monitoring_prc extends CI_Controller {
	...
	public function detail($id){
		...
		$data['doc_nego'] = $this->prc_tender_nego->ptm_many($id);
	...
	public function get_datatable() {
		...
		if($this->session->userdata('COMPANYID')=='3000' || $this->session->userdata('COMPANYID')=='4000'){ //PADANG and TONASA
			...
			// GRESIK, INDONESIA Tbk, INDONESIA VO 

			// $this->load->model('adm_mrpc');
			// $mrp_plant = $this->adm_mrpc->get(array('ID'=>$this->session->userdata('ID')));
		...
		foreach ((array)$datatable as $key => $val) {
			...
			$pr = $val['PPI_PRNO'];
			if(empty($val['PPI_PRNO'])){
				$this->retender_item->join_pr();
				$ambilpr_retender = $this->retender_item->ptm($val['PTM_NUMBER']);
				$datatable[$key]['PPI_PRNO'] = $ambilpr_retender[0]['PPI_PRNO'];
				$pr = $ambilpr_retender[0]['PPI_PRNO'];
			}
			$hitungPR = $this->prc_pr_item->get(array('PPI_PRNO' => $pr));	
			if(count($hitungPR)>0){
				$tender_brpx = $hitungPR[0]['COUNT_TENDER'];
			} else {
				$tender_brpx = 0;
			}			
			//$datatable[$key]['hitungPRA']=count($hitungPR);
			$datatable[$key]['hitungPRA']=$tender_brpx;

			$buyer = $this->adm_employee->get(array('ID' => $val['PTM_ASSIGNMENT']));				
			$datatable[$key]['buyer']=$buyer[0]['FULLNAME'];
			$datatable[$key]['cek_buyer_gak']=$kd_user;
		}
	...
	public function get_item_retender($ptm = null){
		$this->load->model('prc_tender_item');
		$this->load->model('prc_tender_main');
		$this->prc_tender_item->join_pr();
		$pti=$this->prc_tender_item->ptm($ptm); //diambil nomor ppi_ID-nya
		$ppiid = $pti[0]['PPI_ID'];
		$stat = 999;
		$itemt = $this->prc_tender_item->ppiid($ppiid,$stat);
		$ptmt = $itemt[0]['PTM_NUMBER'];
		$ptit = $this->prc_tender_main->ptm($ptmt);
		// echo "<pre>";
		// print_r($ptit);die;
		$status=array();
		foreach ($ptit as $value) {
			$status[] = array('SLB'=>$value['PTM_SUBPRATENDER'],'LB'=>$value['PTM_PRATENDER'],'NAME'=>$value['PTM_SUBJECT_OF_WORK'], 'PTMT'=>$value['PTM_NUMBER']);
		}
