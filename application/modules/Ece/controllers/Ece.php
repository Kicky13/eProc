...
class Ece extends CI_Controller {
	...
	public function change($id, $group_id){
		...
		for($i=0;$i<count($tit);$i++){
			$ambil_rendah = $this->prc_tender_quo_item->getHargaTerendah($tit[$i]);
			$data['harga_terendah'][$tit[$i]] = $ambil_rendah[0];
		}
	...
	public function approv($id, $group_id){
		...
		foreach ($pec as $val) {
			...
			$ambil_rendah = $this->prc_tender_quo_item->getHargaTerendah($val['TIT_ID']);
			$data['pec'][$val['TIT_ID']]['PQI_PRICE'] = $ambil_rendah[0]['PQI_PRICE'];
		}
