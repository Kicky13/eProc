...
class Monitoring_prc_user extends CI_Controller {
	...
	public function index($cheat = false) {
		...
		$this->layout->add_js('pages/monitor_prc_user.js');		
	...
	public function get_datatable() {
		$this->load->model('prc_tender_main_user');
		$this->load->model('prc_tender_item_user');
		...
		$this->prc_tender_main_user->join_latest_activity();
		$this->prc_tender_main_user->join_prep();
		$this->prc_tender_main_user->join_prItem();
		$this->prc_tender_main_user->join_pr_req();
		$this->prc_tender_main_user->where_req_in($MKCCTR);
		$data = array();

		$datatable = $this->prc_tender_main_user->get();
		foreach ((array)$datatable as $key => $val) {
			if($val['MASTER_ID']==15){
				$rg=array();
				$pti = $this->prc_tender_item_user->ptm($val['PTM_NUMBER']);
