...
class Procurement_undang_harga extends CI_Controller {
	...
	public function index($id) {
		...
		if ($data['ptm']['PTM_STATUS'] == 19 && $data['ptm']['PENAWARAN_KE']!=''){
	...
	public function save_bidding() {
		...
		//--LOG MAIN--//
		$this->log_data->main($this->session->userdata['ID'],$this->session->userdata['FULLNAME'],
			$this->authorization->getCurrentRole(),'Penawaran Ulang',$action,$this->input->ip_address()
			);
		$LM_ID = $this->log_data->last_id();
			//--END LOG MAIN--//
		if ($next_process == 4){
			...
			if($this->is_kadep){
				...
				// print_r($emp);die;
				$id['EMP_ID'] = $emp[0]['ID'];
				$this->prc_process_holder->update($id, array('PTM_NUMBER'=>$ptm_number));
			}else{
				if(!empty($batas_tanggal_harga)){
					$batas_tanggal_harga = strtotime($this->input->post('batas_vendor_harga'));
					$batas = oracledate($batas_tanggal_harga);
				...
				$approval = 1;
				$this->prc_tender_main->updateByPtm($ptm_number, array('APPROVAL_PENAWARAN' => $approval));

				$atasan1 = $this->adm_employee->atasan($this->authorization->getEmployeeId());
				// echo "<pre>";
				// print_r($atasan1);die;
				$id['EMP_ID'] = $atasan1[0]['ID'];
				$this->prc_process_holder->update($id, array('PTM_NUMBER'=>$ptm_number));
		...
		//tambah note untuk vendor 24 april 2018
		$NOTE_UNTUK_VENDOR=$this->input->post('NOTE_UNTUK_VENDOR');
		if(!empty($NOTE_UNTUK_VENDOR)){
			$this->prc_tender_main->updateByPtm($ptm_number, array('NOTE_UNTUK_VENDOR' => $NOTE_UNTUK_VENDOR));
			$this->log_data->detail($LM_ID,'Procurement_undang_harga/save_bidding','prc_tender_main','update',array('NOTE_UNTUK_VENDOR' => $NOTE_UNTUK_VENDOR),array('PTM_NUMBER'=>$ptm_number));
				//--END LOG DETAIL--//
		}
		// end
