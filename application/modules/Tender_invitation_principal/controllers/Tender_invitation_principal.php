...
class Tender_invitation_principal extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model(array('prc_tender_vendor','prc_tender_item','user_principal_model'));
	}
	...
	function index() {
		...
		$this->layout->add_js('pages/tender_invitation_principal.js');
	...
	function detail_invitation($ptv_id) {
		...
		$this->layout->set_table_js();
		$this->layout->set_table_cs();
		$this->layout->add_js('pages/detail_invitation_principal.js');
	...
	function do_update_tender_invitation($ptv_id) {
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_invitation_principal/do_update_tender_invitation','prc_tender_vendor','update',array('PTV_STATUS' => $status, 'ALASAN' => $alasan, 'PTM_NUMBER' => $ptm_number),array('PTV_ID' => $ptv_id));
			//--END LOG DETAIL--//
