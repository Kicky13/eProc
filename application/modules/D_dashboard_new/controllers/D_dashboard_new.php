...
class D_dashboard_new extends CI_Controller {
	...
	public function loadVendorDashboard()
	{
		...
		if($vnd_header['STATUS_PERUBAHAN']==1){
			...
			$this->load->model('ec_dashboard');

			$get_Company = $this -> ec_dashboard -> get_MasterCompany($this->session->userdata('VENDOR_NO'));
			//$get_Company = $this -> ec_dashboard -> get_MasterCompany('0000112914');

			$data['company_id'] = $get_Company[0]['COMPANYID'];
			$data['company_name'] = $get_Company[0]['COMPANYNAME'];
			/* daftar invoice yang bisa ditagihkan */
			...
			$this->layout->render('vendor_dashboard_new', $data);
