...
class Vendor_Management extends CI_Controller {
	...
	public function approve_regisration($vendor_id) {
		// echo $this->session->userdata['EM_COMPANY'];die;
		if (empty($vendor_id)) {
		...
		$data['account_group'] = $this->m_vnd_account_group->get(array("COMPANY_ID" => intval($this->session->userdata['EM_COMPANY'])));
...
//-- Approve update profile vendor oleh kabiro --//
	function accept_vendor_update() {
		...
		$data_vendor['BANKS'] = 'ID';
		$data_vendor['BANKL'] = 'HSBC12920C06'; //$this->input->post('bank_key');
		$data_vendor['ZTERM'] = 'ZG30';
		// CREATE VENDOR NO
		$vendors = $this->vnd_header->get_all(array("VENDOR_ID" => intval($vendor_id)));
		foreach ((array)$vendors as $key => $vendor) {
			$vendor_type = $vendor["VENDOR_TYPE"];
			$data_vendor['BUKRS'] = $vendor["COMPANYID"];
			$data_vendor['STRAS'] = $vendor["ADDRESS_STREET"];
			...
			$npwp_number = preg_split('/[-.;]+/', $tampung_npwp);
			$data_vendor['STCD1'] = implode('', $npwp_number);
			$prefix = $this->m_vnd_prefix->get_all(array("PREFIX_ID"=>$vendor["PREFIX"]));
			$data_vendor['NAME1'] = $vendor["VENDOR_NAME"].".".$prefix[0]["PREFIX_NAME"];
			if ($vendor["PREFIX"] == 1 || $vendor["PREFIX"] == 2 || $vendor["PREFIX"] == 0) {
				$n_prefix = 'Company';
			} else {
				$n_prefix = $prefix[0]["PREFIX_NAME"];
			}
			$data_vendor['ANRED'] = $n_prefix;
			
			$district = $this->adm_district->get(array("COMPANY_ID"=>$vendor["COMPANYID"]));
			$data_vendor['EKORG'] = $district[0]['DISTRICT_CODE'];
		...
		$vendor_bank = $this->vnd_bank->get_all(array("VENDOR_ID" => intval($vendor_id)));
		foreach ((array)$vendor_bank as $key => $bank) {
			$bank['VENDOR_ID'] = $vnd_id;
			$data_vendor["BANKN"] = $bank["ACCOUNT_NO"];
			$data_vendor["KOINH"] = $bank["ACCOUNT_NAME"]; 
		...
		$company_address = $this->vnd_address->get_all(array("VENDOR_ID" => intval($vendor_id)));
		foreach ((array)$company_address as $key => $address) {
			$data_vendor['TELF1'] = $address["TELEPHONE1_NO"];
			$data_vendor['TELFX'] = $address["FAX"]; //uat tonasa
		...
		$where_m_vnd_account_group = array('COMPANY_ID'=>$data_vendor['BUKRS'], 'VENDOR_TYPE'=>$vendor_type);
		$accgroup = $this->m_vnd_account_group->get($where_m_vnd_account_group);
		$data_vendor['KTOKK'] = $accgroup[0]['GROUP'];
		// $data_vendor['KTOKK'] = $account_group;
		$reconc = $this->m_vnd_reconc_acc->get(array('VENDOR_TYPE'=>$vendor_type));
		$data_vendor['AKONT'] = $reconc[0]['GL_ACCOUNT'];
		...
		$this->load->library('sap_handler');
		$param_sap = $data_vendor;
		// $return_sap = $this->sap_handler->updateVendor($data_vendor);
		// echo "<pre>";
		// print_r($return_sap);
		// die;
		// update ke sap
