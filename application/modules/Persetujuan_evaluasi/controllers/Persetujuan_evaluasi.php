...
class Persetujuan_evaluasi extends CI_Controller {
	...
	public function get_next_holder($id) {
		...
		// if($level=='DEPT'){
		// 	$this->is_kadep = ($level == 'DEPT' || $level == 'REPT'); 
		// //nyo 29012018 PERMINTAAN PAK SAID 
		// } else if($level=='DIR') {
		// 	$this->is_kadep = ($level == 'DIR' || $level == 'REPT'); 
		// //nyo 22022017
		// }

		$this->is_kadep = ($level == 'DEPT' || $level == 'DIR' || $level == 'REPT'); //nyo 22022017
	...
	public function save_bidding() {
		...
		if($next_process==1 || $next_process=="1"){
			$status_apprv = 'OK';
		} else {
			$status_apprv = 'REJECT';
		}
		...
		// BARU
		$this->log_data->main($this->session->userdata['ID'],$this->session->userdata['FULLNAME'],
			$this->authorization->getCurrentRole(),'Approval Evatek',$status_apprv,$this->input->ip_address()
		);
		...
		if($next_process==1 || $next_process=="1"){
			...
			$this->app_process->where_unique($ptm[0]['KEL_PLANT_PRO'], $ptm[0]['SAMPUL'], $ptm[0]['JUSTIFICATION']);
			$ambil_status_evatek = $this->app_process->get(array('NAMA_BARU' => 'Evatek'));
			$this->load->model('prc_evaluator');
			$ambil_evaluator = $this->prc_evaluator->ptm_desc($id);

			$this->prc_tender_main->updateByPtm($id, array('PTM_STATUS' => $ambil_status_evatek[0]['CURRENT_PROCESS']));
			$this->prc_process_holder->update(array('EMP_ID' => $ambil_evaluator[0]['EMP_ID']), array('PTM_NUMBER' => $id));

			// echo "<pre>";
			// print_r($ambil_status_evatek);
			// print_r($ambil_evaluator);
			// die;
			// reject evatek
		}
		...
		if ($doc != false) {
			foreach ((array) $doc as $key => $value) {
				$name =  $key;
				$this->prc_add_item_evaluasi->update(array("IS_SHARE" => $value), array('ADD_ID' => $name));
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Persetujuan_evaluasi/save_bidding','prc_add_item_evaluasi','update',array("IS_SHARE" => $value), array('ADD_ID' => $name));
				//--END LOG DETAIL--//
			}
		}
