...
class EC_Procurement_pratender extends CI_Controller {
	...
	public function get_detail($id = null,$cegatan = false) {
		...
		// die;
		if ($id != null) {
			redirect('EC_Procurement_pratender/attempt/' . $id);
		}
		...
		if ($this->form_validation->run() == FALSE && $postvendor[0]=="" && $vndT[0]=="") {
			...
			// die;
			$data['evatek'] = $this->get_all_template_view($this->session->userdata['EM_COMPANY']);

			$this->layout->set_table_js();
			$this->layout->set_table_cs();
			$this->layout->set_datetimepicker();
			$this->layout->add_js('pages/mydatetimepicker.js');
			$this->layout->add_js('pages/EC_procurement_pratender.js?3');
	...
	public function update() {
		...
		for ($i=0; $i < count($_POST['item']); $i++) {
			...
			if ($qty > $ppi['PPI_QUANTOPEN'] - $ppi['PPI_QTY_USED']) {
				redirect('EC_Procurement_pratender/get_detail');
			}
		}
		...
		if ($subpratender[0]['TYPE'] == 'E') {
			...
			if (!isset($subpratender[0]['MESSAGE']) || $subpratender[0]['MESSAGE'] == '') {
				$this->session->set_flashdata('gagal_subpratender', $subpratender[0]);
				$this->session->set_flashdata('detail_gagal_subpratender', $subpratender);
				redirect('EC_Procurement_pratender/attempt/' . $ptm_number);
			}
			...
			$LM_ID = $this->log_data->last_id();
					//--END LOG MAIN--//
					//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_main','insert',$ptm);
					//--END LOG DETAIL--//

			$new_update['PTM_SUBPRATENDER'] = $subpratender[0]['MESSAGE'];
			$this->prc_tender_main->updateByPtm($ptm_number, $new_update);
					//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_main','update',$new_update,array('PTM_NUMBER'=>$ptm_number));
					//--END LOG DETAIL--//
			...
			for ($i=0; $i < count($_POST['item']); $i++) {
				...
				$pritemset = $pritemset[0];
					...
					// 		//--LOG DETAIL--//
					// 	$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_pr_item','update',array('PPI_QTY_USED' => $used), array('PPI_ID' => $ppi_id));
					// 		//--END LOG DETAIL--//
					// 	//*/
			...
			$this->prc_tender_item->insert_batch($newTits);
					//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_item','insert',$newTits);
					//--END LOG DETAIL--//
			...
			$this->prc_tender_approve->insert($tap);
					//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_approve','insert',$tap);
					//--END LOG DETAIL--//
			...
			foreach ($postvendor as $key) {
				if($key!=""){
					...
					$this->prc_tender_vendor->insert_array($data);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_vendor','insert',$data);
						//--END LOG DETAIL--//

					$tapv['PTV_VENDOR_CODE'] = $key;
					$this->prc_tender_approve_vendor->insert($tapv);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_approve_vendor','insert',$tapv);
						//--END LOG DETAIL--//
					$tapv['TAPV_ID']++;
				}
			}
			...
			if ($this->input->post('coba_vnd_terpilih_tambahan') != false) {		
				...
				foreach ($postVendorTmbn as $key) {
					foreach ($this->input->post('type_tender') as $type) {
						$ven = explode(":", $type);
						if($ven[0]==$key){
							$dataT['PTV_TENDER_TYPE'] = $ven[1] ;
							break;
						}
					}
					foreach ($this->input->post('list_principal') as $pricpl) {
						if($pricpl!=0){
							$prc = explode(":", $pricpl);
						    if($prc[0]==$key){
						    	$dataT['PTV_PC_CODE'] = $prc[1] ;
						    	break;
						    }
						}else{
							$dataT['PTV_PC_CODE'] = 0;
					...
					if($dataT['PTV_TENDER_TYPE']==2){
						$dataT['PTV_APPROVAL'] = '0';
					}else{
						$dataT['PTV_APPROVAL'] = '';
					}
					...
					$dataT['PTV_VENDOR_CODE'] = $key;
						...
						$this->prc_tender_vendor->insert_array($dataT);
							//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_vendor','insert',$dataT);
							//--END LOG DETAIL--//
						...
						$this->prc_tender_approve_vendor->insert($tapvT);
							//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_approve_vendor','insert',$tapvT);
							//--END LOG DETAIL--//
					}
				}
				//*/
				...
				$ptp['PTP_EVALUASI_TEKNIS'] = $this->input->post('ptp_template_evaluasi');
				...
				$this->prc_tender_prep->insert_single($ptp);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_prep','insert',$ptp);
					//--END LOG DETAIL--//
				...
				$this->comment->insert_comment_tender($dataComment);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/update','prc_tender_comment','insert',$dataComment);
					//--END LOG DETAIL--//
		...
		public function approval($id) {
			...
			{
				$data['title'] = "Approval Perencanaan Baru";
				$this->layout->set_table_js();
				$this->layout->set_table_cs();
				$this->layout->set_datetimepicker();
				$this->load->model('prc_doc_type_master');
			...
			// $data['next_process']['NAMA_BARU'] = 'Pilih Vendor';
				...
				foreach ($data['vendor_tambahan'] as $key => $value) {
					$listprincipal = $this->user_principal_model->getPrincipalList($value['PTV_VENDOR_CODE']);
					$data['vendor_tambahan'][$key]['list_principal'] = $listprincipal;
				}
				$data['evatek'] = $this->get_all_template_view($this->session->userdata['EM_COMPANY']);
				...
				$this->layout->set_datetimepicker();
				$this->layout->add_js('pages/mydatetimepicker.js');
				$this->layout->add_js('pages/EC_procurement_pengadaan.js');
				// $this->layout->add_js('pages/EC_procurement_pratender.js?3');
				$this->layout->add_js('plugins/numeral/numeral.js');
				$this->layout->add_js('plugins/numeral/languages/id.js');
				$this->layout->add_css('plugins/select2/select2.css');
		...
		public function submit_approval($id) {
			...
			if($submit == "accept"){
				...
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_prep','update',$ptp,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
				...
				$this->prc_tender_approve->insert($tap);
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_approve','insert',$tap);
				//--END LOG DETAIL--//
				...
				$this->prc_tender_vendor->delete($data);
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_vendor','delete',null,$data);
				//--END LOG DETAIL--//
				$tapv['TAP_ID'] = $tap_id;
				$tapv['TAPV_ID'] = $this->prc_tender_approve_vendor->get_id();
				...
				foreach ($postvendor as $key) {
					if($key!=""){
						...
						$this->prc_tender_vendor->insert_array($data);
						//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_vendor','insert',$data);
						//--END LOG DETAIL--//

						$tapv['PTV_VENDOR_CODE'] = $key;
						$this->prc_tender_approve_vendor->insert($tapv);
						//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_approve_vendor','insert',$tapv);
						//--END LOG DETAIL--//
						$tapv['TAPV_ID']++;
					}
				}
				...
				if ($this->input->post('vendor_tambahan') != false) {		
					...
					foreach ($postVendorTmbn as $key) {
						...
						$this->prc_tender_vendor->insert_array($dataT);
						//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_vendor','insert',$dataT);
						//--END LOG DETAIL--//
						...
						$this->prc_tender_approve_vendor->insert($tapvT);
						//--LOG DETAIL--//
						$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_approve_vendor','insert',$tapvT);
						//--END LOG DETAIL--//
					}
				}
				//*/
			...
			// insert and upload comment attachment
				...
				$this->comment->insert_comment_tender($dataComment);
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_comment','insert',$dataComment);
				//--END LOG DETAIL--//
				...
				foreach ($item as $key => $value) {
					...
					//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_pr_item','update',array('PPI_QTY_USED' => $pr_item[0]['PPI_QTY_USED'] - $value['TIT_QUANTITY']),array('PPI_ID'=>$pr_item[0]['PPI_ID']));
					//--END LOG DETAIL--//
				}
				...
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/submit_approval','prc_tender_main','update',array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ),array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
		...
		public function do_attempt($id) {
			...
			// insert and upload comment attachment
				...
				foreach ($item as $key => $value) {
					...
					$this->prc_pr_item->update(array('PPI_QTY_USED' => $pr_item[0]['PPI_QTY_USED'] - $value['TIT_QUANTITY']), array('PPI_ID'=>$pr_item[0]['PPI_ID']));
					//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pratender/do_attempt','prc_pr_item','update',array('PPI_QTY_USED' => $pr_item[0]['PPI_QTY_USED'] - $value['TIT_QUANTITY']),array('PPI_ID'=>$pr_item[0]['PPI_ID']));
					//--END LOG DETAIL--//
				}
				...
				$this->prc_tender_main->updateByPtm($id, array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ));
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/do_attempt','prc_tender_main','update',array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ),array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			// insert comment
				...
				$this->comment->insert_comment_tender($dataComment);
				//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pratender/do_attempt','prc_tender_comment','insert',$dataComment);
				//--END LOG DETAIL--//
	...
	public function get_po() {
		...
		$ans = $this->load->view('EC_Procurement_pratender/detail_po', $data, true);
	...
	public function get_po_from_ptm($id) {
		...
		$result['detail'] = $this->load->view('EC_Procurement_pratender/approval_detail_po', $data, true);

		$result['vnd_terpilih'] = $this->load->view('EC_Procurement_pratender/approval_vendor_terpilih', $data, true);
	...
	public function search_dokumenPR(){
		...
		echo $this->load->view('EC_Procurement_pratender/dokumen_pr', $data, true);
	}
	...
	public function get_all_template_view($opco) {
		$this->load->model('prc_evaluation_template');
		if($opco != '99'){
