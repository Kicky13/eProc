...
class EC_Proc_verify_entry extends CI_Controller {
	...
	public function index($id) {
		...
		$data['title'] = 'Verifikasi Penawaran Vendor Baru';
		...
		if(isset($data['pqi'])){
			...
			foreach ($data['pqi'] as $key => $value) {
				...
				foreach ($value as $k => $val) {
					...
					$data['evatek'][$val['PTV_VENDOR_CODE']] = $this->prc_tender_quo_eval->ptqe($val['PQM_ID']);
				}
				$arr[$key] = $arr2;
			}
			$data['pqi'] = $arr;
		...
		$this->layout->add_js('pages/EC_verify_entry.js');
	...
	public function approval($ptm, $ptv, $approve) {
		...
		redirect('EC_Proc_verify_entry/index/'.$ptm);
	}
	...
	public function saveBatal(){
		...
		if (!isset($tit_id) || empty($tit_id)) { //redirect lg ke batal
			...
			redirect('EC_Proc_verify_entry/batal/'.$noptm.'/'.$proses.'/'.$this->input->post('kembali').'/'.$this->input->post('index'));
		}
		...
		foreach ($tit_id as $val) {
			$this->prc_tender_item->update(array('TIT_STATUS'=>999),array('PRC_TENDER_ITEM.TIT_ID'=>$val)); //set status item jadi (999)'dibatalkan'
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/saveBatal','prc_tender_item','update',array('TIT_STATUS'=>999),array('PRC_TENDER_ITEM.TIT_ID'=>$val, 'PTM_NUMBER'=>$noptm));
				//--END LOG DETAIL--//
		}
		...
		if(count($pti)==0){
			$this->prc_tender_main->updateByPtm($noptm,array('PTM_STATUS'=>-999)); //set status ptm jadi (-999)'dibatalkan'
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/saveBatal','prc_tender_main','update',array('PTM_STATUS'=>-999),array('PTM_NUMBER'=>$noptm));
				//--END LOG DETAIL--//
		}
		...
		$this->comment->insert_comment_tender($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/saveBatal','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
	...
	public function save_bidding() {
		...
		if ($next_process == '999') { //BATAL
			redirect('EC_Proc_verify_entry/batal/' . $id .'/'.$this->input->post('process_name').'/EC_Proc_verify_entry/index');
		}
		...
		if(!$lanjut){
			...
			redirect('EC_Proc_verify_entry/index/' . $id);
		}
		...
		$this->prc_tender_main->updateByPtm($id, array('IS_VENDOR_CLOSED' => 1));
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_main','update',array('IS_VENDOR_CLOSED' => 1),array('PTM_NUMBER'=>$id));
			//--END LOG DETAIL--//
		...
		if($ptp['PTP_JUSTIFICATION_ORI']==5){//untuk RO
			$this->prc_tender_main->updateByPtm($id, array('IS_EVALUATED' => 1));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_main','update',array('IS_EVALUATED' => 1),array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			foreach ($ptqi as $v) {
				...
				$this->prc_tender_quo_item->update($set, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_quo_item','update',$set,$where);
					//--END LOG DETAIL--//
			}
		}
		...
		foreach ($vendors as $vnd) {
			...
			if ($vnd['PTV_STATUS'] != 2) {
				...
				if (!in_array($vendor['PTV_VENDOR_CODE'], $this->input->post('check'))) {
					...
					$this->prc_tender_vendor->update($set, $vendor);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_vendor','update',$set,$vendor);
						//--END LOG DETAIL--//
					$set['PQM_STOP_STATUS'] = 1;
					unset($set['PTV_STATUS']);
					$this->prc_tender_quo_main->update($set, $vendor);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_quo_main','update',$set,$vendor);
						//--END LOG DETAIL--//
		...
		$this->comment->insert_comment_tender($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_bidding','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//

		$this->penilaian_otomatis->insert('EC_proc_verify_entry', $vendors, $ptm['PTM_PRATENDER'], null, $LM_ID);
		//die('stop');
	...
	public function save_harga() {
		...
		$this->comment->insert_comment_tender($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_verify_entry/save_harga','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
