...
//    var_dump($data['detailRR']);die();
    foreach ($data['detailRR'] as $key => $value) {
        ...
        if($value['KETERLAMBATAN']['denda']>0 && $value['JENISPO']!=='BAHAN'){            
//         $denda='Denda Keterlambatan : '.$value['KETERLAMBATAN']['selisih'].' Hari<br/>Rp. '            
//            .ribuan($value['KETERLAMBATAN']['denda']);        
            $denda=$value['KETERLAMBATAN']['denda'];        
        }else{
            $denda='';
        }        
        $lokasi_Gudang=$value['LGORT'];

        $tr = '<tr>
        <td style="vertical-align: top; padding-top:18px; height:'.$tinggi.'" align="center" rowspan="'.$t_jum.'">'.$no++.'</td>
        <td style="height:'.$tinggi.'" align="center">'.$value['MATERIAL_NO'].'<br/>'.$value['LGPLA'].'</td>
        <td style="height:'.$tinggi.'" align="center">'.$value['GR_ITEM_UNIT'].'</td>
        <td style="height:'.$tinggi.'" align="center">'.$value['GR_ITEM_QTY'].'<br/>'.$lokasi_Gudang.'</td>
        <td style="height:'.$tinggi.'" align="center">'.$desc.'<br/>'.$value['LONG_TEXT_MAT'].'<br/>'.$denda.            
        '</td>
    ...
    if(!empty($totalDenda)){ 
        $hari=$data['detailRR'][0]['KETERLAMBATAN']['selisih'];
        $tr='<tr>'
        . '<td style="height:'.$tinggi.'" colspan="7" align="center">Denda Keterlambatan: '.$hari.' Hari (Rp '.ribuan($totalDenda)
        . ')</tr>';
        array_push($detailRR,$tr);
    }
}
...
<body>
                    ...
                    <div>
                        <table class="noborder">
                            ...
                            <tr>
                                <td>UK PEMINTA</td>
                                <td> : </td>
                                <td><?php echo $ambil_uk_peminta[0]['AFNAM'] ?></td>
                            </tr>
                            <tr>
                                <td></td>
                                <td></td>
                                <td><?php echo $ambil_uk_peminta[0]['NAME_TEXT']; ?></td>
                            </tr>
                        </table>
                    </div>
                </td>
    ...
    <?php if($print_type == 'LOT'){
        ...
        <td style="height:'.$tinggi.'" align="center">'.ribuan($totalQty).' '.$data['detailRR'][0]['GR_ITEM_UNIT'].'</td>
        </tr>
        </table>'
        ;
    }
