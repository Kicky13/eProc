...
class EC_Auction_itemize extends CI_Controller {
	...
	function index($status = null) {
		...
		// echo "<pre>";
		// print_r($emp);die;
		$data['aunction'] = $this -> ec_list_auction_itemize -> get_list_auction_user($emp);
		// echo "<pre>";
	...
	function history($status = null) {
		$this -> load -> model('ec_list_auction_itemize');
		$emp = $this -> session -> userdata['ID'];
		// echo "<pre>";
		// print_r($emp);die;
		$data['aunction'] = $this -> ec_list_auction_itemize -> get_list_auction_history($emp);
		// echo "<pre>";
		// print_r($data['aunction']);die;
		// echo "<pre>";
		// print_r($emp);die;
		$data['title'] = 'History Auction';
		$data['status'] = $status;
	...
	function show_detail($notender, $nobatch) {
		...
		$data['check'] = $this -> ec_auction_itemize_m -> getBatchDetailClose($notender, $nobatch);
	...
	public function getListpeserta($NO_TENDER, $NO_BATCH) {
		$this -> load -> model('ec_auction_itemize_m');

		$detil_auction = $this -> ec_auction_itemize_m -> get_Auction($NO_TENDER, $NO_BATCH);
		$cek_close = $detil_auction['IS_ACTIVE'];
    	// echo "<pre>";
    	// print_r($detil_auction);die;
		// $dataitem = $this -> ec_auction_itemize_m -> get_Itemlist($NO_TENDER);
		if ($cek_close != 0){
			$ketPeserta = $this -> ec_auction_itemize_m -> getAllPeserta($NO_TENDER);
		} else {
			$ketPeserta = $this -> ec_auction_itemize_m -> getAllPesertaHist($NO_TENDER);
		}
	...
	public function print_e_auction2($notender,$nobatch) {
		...
		$data['nobatch'] = $nobatch;
		$data['Detail_Auction'] = $this -> ec_auction_itemize_m -> get_Auction($notender,$nobatch);
		$cek_close = $data['Detail_Auction']['IS_ACTIVE'];

		$data['Itemlist'] = $this -> ec_auction_itemize_m -> get_Itemlist($notender);
		$data['det_batch'] = $this -> ec_auction_itemize_m -> getBatchDetail($notender,$nobatch);
		if ($cek_close != 0){
			$data['Peserta'] = $this -> ec_auction_itemize_m -> getAllPesertaBatch($notender, $data['det_batch'][0]['ITEM']);
		}else {
			$data['Peserta'] = $this -> ec_auction_itemize_m -> getAllPesertaBatchHist($notender, $data['det_batch'][0]['ITEM']);
		}
		// $data['Peserta'] = $this -> ec_auction_itemize_m -> getAllPeserta($notender);
		$data['undurdiri'] = $this -> ec_auction_itemize_m -> get_list_Peserta_Undur_Diri($notender);
		// echo "<pre>";
		// print_r($data['det_batch']);die;
		...
		<table>
			...
			<table width="100%" border="" class="table table-hover" cellspacing="0">
				<thead>
					<tr>
						<th rowspan="2" class="col-md-1 Kiri Bawah Atas Kanan" width="5px">No</th>
		...
		//resultolehranking
			...
			<table class="table table-hover" width="100%" cellspacing="0" border="1">
				<thead>
					<tr>
						<th rowspan="2" class="col-md-1 Kiri Bawah Atas Kanan" width="5px">No</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Nomor<br>Item</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Merk</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan" width="50px">Delivery Time</th>
				...
				<tbody>
					...
					foreach ($dataItem as $value){ 
						...
						if ($cek_close != 0){
							...
							$urutkanRanking = $this -> ec_auction_itemize_m -> urutkanRankingHist($notender, $value['ID_ITEM']);
						}
							// $urutkanRanking = $this -> ec_auction_itemize_m -> urutkanRanking($notender, $value['ID_ITEM']);
						// echo "<pre>";
					...
					if($harga!=0 or $harga!="0"){
						$tampil2.='
						<tr>
							<td class="Kiri Bawah Atas Kanan" align="center"> '.++$i.'</td>
							<td class="Kiri Bawah Atas Kanan"> '.$value['KODE_BARANG'].'</td>
							<td class="Kiri Bawah Atas Kanan"> '.$merk.'</td>
							<td class="Kiri Bawah Atas Kanan" align="center"> '.$delivery.'</td>
							<td class="Kiri Bawah Atas Kanan" align="center"> '.$value['UNIT'].'</td>
							<td class="Kiri Bawah Atas Kanan" align="left"> '.$CURRENCY.' '.$harga.'</td>
							'.$initialR.'
						</tr>
						';	
						// $harga = 0;	
		...
		foreach ($dataPeserta as $dp) {
			...
			if ($cek_close != 0){
				...
				$ambilData1 = $this -> ec_auction_itemize_m -> getItemPesertaHist($this -> session -> userdata['ID'],$dp['KODE_VENDOR'],$notender, $ketBatch[0]['ITEM']);
			}
			// $ambilData1 = $this -> ec_auction_itemize_m -> getItemPeserta($this -> session -> userdata['ID'],$dp['KODE_VENDOR'],$notender, $ketBatch[0]['ITEM']);
			// echo "<pre>";
			...
			foreach ($ambilData1 as $ad1) {
				...
				<tr>
					<td class="Kiri Bawah Atas Kanan" align="center">'.$no.'</td>
					<td class="Kiri Bawah Atas Kanan">'.$ad1['KODE_BARANG'].'</td>
					<td class="Kiri Bawah Atas Kanan">'.$ad1['NAMA_BARANG'].'</td>
					<td class="Kiri Bawah Atas Kanan" align="left">'.$ad1['MERK'].'</td>
					<td class="Kiri Bawah Atas Kanan" align="center">'.$ad1['DELIVERY_TIME'].'</td>
					<td class="Kiri Bawah Atas Kanan" align="left">'.$ad1['CURRENCY'].' '.number_format($ad1['HARGATERKINI'],0,".",".").'</td>
			...
			<table cellspacing="0" width="100%">
				<tr>
					<th class="Kiri Bawah Atas Kanan" width="5px">No</th>
					<th class="Kiri Bawah Atas Kanan">Nomor Item</th>
					<th class="Kiri Bawah Atas Kanan">Deskripsi Item</th>
					<th class="Kiri Bawah Atas Kanan">Merk</th>
					<th class="Kiri Bawah Atas Kanan">Delivery Time</th>
					<th class="Kiri Bawah Atas Kanan" width="100px">Harga</th>
	...
	public function print_auction_report($NO_TENDER, $nobatch, $iditem) {

		$this -> load -> model('ec_auction_itemize_m');
		
		$ketBatch = $this -> ec_auction_itemize_m -> getBatchDetail($NO_TENDER, $nobatch);
		...
		$cek_close = $data['curr']['IS_ACTIVE'];
		// $data['Peserta'] = $this -> ec_auction_itemize_m -> get_list_Peserta($NO_TENDER);
		// $data['dataItem'] = $dataItem;
		// $data['urutkanRanking'] = $this -> ec_auction_itemize_m -> urutkanRanking($NO_TENDER, $value['ID_ITEM']);
		...
		<body>
			<table style="width:100%;" >
				<tr>
					<th align="center"><h2>Report Auction</h2></th>
				</tr>
				<tr>
					<td style="padding-top:-20px"><hr></hr></td>
				</tr>
			</table>
			<table width="100%" class="table border="0" cellspacing="0" table-hover">
				<thead>
					<tr>
						<th width="2%" rowspan="2" class="col-md-1 Kiri Bawah Atas Kanan">No</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Kode Item</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Deskripsi</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Merk</th>
						<th width="15%" rowspan="2" class="Kiri Bawah Atas Kanan">HPS</th>
						<th width="15%" rowspan="2" class="Kiri Bawah Atas Kanan">Harga Ranking 1</th>
						<th rowspan="2" class="Kiri Bawah Atas Kanan">Prosentase</th>
						<th colspan="'.count($dataPeserta).'" class="Kiri Bawah Atas Kanan"><center>Ranking</center></th>
					</tr>
					<tr>
						'.$tampilRanking1.'
					</tr>
				...
				<tbody>
					...
					foreach ($dataItem as $value){
						...
						if ($cek_close != 0){
							...
							$urutkanRanking = $this -> ec_auction_itemize_m -> urutkanRankingHist($NO_TENDER, $value['ID_ITEM']);
						}
    				// echo "<pre>";
    				// print_r($urutkanRanking);
    				// die;
    					...
    					$keterangan .='
    					<td>'.$UR['INITIAL'].' = '.$UR['NAMA_VENDOR'].'</td>
    				...
    				$tampil.='
    				<tr>
    					<td class="Kiri Bawah Atas Kanan"> '.++$i.'</td>
    					<td class="Kiri Bawah Atas Kanan"> '.$value['KODE_BARANG'].'</td>
    					<td class="Kiri Bawah Atas Kanan"> '.$value['NAMA_BARANG'].'</td>
    					<td class="Kiri Bawah Atas Kanan"> '.$MERK.'</td>
    					<td class="Kiri Bawah Atas Kanan"> '.$value['CURR']." ".number_format($value['HPS'],0,".",".").'</td>
    					<td class="Kiri Bawah Atas Kanan"> '.$harga.'</td>
    					<td class="Kiri Bawah Atas Kanan" align="center"> '.$label.'</td>
    					'.$initialR.'
    				</tr>
    				';	
    				$harga = 0;											
    ...
    ';
    	// echo $persen_sukses." - ".$persen_warning;
    	// echo $tampil;exit;
    	// echo $persen_sukses." - ".$persen_warning; 
		// echo $tampil;exit;
		// print_r($dataPeserta);die;
...
public function importExcelEdit($notender)
{
	...
	if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
		...
		$tmp    = './upload/temp/'.$_FILES['import_excel']['name'];
            ...
            $_sheet = $excel->setActiveSheetIndexByName('Sheet1');
    ...
    public function hasilawal()
    {
    	...
    	// print_r($NO_TENDER);
    	// die;	
    	if ($NO_TENDER==null or $NO_TENDER==""){
    		$dataItem = $this -> ec_auction_itemize_m -> get_items_temp($iduser);
    		$dataPeserta = $this -> ec_auction_itemize_m -> get_list_Peserta_temp($iduser);
    	} else {
    		$dataItem = $this -> ec_auction_itemize_m -> get_items($NO_TENDER);
    		$dataPeserta = $this -> ec_auction_itemize_m -> get_list_Peserta($NO_TENDER);
    	}
    	// echo "<pre>";
    	// print_r($dataItem);die;
    	...
    	<table class="table table-hover">
    		<thead>
    			<tr>
    				...
    				<th rowspan="2">HPS</th>
    				<th colspan="'.count($dataPeserta).'"><center>Vendor</center></th>
    			</tr>
    			<tr>
    				'.$tampilRanking1.'
    			</tr>
    		...
    		<tbody>
    			...
    			foreach ($dataItem as $value){
    				...
    				if ($NO_TENDER==null or $NO_TENDER==""){
    					$urutkanRanking = $this -> ec_auction_itemize_m -> urutkanRankingtmp($iduser, $value['ID_ITEM']);
    				}else {
    					$urutkanRanking = $this -> ec_auction_itemize_m -> urutkanRanking1($NO_TENDER, $value['ID_ITEM']);
    				}
    				// echo "<pre>";
    				// print_r($urutkanRanking);
    				// die;
    ...
    public function resultOlehRanking()
    {
    		...
    		<tbody>
    			...
    			foreach ($dataItem as $value){
    				...
    				// $tooltip = "";
    					...
    					$ambilQty = $this -> ec_auction_itemize_m -> get_list_PesertaHist($NO_TENDER, $value['ID_ITEM']);
    					$initialR = "";
    				}
    				...
    				<tr>
    					<td> '.++$i.'</td>
    					<td> '.$value['KODE_BARANG'].$tombol.'</td>
    					<td> '.$MERK.'</td>
    					<td> '.$value['CURR']." ".number_format($value['HPS'],0,".",".").'</td>
    					<td> '.$harga.'</td>
    					<td> '.$label.'</td>
    	...
    	';
    	// echo $persen_sukses." - ".$persen_warning;
    	$persent_sukses_total = '<span style="font-size: 30px;color: red;"><strong>'.number_format(intval($persen_sukses)/intval($persen_sukses+$persen_warning)*100,1,",",".").'%</strong></span>';
    	$persent_warning_total = '<span style="font-size: 30px;color: green;"><strong>'.number_format(intval($persen_warning)/intval($persen_sukses+$persen_warning)*100,1,",",".").'%</strong></span>';
    	echo "<center>".$persent_warning_total." &nbsp&nbsp&nbsp ".$persent_sukses_total."</center>"; 
    	echo $tampil;
    }
    			...
    			foreach ($dataPeserta as $value){
    				...
    				for ($i=0; $i < count($dataPeserta); $i++) {
    					...
    					if ($cek_close != 0){
    						...
    						$ambilRank = $this -> ec_auction_itemize_m -> getReportItemRankingHist($this -> session -> userdata['ID'], $value['KODE_VENDOR'], $NO_TENDER, ($i+1), $NO_BATCH, $ketBatch[0]['ITEM']);    					
    					} 
    ...
    public function exportReport($NO_TENDER,$nobatch){ 
        //membuat objek
    	$this->load->library('excel');//Load library excelnya
    	$this -> load -> model('ec_auction_itemize_m');
    	// $iduser = $this -> session -> userdata['ID'];
    	...
    	$dataItem = $this -> ec_auction_itemize_m -> getBatchDetailItem($NO_TENDER, $ketBatch[0]['ITEM']);
    	$dataPeserta = $this -> ec_auction_itemize_m -> get_list_Peserta($NO_TENDER);
    	$auction = $this -> ec_auction_itemize_m -> get_Auction($NO_TENDER, $nobatch);
    	$cek_close = $auction['IS_ACTIVE'];
    	// echo "<pre>";
    	// print_r($dataPeserta);
    	// die;
    	...
    	$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	...
    	$objPHPExcel->getActiveSheet()->getStyle('D1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	$objPHPExcel->getActiveSheet()->mergeCells('E1:E2');
    	$objPHPExcel->getActiveSheet()->getStyle('E1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	$objPHPExcel->getActiveSheet()->mergeCells('F1:F2');
    	$objPHPExcel->getActiveSheet()->getStyle('F1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	$objPHPExcel->getActiveSheet()->mergeCells('G1:G2');
    	$objPHPExcel->getActiveSheet()->getStyle('G1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	
    	$colNumber = PHPExcel_Cell::columnIndexFromString('H'); //7
    	$colmerge = $colNumber + count($dataPeserta) - 2; //14-2 = 12
    	$colString = PHPExcel_Cell::stringFromColumnIndex($colmerge); //12 -> M
    	//echo $colString;die;
    	$objPHPExcel->getActiveSheet()->mergeCells('H1:'.$colString.'1');
    	$objPHPExcel->getActiveSheet()->getStyle('H1')->getAlignment()->applyFromArray(
    		array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    		);
    	$row = 3;
    	foreach ($dataItem as $value) {
    		...
    		$objPHPExcel->getActiveSheet()->getStyle('A1:'.$colString.$row)->applyFromArray($styleArray);
    		$row++;
    	}
    	$rank = 7;
    	for ($i=0; $i < count($dataPeserta); $i++) {
    		$colString = PHPExcel_Cell::stringFromColumnIndex($rank); //H
    		$rank++;
    		
    		$objPHPExcel->getActiveSheet()->getStyle($colString.'2')->getAlignment()->applyFromArray(
    			array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    			);
    	}
    	// echo "<pre>";
    	...
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, 1, "Kode Item");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, 1, "Deskripsi");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, 1, "Merk");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, 1, "HPS");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, 1, "Harga Ranking 1");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "Prosentase");
    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, 1, "Ranking");
    	$rank = 7;
    	for ($i=0; $i < count($dataPeserta); $i++) { 
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($rank, 2, $i+1);
    		$rank++;
    	}
    	...
    	foreach ($dataItem as $value){
    		...
    		foreach ($urutkanRanking as $UR) {
    			...
    			if($UR['HARGA']){ //jika tidak lolos
    				$initialR = $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($rank, $row, $UR['INITIAL']);	
    					// echo "<pre>";
    				// print_r($initialR);
    				// die;
    			} else {
    				$initialR = $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($rank, $row, '');
    			}
    			$colString = PHPExcel_Cell::stringFromColumnIndex($rank);
    			$objPHPExcel->getActiveSheet()->getStyle($colString.$row)->getAlignment()->applyFromArray(
    				array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
    				);
    			$rank++;
    		}
    		if($persen>100){
    			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('ff0000');
    			$label = number_format($persen,1,",",".").'%';
    			$persen_sukses = $persen_sukses+1;
    		} else {
    			$objPHPExcel->getActiveSheet()->getStyle('G'.$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00ff04');
    			$label = number_format($persen,1,",",".").'%';
    			$persen_warning = $persen_warning+1;
    		}

    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, ($i));
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $value['KODE_BARANG']);
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $value['NAMA_BARANG']);
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $MERK);
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $value['CURR']." ".number_format($value['HPS'],0,".","."));
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $harga);
    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $label);
    		$initialR;
    		$i++;
    		$row++;
    	}
	  ...
	  header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        //Nama File
	  header('Content-Disposition: attachment;filename="Laporan_Auction.xlsx"');
