...
class Tender_winner_tonasa extends CI_Controller {
	...
	*/
	...
	public function printPO($id)
	{
		...
		// $this->mpdf = new mPDF('utf-8', array(213,261));
		$this->mpdf = new mPDF();
		$this->mpdf->WriteHTML($html);
		$this->mpdf->setTitle($data['title']);
		$this->mpdf->Output($data['title'].".pdf",'I');    
	}
	...
	public function cetak_evatek($data){
		...
		$this->mpdf = new mPDF('utf-8', array(180,297));
		$html = $this->load->view('print_lp3_sg1', $data, true);
		$this->mpdf->AddPage('P',
			'', '', '', '',
	            10, // margin_left
	            10, // margin right
		...
		$html2 = $this->load->view('print_lp3_sg2', $data, true);
		$this->mpdf->AddPage('L',
			'', '', '', '',
	            10, // margin_left
	            10, // margin right
  ...
  //       pdf_create($html, $filename, $paper, $orientation,false);
	...
	public function submit() {	
		...
		if($po_id == null){
			...
			foreach ($winner as $key => $each_winner) {
				...
				if($satu == 0){
					...
					$this->po_header->insert($po);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Tender_winner/submit','po_header','insert',$po);
						//--END LOG DETAIL--//
				...
				$this->po_detail->insert($pod);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Tender_winner/submit','po_detail','insert',$pod);
					//--END LOG DETAIL--//
			...
			$this->log_data->detail($LM_ID,'Tender_winner/submit','po_header','update',$poUpdate,array('PO_ID' => $po_id));
				//--END LOG DETAIL--//
