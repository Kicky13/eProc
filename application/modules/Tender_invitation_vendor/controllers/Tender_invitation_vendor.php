...
class Tender_invitation_vendor extends CI_Controller {
	...
	function index() {
		...
		$this->layout->add_js('pages/tender_invitation_vendor.js');
	...
	function do_update_tender_invitation($ptv_id) {
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'Tender_invitation_vendor/do_update_tender_invitation','prc_tender_vendor','update',array('PTV_STATUS' => $status, 'ALASAN' => $alasan, 'PTM_NUMBER' => $ptm_number),array('PTV_ID' => $ptv_id));
			//--END LOG DETAIL--//
