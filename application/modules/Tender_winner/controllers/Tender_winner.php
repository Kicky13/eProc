...
class Tender_winner extends CI_Controller {
	...
	public function listPO(){
		...
		if($this->input->post()){
			...
			if(!empty($creator)){
				$this->po_header->where_creator($creator);
		...
		if($is_approve==1){//approve
			...
			if($max_approve == $real_stat){
				...
				if($sukses){
					...
					$this->load->model('adm_employee');
					$buyer_data = $this->adm_employee->ambilData($data_ptm[0]['PTM_ASSIGNMENT']);
					$data_email = array(
						'EMAIL_ADDRESS'=>$buyer_data[0]['EMAIL'],
						'data'=>array(
							'PTM_SUBJECT_OF_WORK'=>$data_ptm[0]['PTM_SUBJECT_OF_WORK'],
							'PPR_PRNO'=>$data_ppr['PPR_PRNO'],
							'vendorno'=>$data[0]['VND_CODE'],
				...
				$this->progress_kirim_email_po_approve($id);
			}
...
//tambah fungsi ambil semua data PO thithe 16 oktober 2017
	...
	public function editReleasePO($id){	
        ...
        $data['data_log'] = $this->po_header->get_log(array('PO_ID'=>$id));
        // echo "<pre>";
        // print_r($data['data_log']);die;
		//tambah fungsi javascript tanggal, save, ambil data PO thithe 12 oktober 2017
		// $this->layout->add_js('pages/prc_tender_winner_edit.js');
        $this->layout->add_js('jquery.maskedinput.js');
        $this->layout->add_css('plugins/bootstrap-datepicker/datepicker.css');
    ...
    public function saveReleasePO(){	
    	...
    	$DOC_PO_PERPANJANG 			= $_FILES['DOC_PO_PERPANJANG']['tmp_name'];
    	$newDOC_PO_PERPANJANG = "";
    	$tes 				= dirname(__FILE__);
    	$pet_pat 			= str_replace('application/modules/Tender_winner/controllers', '', $tes);
    	if (isset($_FILES) && !empty($_FILES['DOC_PO_PERPANJANG']['name'])) {
    		$type = explode('.', $_FILES['DOC_PO_PERPANJANG']['name']);
			// if(end($type)=="doc" || end($type)=="docx" || end($type)=="pdf"){
    		$this->_myFile = "DOC_PO_PERPANJANG". $PO_ID . date('YmdHms') . "." . end($type);
    		$this->_path = $pet_pat . 'upload/temp/' . $this->_myFile;
    		if (move_uploaded_file($DOC_PO_PERPANJANG, $this->_path)) {
    			$newDOC_PO_PERPANJANG = $this->_myFile;
    		}
			// }
    	}
    	// echo $newDOC_PO_PERPANJANG;die;
    	$where['PO_ID'] 	= $PO_ID;
    	$set['HEADER_TEXT'] 	= $this->input->post('HEADER_TEXT');
    	$set['DOC_PO_PERPANJANG'] 	= $newDOC_PO_PERPANJANG;
    	$this->po_header->update($set,$where);

    	$dataupdate['PO_ID'] 	= $PO_ID;
    	$dataupdate['DOC_DATE'] 	= $DOC_DATE[0];
    	$dataupdate['DDATE'] 	= $DDATE[0];
    	$this->po_header->updateHeaderPo($dataupdate);
    	...
    	foreach ($id_item as $key) {
    		...
    		$data1['PO_ID'] 	= $PO_ID;
    		$this->po_detail->updateDetailPo($data1);
    		$i++;
    	}
    	...
    	$id_baru = $this->po_header->get_id_log();
    	$log['ID'] 	= $id_baru;
    	$log['PO_ID'] 	= $PO_ID;
    	$log['PO_NUMBER'] 	= $PO_NUMBER;
    	$log['DOC_PO_LAMA'] 	= $newDOC_PO_PERPANJANG;
    	$log['UPDATED_BY'] 	= $this->session->userdata['ID'];
    	$log['UPDATED_BY_NAME'] 	= $this->session->userdata['FULLNAME'];
    	$log['UPDATED_AT'] 	= date('d-M-Y g.i.s A');
    	$this->po_header->insert_log($log);
    	$log['DOC_DATE'] 	= $DOC_DATE[0];
    	$log['DDATE'] 	= $DDATE[0];
    	$this->po_header->update_log_po($log);
		//log

    	$this->change_detail_po($PO_ID, $PO_NUMBER);

    	$this->session->set_flashdata('success', 'Create Update Po. '.$PO_NUMBER);
    	redirect('Tender_winner/editReleasePO/'.$PO_ID);
    }
	...
	public function printPO($id)
	{
		...
		$html = $this->load->view('ketentuan', $data, true);
		// $ket = $this->load->view('ketentuan');      
		...
		if($cekfile=="po_jasa_si"){
			...
			$pagecount = $this->m_pdf->pdf->SetSourceFile($target_file.'KETENTUAN.pdf');
	...
	public function detailPO($id,$is_approval = false,$is_print='false'){
		...
		// rekap nego kayak di halaman nego
		$this->prc_tender_item->join_nego_item();
		$where = array('PRC_TENDER_ITEM.PTM_NUMBER' => $data['po_header']['PTM_NUMBER']);		
		...
		foreach ((array)$ptqi as $val) {	
			if(isset($vendors[$val['PTV_VENDOR_CODE']])){
				...
				$harneg = $this->prc_nego_detail->get(array('VENDOR_NO'=>$val['PTV_VENDOR_CODE'],'TIT_ID'=>$val['TIT_ID']));
		...
		// echo "<pre>";
		// print_r($data);die;
		// end rekap nego kayak di halaman nego
  ...
  //       pdf_create($html, $filename, $paper, $orientation,false);
    ...
    public function change_delivery_date($po_id) {
    	$this->load->model('po_detail');
    	$this->po_detail->where_po($po_id);
    	$po = $this->po_detail->get();
    	// echo "<pre>";
    	...
    	foreach ($po as $val) {
    		$poitem['rfq_no'] = $val['RFQ_NO'];
    		$poitem['rfq_item'] = $val['EBELP'];
    		$poitem['delivery_date'] = date('d.m.Y', oraclestrtotime($val['DDATE']));
    		$return = $this->sap_handler->changeDeliveryDateRfq($poitem);
    	}
    ...
    public function change_detail_po($po_id, $po_number) {
    	// $this->load->model('po_header');
    	// $this->po_header->where_po($po_id);
    	// $po_header = $this->po_header->get();
    	...
    	$this->load->model('po_detail');
    	$this->po_detail->where_po($po_id);
    	$po_item = $this->po_detail->get();
    	// echo "<pre>";
    	// print_r($po);die;
    	$this->load->library('sap_handler');
    	
    	foreach ($po_item as $val) {
    		$poitem['rfq_no'] = $val['RFQ_NO'];
    		$poitem['rfq_item'] = $val['POD_NUMBER'];
    		$poitem['delivery_date'] = date('d.m.Y', oraclestrtotime($val['DDATE']));
    		$return = $this->sap_handler->change_detail_po($po_number, $poitem);
    	}
    ...
    public function kirim_email_po($data_email){		
    	...
    	$this->email->subject("[Development]Pemberitahuan Tender Awarded / PO Released dari eProcurement ".$this->session->userdata['COMPANYNAME'].".");
    	$content = $this->load->view('email/tender_awarded',$data_email['data'],TRUE);
    	$this->email->message($content);
    	$this->email->send();
    }

    public function kirim_email_po_approve($data_email){		
    	$this->load->library('email');
    	$this->config->load('email'); 
    	$semenindonesia = $this->config->item('semenindonesia'); 
    	$this->email->initialize($semenindonesia['conf']);
    	...
    	$this->email->subject("[".$data_email['EMAIL_ADDRESS']."]Pemberitahuan PO Approval dari eProcurement ".$this->session->userdata['COMPANYNAME'].".");
    	$content = $this->load->view('email/tender_approve',$data_email['data'],TRUE);
    	$this->email->message($content);
    	$this->email->send();
    }

    public function progress_kirim_email_po_approve($po_id){		
    	$this->load->model('adm_employee');
    	$this->load->model('po_approval');
    	$this->load->model('po_header');
    	$this->load->model('po_detail');
    	...
    	$this->po_approval->where_po($po_id);
    	$this->po_approval->where_approve();
    	$ambil_pegawai = $this->po_approval->get();

    	$pegawai_data = $this->adm_employee->ambilData($ambil_pegawai[0]['APPROVE_BY']);

    	$data_email = array(
    		'EMAIL_ADDRESS'=>$pegawai_data[0]['EMAIL'],
    		'data'=>array(
    			'PTM_SUBJECT_OF_WORK'=>$data_ptm[0]['PTM_SUBJECT_OF_WORK'],
    			'PPR_PRNO'=>$data_ppr['PPR_PRNO'],
    			'vendorno'=>$data[0]['VND_CODE'],
    			'vendorname'=>$data[0]['VND_NAME'],
    			'no_po'=>$data[0]['LP3_NUMBER'],
    			'doc_date'=>date('d M Y', oraclestrtotime($data[0]['DOC_DATE'])),
    			'ddate'=>date('d M Y', oraclestrtotime($data[0]['DDATE'])),
    			'total'=>number_format($data[0]['TOTAL_HARGA'],2,",",".")." ".$po_detail[0]['CURR'],
    		)
