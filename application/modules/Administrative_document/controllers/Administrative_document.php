...
class Administrative_Document extends CI_Controller {
	...
	public function do_update_company_board() {
		...
		if ($this->input->post('npwp_doc')) {
			$data['NPWP_FILE'] = $this->input->post('npwp_doc');
		}
	...
	function do_insert_company_board() {
		...
		if ($this->input->post('ktp_expired_date')) {
			$ktp_expired_date = vendortodate($this->input->post('ktp_expired_date'));
		} else {
			$ktp_expired_date = '';
		}
		if ($this->input->post('ktp_doc')) {
		...
		if ($this->input->post('npwp_doc')) {
			$npwp_doc = $this->input->post('npwp_doc');
		}
		$data = array(
				'VENDOR_ID' => $vendor_id,
				'TYPE' => $type,
	...
	function do_update_bank_financial(){
		...
		$new = $this->input->post('file_bank');
		$old = $this->input->post('file_lama');
		...
		$where = array("VENDOR_ID" => $data['VENDOR_ID'],'BANK_ID' => $bank_id);
	...
	function do_insert_financial_report() {
		...
		if($this->input->post('file_rpt')) {
			$file_rpt = $this->input->post('file_rpt');
		} else {
			$file_rpt = "";
		}
		$data = array(
	...
	function do_update_laporan_keuangan(){
		...
		$where = array("VENDOR_ID" => $data['VENDOR_ID'],'FIN_RPT_ID' => $fin_rpt_id);
