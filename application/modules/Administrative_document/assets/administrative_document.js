...
$(document).ready(function(){
	...
	$('#save_new_legal').click(function(){
		...
		if (vendor_type == 'NASIONAL' || vendor_type == 'EXPEDITURE') {
			...
			/*API*/
		// 	if ($('#api_no').val()!==''){
		// 		$('.legal_form').find('.api_upload').each(function() {
		// 			if ($(this).data('uploaded') != true) {
		// 				$(this).css('color','red');
		// 				gagal = true;
		// 			}
		...
		if (success) { 
			if (continueProcess) {
				...
				$.ajax({
					...
					{	
						swal("Berhasil!", "Simpan data berhasil!", "success")
						url = base_url+'Administrative_document/company_board';
			...
			if($('#npwp_prop').length)
				...
				$('#saveandcont_company_board').click(function(){
					...
					if (success) {
						...
						if (continueProcess) {
							swal("Berhasil!", "Tambah data berhasil!", "success")
							url = base_url+'Administrative_document/bank_and_financial_data';
							window.location.href = url;
						};
					};
					success = true;
				...
				if (url[url.length-1].toLowerCase() == "bank_and_financial_data") {
					...
					$('#addFinReport').click(function(){
						...
						gagal = false;
						$('.insert_financial_report').find('.rpt').each(function() {
							if ($(this).data('uploaded') != true) {
								$(this).css('color','red');
								gagal = true;
							}
