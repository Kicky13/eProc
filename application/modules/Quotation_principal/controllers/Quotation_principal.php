...
class Quotation_principal extends CI_Controller {
	...
	public function index() {
		...
		$this->layout->add_js('pages/quotation_principal.js');
	...
	public function get_quotation_list($from=null) {
		...
		$quotations = $this->prc_tender_vendor->get(array(
				'PTV_VENDOR_CODE' => $value['VENDOR_NO'], 
				'PTV_STATUS >=' => 1,
				'IS_VENDOR_CLOSED' => 0,
				'PTV_APPROVAL >=' => 0,
				'PTV_PC_CODE' => $this->session->userdata('PC_CODE')
			));
	...
	public function inputQuotation($id, $vendor_no, $viewSubmitted=null) {
		$id = url_decode($id);
		...
		if (empty($pqm)) {
			...
			$data['evatek'] = $this->prc_tender_quo_eval->ptqe($data['pqm_id']);
		}
		//////////////////////////////////////////////////////////////////////
		...
		// var_dump($data['template_eval']);
		// var_dump($data['item']); die();
		// END PO
		$this->layout->set_table_js();
		$this->layout->set_table_cs();
		$this->layout->add_js('pages/input_quotation_principal.js');
	...
	public function save_bidding_tes() {
		$this->load->model('prc_tender_prep');
		$this->load->model('prc_tender_quo_eval');
		...
		$id = url_decode($id);
		// $no = 233;
		// var_dump($this->input->post('cek_eval'));
		// var_dump();
		// var_dump($this->input->post('optradio'.$no));
		/* Insert into PRC_TENDER_QUO_EVAL */
		$id_pptu = $this->input->post('cek_eval');
		$ptp = $this->prc_tender_prep->ptm($id);
		$id_evaluasi = $ptp['PTP_EVALUASI_TEKNIS'];
		$template = $this->get_detail_template($id_evaluasi);

		foreach ($template['detail'] as $value) {
			if($value['PRASYARAT']==1){
				foreach ($value['uraian'] as $key) {
					$ptqe['PQE_ID'] = $this->prc_tender_quo_eval->get_id();
					$ptqe['PQM_ID'] = '100';
					$ptqe['PPD_ID'] = $id_evaluasi;
					$ptqe['PPTU_ID'] =  $key['PPTU_ID'];
					$ptqe['PQE_CHECK'] = 0;
					$ptqe['PQE_RESPON'] = '';
					foreach ($id_pptu as $pptu) {
						if($key['PPTU_ID']==$pptu){
							$ptqe['PQE_CHECK'] = 1;
							$ptqe['PQE_RESPON'] = $this->input->post('optradio'.$key['PPTU_ID']);
							continue;
						}
					}
					$this->prc_tender_quo_eval->insert($ptqe);
				}
			}			
		}
	}
	public function save_bidding() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$error = str_replace("</p>", "", $error);
			// echo $error;
			redirect('Quotation_principal/inputQuotation/'.$this->input->post('ptm_number'));
			return;
		} else {
			$id = $this->input->post('ptm_number');
			$id = url_decode($id);
			...
			// $vendorno = $this->session->userdata('VENDOR_NO');
			$vendorno = $this->input->post('vendor_no');
			...
			if ($pqm_id != null) {
				$this->prc_tender_quo_main->update($set, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_quo_main','update',$set,$where);
					//--END LOG DETAIL--//
			}
			unset($where);
			unset($set);
			...
			if ($pqm_id == null) {
				$this->prc_tender_quo_main->insert($pqmnew);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_quo_main','insert',$pqmnew);
					//--END LOG DETAIL--//
				$newid = $this->prc_tender_quo_main->get_id() - 1;
			} else {
				$this->prc_tender_quo_main->update($pqmnew, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_quo_main','update',$pqmnew,$where);
					//--END LOG DETAIL--//
			}

			/* Delete item PRC_TENDER_QUO_EVAL */
			if ($pqm_id != null) {
				unset($where);
				$where['PQM_ID'] = $newid;
				$this->prc_tender_quo_eval->delete($where);	
			}
			/* Insert into PRC_TENDER_QUO_EVAL */
			$id_pptu = $this->input->post('cek_eval');
			$ptp = $this->prc_tender_prep->ptm($id);
			...
			$this->prc_tender_quo_item->delete($where);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_quo_item','delete',null,$where);
				//--END LOG DETAIL--//
			...
			if ($this->input->post('ptqi') != null) {
				foreach ($this->input->post('ptqi') as $tit_id => $ptqi) {
					if (in_array($tit_id, (array)$this->input->post('check'))) {
						...
						$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_quo_item','insert',$log_ptqi);
							//--END LOG DETAIL--//
					}
				}
			}
			...
			foreach ($file_upload as $key => $val) {
				if(!empty($val)){
					...
					$this->prc_eval_file->insert($pef);	
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_eval_file','insert',$pef);
						//--END LOG DETAIL--//
				}			
			}
			...
			$this->prc_tender_vendor->update($set, $where);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_principal/save_bidding','prc_tender_vendor','update',$set, $where);
				//--END LOG DETAIL--//
	...
	public function save_harga() {
		...
		foreach($this->input->post('ptqi') as $ptqi) {
			...
			$this->prc_tender_quo_item->update($newptqi, $where_ptqi);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_principal/save_harga','prc_tender_quo_item','update',$newptqi, $where_ptqi);
				//--END LOG DETAIL--//
		}
		//*/
		...
		$this->log_data->detail($LM_ID,'Quotation_principal/save_harga','prc_tender_vendor','update',$set_ptv2, $where);
			//--END LOG DETAIL--//
	...
	public function viewDokppm($id = null){
		...
		//$this->config->set_item('base_url',$tempBase);
		if (strpos($baseUrl, 'semenindonesia.com') !== false) { //server production
			if(empty($id)){
				die('tidak ada attachment.');
			}
			if(file_exists(BASEPATH.'../upload/ppm_document/'.$id)){				
				...
				redirect(INT_BASEURL.'upload/ppm_document/'.$id);
			}
