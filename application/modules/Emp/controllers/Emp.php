<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	public function update(){
		...
		if($btn=='Departemen'){
			...
			$this->db->where('COMPANY', 4000);
			$this->db->delete('HRIS_V_UNIT_KERJA');
			// die;

			$slc_dept = $db2->select('company AS COMPANY,muk_begda AS MUK_BEGDA,muk_cctr AS MUK_CCTR,muk_endda AS MUK_ENDDA,
				muk_kode AS MUK_KODE,muk_level AS MUK_LEVEL,muk_nama AS MUK_NAMA,muk_parent AS MUK_PARENT,muk_short AS MUK_SHORT')->where('company',4000)->get('v_unit_kerja');
			...
			$slc_jab = $db2->select('mjab_kode AS MJAB_KODE,mjab_nama AS MJAB_NAMA,company AS COMPANY,muk_kode AS MUK_KODE,
				...
				mjab_endda AS MJAB_ENDDA,is_chief AS IS_CHIEF')->where('company',4000)->get('v_jabatan');
			...
			$this->db->where('COMPANY', 4000);
			$this->db->delete('HRIS_V_KARYAWAN');
			// die;
			...
			')->where('company',4000)->get('v_karyawan');
			...
			')->where('company',4000)->get('v_atasan');
