...
class Nego_invitation_principal extends CI_Controller { 
	...
	function index() {
		...
		$this->layout->add_js('pages/nego_invitation_principal.js');
		$this->layout->render('invitation_list',$data);
	}

	function detail_invitation($ptv_id, $vendorno) {
		$ptv_id = url_decode($ptv_id);
		$this->load->model('prc_nego_detail');
		$data['title'] = "Nego Invitation Detail Principal";
		...
		$items = $this->prc_tender_quo_item->ptm_ptv($data['tender_invitation']['PTM_NUMBER'], $vendorno);
		//echo $items; die;
		$data['negos'] = $this->prc_tender_nego_sech->ptm_ptv($data['tender_invitation']['PTM_NUMBER'], $vendorno);
		...
		$this->layout->set_table_js();
		$this->layout->add_js('pages/nego_invitation_principal.js');
		$this->layout->add_js('pages/nego_invitation_principal_detail.js');
	...
	function get_invitation_list() {
		...
		$invitations = $this->prc_tender_vendor->ptv($value['VENDOR_NO']);
	...
	function do_update_tender_invitation($ptv_id, $vendorno) {
		...
		if(strtotime(date('d-M-Y g.i.s A'))>$negoend_time){
			...
			redirect('Nego_invitation_principal/detail_invitation/' . url_encode($ptv_id));
			exit();
		}
		
			//--LOG MAIN--//
		$this->log_data->main($vendorno,$ptv[0]['VENDOR_NAME'],
		...
		if(!empty($_FILES['file_negosiasi']['name'])){
			...
			$this->prc_tender_nego_vendor_doc->insert($ptnvd);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Nego_invitation_principal/do_update_tender_invitation','prc_tender_nego_vendor_doc','insert',$ptnvd);
				//--END LOG DETAIL--//
		}
		...
		foreach ($this->input->post('pqi') as $val) {
			...
			//if ($negodetail[0]['HARGA'] != $valfinalprice) {
				...
				$this->log_data->detail($LM_ID,'Nego_invitation_principal/do_update_tender_invitation','prc_nego_detail','update',$setnegodetail2,$wherenegodetail);
					//--END LOG DETAIL--//
			//}
		}

		if($ptv[0]['PTV_TENDER_TYPE']==1 && $ptv[0]['PTV_APPROVAL']==2){
			$this->prc_tender_vendor->update(array('PTV_APPROVAL' => '4'),array('PTV_ID' => $ptv_id));
		}else if($ptv[0]['PTV_TENDER_TYPE']==2){
			$this->prc_tender_vendor->update(array('PTV_APPROVAL' => '3'),array('PTV_ID' => $ptv_id));
		}
		//*/

		// exit();
		$this->session->set_flashdata('success', 'success');
		redirect('Nego_invitation_principal/detail_invitation/' . url_encode($ptv_id).'/'.$vendorno);
	}
