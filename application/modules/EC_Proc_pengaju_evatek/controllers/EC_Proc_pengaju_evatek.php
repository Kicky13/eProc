...
class EC_Proc_pengaju_evatek extends CI_Controller {
	...
	public function index($id) {
		...
		$data['title'] = 'Pengajuan Evaluasi Teknis Baru';
		$data['ptm_number'] = $id;
	...
	public function save_approval(){
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_approval','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if($submit == '1'){
			...
			if($app[0]['PROCESS_MASTER_ID']=='EC_Proc_pengaju_evatek/approval'){
	...
	public function back_current_process($id){
		...
		$where = array(
				...
				'PROCESS_MASTER_ID'=>'EC_Proc_pengaju_evatek/index',
			);
	...
	public function save_bidding() {
		...
		if ($next_process == '999') { //BATAL
			redirect('Proc_verify_entry/batal/' . $id .'/'.$this->input->post('process_name').'/EC_Proc_pengaju_evatek/index');
		}
		...
		if ($post['evaldio'] == 1) {
			...
			if (empty($post['evaluator'])) {
				redirect('EC_Proc_pengaju_evatek/index/' . $post['ptm_number']);
			}
		...
		$this->prc_evaluator->deleteByPtm($id);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_bidding','prc_evaluator','delete',null,array('PTM_NUMBER' => $id));
			//--END LOG DETAIL--//
		...
		$this->prc_evaluator->insert($pe);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_bidding','prc_evaluator','insert',$pe);
			//--END LOG DETAIL--//
		...
		foreach ($pqis as $pqi) {
			foreach ($ppd as $pp) {
				...
				$this->prc_do_evatek->insert($det);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_bidding','prc_do_evatek','insert',$det);
					//--END LOG DETAIL--//
				...
				foreach ($eus as $eu) {
					...
					$this->prc_do_evatek_uraian->insert($deu);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_bidding','prc_do_evatek_uraian','insert',$deu);
						//--END LOG DETAIL--//
				}
			}
		}
		...
		$this->comment->insert_comment_tender($dataComment);
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Proc_pengaju_evatek/save_bidding','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
