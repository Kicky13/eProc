...
class Master_PO extends CI_Controller {
	...
	function view() {
		...
		$data['title'] = 'Master HS';
		$data['po'] = $this->m_master_po->tampil_data();
		// $this->load->view('transaksi_po',$data);
		$this->layout->set_table_js();
		$this->layout->set_table_cs();
		$this->layout->set_validate_css();
		$this->layout->set_validate_js();
		$this->layout->add_js('pages/Master_PO.js');
		$this->layout->render('transaksi_po', $data);

	}

	public function get_new_master_po() { // function untuk menampilakan data vendor yang expired
		$list_data = $this->list_master_po_data();
		$list_data = $list_data[0];
		$data_array = array('data'=>$list_data);
		echo json_encode($data_array);
	}


	function list_master_po_data(){ // function untuk menampilkan vendor mana saja yang memiliki data expired
		$data = array();
		$list_master_po = $this->m_master_po->tampil_data()->result_array();
		array_push($data,$list_master_po);
		return $data;
	}

	public function edit($id){
		$where = array('ID' => $id);
		$data['dataCode'] = $this->m_master_po->edit_data($where,'MON_PO_TRANS_MASTER')->result();
	...
	public function update(){ 
		...
		$this->m_master_po->update_data($where,$data,'MON_PO_TRANS_MASTER');
		redirect('Master_PO/view');
	}
	...
	public function tambah_aksi(){
		...
		$this->m_master_po->input_data($data,'MON_PO_TRANS_MASTER');
		redirect('Master_PO/view');
	}
	...
	public function hapus(){
		...
		$id = $this->input->post('id');
		$where = array('ID' => $id);
		$query = $this->m_master_po->hapus_data($where,'MON_PO_TRANS_MASTER');
		echo $id;
		// redirect('Master_PO/view');
	}
 ...
 //            		$checkAdaTidak = $this -> ec_auction_itemize_m -> getDetailItem2($KODE_BARANG, $iduser);
 ...
 //    	$tampil.='
 //    	<form class="form-horizontal" action="'.base_url().'EC_Auction_itemize/addHargaAwal" method="POST">
 ...
 //    						<td>
 //    							<input type="hidden" class="form-control" name="kd_itm[]" id="kd_itm[]" required="" value="'.$value['KODE_BARANG'].'">
 //    							<input type="hidden" class="form-control" name="currency[]" id="currency[]" required="" value="'.$resultPT[0]['CURRENCY'].'">
 //    							<input type="text" class="form-control format-koma" name="harga_awal[]" id="harga_awal[]" required="" placeholder="Harga Awal" value="'.number_format($harga_awal,0,".",".").'">
 //    						</td>
 //    						<td>
 //    							<input type="text" class="form-control format-koma" name="bea_masuk[]" id="bea_masuk[]" required="" placeholder="BEA MASUK" value="'.number_format($bea_masuk,0,".",".").'">
 //    						</td>
 //    					</tr>
 //    					';												
 //    				}
 ...
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, 1, "Deskripsi_Item");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, 1, "Kuantiti");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, 1, "Uom");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, 1, "Tipe Auction");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "Currency");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, 1, "Harga_Awal");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, 1, "Bea_masuk");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, 1, "Delivery_Time");
 //    	// $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, 1, "");
 //    	// $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 1, "Keterangan Currency");
 //    	// $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12, 1, "");
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 1, "Informasi");
 ...
 //    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $hs['NAMA_BARANG']);
 //    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $hs['JUMLAH']);
 //    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $hs['UNIT']);
 //    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $tipe);
 //    		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $ambilCurrPeserta[0]['CURRENCY']);
 ...
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 2, 'Delivery_Time = Satuan Hari');
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 3, 'Bea_masuk = Dalam bentuk prosentase dan ditulis tanpa %(Angkanya saja)');
 //    	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, 4, 'Currency = '.$fieldsCurr[0]['CURR_CODE'].'('.$fieldsCurr[0]['CURR_NAME'].')');
 ...
 //    	$objPHPExcel->getActiveSheet()->getStyle('H2:H'.($row-1))
