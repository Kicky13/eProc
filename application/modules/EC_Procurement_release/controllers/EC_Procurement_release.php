...
class EC_Procurement_release extends CI_Controller {
	...
	public function index($id) {
		...
		$data['title'] = "Approval Pengadaan Baru";
		...
		$this->layout->add_js('pages/EC_procurement_pengadaan_khususRO.js');
	...
	public function save_bidding() {
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if ($submit == "accept") {				
			...
			if (($rfqdate > $quodeadline) || ($quodeadline > $ddate)) {
				...
				redirect('EC_Procurement_release/index/' . $id);
			}
			//*/

			$this->prc_tender_prep->updateByPtm($id, $ptp);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_prep','update',$ptp,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			$this->prc_tender_main->updateByPtm($id, $ptm);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_main','update',$ptm,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			//*/
			...
			if ($oldptm['PTM_PRATENDER'] == '') {
				...
				$this->prc_tender_approve->insert($tap);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_approve','insert',$tap);
					//--END LOG DETAIL--//
				...
				$this->prc_tender_vendor->delete($data);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_vendor','delete',null,$data);
					//--END LOG DETAIL--//
				$tapv['TAP_ID'] = $tap_id;
				...
				foreach ($postvendor as $key) {
					...
					$this->prc_tender_vendor->insert_array($data);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_vendor','insert',$data);
						//--END LOG DETAIL--//

					$tapv['PTV_VENDOR_CODE'] = $key;
					$this->prc_tender_approve_vendor->insert($tapv);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_approve_vendor','insert',$tapv);
						//--END LOG DETAIL--//
					$tapv['TAPV_ID']++;
					...
					if($this->input->post('ptp_justification') == 5){ //Penunjukan Langsung - Repeat Order (RO) 
						...
						foreach ($tnd_itm as $val) {
							...
							$this->prc_tender_item->update($item, $wher);
								//--LOG DETAIL--//
							$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_item','update',$item,$wher);
								//--END LOG DETAIL--//
						}
					}
			...
			if ($doc != false) {
				foreach ((array) $doc as $key => $value) {
					...
					$this->prc_plan_doc->update(array("IS_SHARE" => $value), array('PPD_FILE_NAME' => $name));
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_plan_doc','update',array("IS_SHARE" => $value), array('PPD_FILE_NAME' => $name));
						//--END LOG DETAIL--//
				}
			}
			...
			if ($process['IDENTITAS_PROCCESS'] == 2) {
				...
				$this->prc_tender_main->updateByPtm($id, array('IS_VENDOR_CLOSED' => 0));
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_main','update',array('IS_VENDOR_CLOSED' => 0), array('PTM_NUMBER' => $id));
					//--END LOG DETAIL--//
				...
				foreach ($tnd_itm as $t) {					
					...
					$this->prc_pr_item->update($setpritem, $wherepritem);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_pr_item','update',$setpritem,$wherepritem);
						//--END LOG DETAIL--//
						//*/
				}
			...
			foreach ($item as $key => $value) {
				...
				$this->prc_pr_item->update(array('PPI_QTY_USED' => $pr_item[0]['PPI_QTY_USED'] - $value['TIT_QUANTITY']), array('PPI_ID'=>$pr_item[0]['PPI_ID']));
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_pr_item','update',array('PPI_QTY_USED' => $pr_item[0]['PPI_QTY_USED'] - $value['TIT_QUANTITY']), array('PPI_ID'=>$pr_item[0]['PPI_ID']));
					//--END LOG DETAIL--//
			}
			$data = $this->prc_tender_main->ptm($id);
			$this->prc_tender_main->updateByPtm($id, array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_release/save_bidding','prc_tender_main','update',array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ), array('PTM_NUMBER' => $id));
				//--END LOG DETAIL--//
	...
	public function generate_rfq($ptm_number, $LM_ID) {
		...
		if (($rfqdate > $quodeadline) || ($quodeadline > $ddate)) {
			$this->session->set_flashdata('error', 'Tanggal RFQ harus kurang dari quotation deadline dan tanggal quotation deadline harus kurang dari delivery date');
			redirect('EC_Procurement_release/index/' . $ptm_number);
		}
		...
		if (empty($bidderlist)) {
			...
			$this->prc_tender_main->updateByPtm($ptm_number, $new_update);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_release/generate_rfq','prc_tender_main','update',$new_update,array('PTM_NUMBER' => $ptm_number));
				//--END LOG DETAIL--//
		...
		if (isset($updateptp)) {
			$this->prc_tender_prep->updateByPtm($ptm_number, $updateptp);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_release/generate_rfq','prc_tender_prep','update',$updateptp,array('PTM_NUMBER' => $ptm_number));
				//--END LOG DETAIL--//
		}
		if (!empty($rfq)) {
			foreach ($rfq as $val) {
				...
				$this->prc_tender_vendor->update($set, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_release/generate_rfq','prc_tender_vendor','update',$set,$where);
					//--END LOG DETAIL--//
				// $error = true;
			}
			...
			foreach ($tit as $item) {
				...
				foreach ($pptu as $uraian) {
					...
					$this->prc_evaluasi_uraian->insert($new_eu);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_release/create_uraian','prc_evaluasi_uraian','insert',$new_eu);
						//--END LOG DETAIL--//
				}
			}
		}
