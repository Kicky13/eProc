...
class Quotation_vendor extends CI_Controller {
	...
	public function index() {
		...
		$this->layout->add_js('pages/quotation_vendor.js');
	...
	public function inputQuotation($id, $viewSubmitted=null) {
		...
		$this->layout->set_table_js();
		$this->layout->set_table_cs();
		$this->layout->add_js('pages/input_quotation_vendor.js');
	...
	public function save_bidding() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$error = str_replace("</p>", "", $error);
			// echo $error;
			redirect('Quotation_vendor/inputQuotation/'.$this->input->post('ptm_number'));
			return;
		} else {
			...
			if ($pqm_id != null) {
				$this->prc_tender_quo_main->update($set, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_quo_main','update',$set,$where);
					//--END LOG DETAIL--//
			}
			unset($where);
			unset($set);
			...
			if ($pqm_id == null) {
				$this->prc_tender_quo_main->insert($pqmnew);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_quo_main','insert',$pqmnew);
					//--END LOG DETAIL--//
				$newid = $this->prc_tender_quo_main->get_id() - 1;
			} else {
				$this->prc_tender_quo_main->update($pqmnew, $where);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_quo_main','update',$pqmnew,$where);
					//--END LOG DETAIL--//
			}
			...
			$this->prc_tender_quo_item->delete($where);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_quo_item','delete',null,$where);
				//--END LOG DETAIL--//
			...
			if ($this->input->post('ptqi') != null) {
				foreach ($this->input->post('ptqi') as $tit_id => $ptqi) {
					if (in_array($tit_id, (array)$this->input->post('check'))) {
						...
						$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_quo_item','insert',$log_ptqi);
							//--END LOG DETAIL--//
					}
				}
			}
			...
			foreach ($file_upload as $key => $val) {
				if(!empty($val)){
					...
					$this->prc_eval_file->insert($pef);	
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_eval_file','insert',$pef);
						//--END LOG DETAIL--//
				}			
			}
			...
			$this->prc_tender_vendor->update($set, $where);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_vendor/save_bidding','prc_tender_vendor','update',$set, $where);
				//--END LOG DETAIL--//
	...
	public function save_harga() {
		...
		foreach($this->input->post('ptqi') as $ptqi) {
			...
			$this->prc_tender_quo_item->update($newptqi, $where_ptqi);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'Quotation_vendor/save_harga','prc_tender_quo_item','update',$newptqi, $where_ptqi);
				//--END LOG DETAIL--//
		}
		//*/
		...
		$this->log_data->detail($LM_ID,'Quotation_vendor/save_harga','prc_tender_vendor','update',$set_ptv2, $where);
			//--END LOG DETAIL--//
