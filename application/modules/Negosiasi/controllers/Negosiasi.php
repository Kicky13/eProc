...
class Negosiasi extends CI_Controller {
	...
	public function index($id = '',$id_nego = '') {
	 ...
	 //                }
		...
		$this->layout->add_js('pages/update_doc_nego.js?2');
	...
	public function update_doc_nego(){
		$this->load->model('prc_tender_nego');

		// $image = $_FILES['files'];
		// $info = getimagesize($_FILES['files']['tmp_name']);
		// $source = $image['tmp_name'];
		// $path = FCPATH . 'upload/temp/';
		// $type = $image['type'];
		// $filename = date('dmY_hia') . $image['name'];
		// $destination = $path . $filename;
		// $type = $image['type'];
		// $upload = move_uploaded_file($image, $destination);

		$image = $_FILES['files'];
		$filename = date('dmY_hia') . $image['name'];
		$source = $image['tmp_name'];
		$tes 				= dirname(__FILE__);
		$pet_pat 			= str_replace('application/modules/Negosiasi/controllers', '', $tes);
		$filenya 			= "";
		$upload 			= false;
		// echo $pet_pat;die;
		if (isset($_FILES) && !empty($filename)) {
			$type = explode('.', $filename);
			
            $this->_myFile = "DOC_NEGO_" . date('dmY_hia') . "_" . $image['name'];
            $this->_path = $pet_pat . 'upload/doc_nego/' . $this->_myFile;
            // echo $this->_path;die;
            if (move_uploaded_file($source, $this->_path)) {
            	$filenya 	= $this->_myFile;
            	$upload 	= true;
            }
