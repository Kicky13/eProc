...
class EC_Procurement_pengadaan extends CI_Controller {
...
//			}
			...
			$data['title'] = "Konfigurasi pengadaan Terbaru";
			...
			// var_dump($data['vendor_tambahan'][0]['PTV_VENDOR_CODE']);
	...
	public function update($id) {
		...
		$LM_ID = $this->log_data->last_id();
			...
			//--LOG DETAIL--//
		$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_comment','insert',$dataComment);
			//--END LOG DETAIL--//
		...
		if ($submit == "accept") {
			...
			if (($rfqdate > $quodeadline) || ($quodeadline > $ddate)) {
				...
				redirect('EC_Procurement_pengadaan/get_detail/' . $id);
			}
			//*/

			$this->prc_tender_prep->updateByPtm($id, $ptp);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_prep','update',$ptp,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			...
			$this->prc_tender_main->updateByPtm($id, $ptm);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_main','update',$ptm,array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
			//*/
			...
			for ($i = 1; $i <= $nfiles ; $i++) { 
				...
				if (!empty($uploaded['add_doc'.$i])) {
					...
					$this->prc_add_item->insert($add);
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_add_item','insert',$add);
						//--END LOG DETAIL--//
				}
			}
			//*/
			...
			$this->prc_tender_approve->insert($tap);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_approve','insert',$tap);
				//--END LOG DETAIL--//
			...
			$this->prc_tender_vendor->delete($data);
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_vendor','delete',null,$data);
				//--END LOG DETAIL--//
			...
			foreach ($postvendor as $key) {
				...
				$this->prc_tender_vendor->insert_array($data);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_vendor','insert',$data);
					//--END LOG DETAIL--//

				$tapv['PTV_VENDOR_CODE'] = $key;
				$this->prc_tender_approve_vendor->insert($tapv);
					//--LOG DETAIL--//
				$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_approve_vendor','insert',$tapv);
					//--END LOG DETAIL--//
				$tapv['TAPV_ID']++;
			}
			...
			if ($doc != false) {
				foreach ((array) $doc as $key => $value) {
					...
					$this->prc_plan_doc->update(array("IS_SHARE" => $value), array('PPD_FILE_NAME' => $name));
						//--LOG DETAIL--//
					$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_plan_doc','update',array('IS_SHARE' => $value),array('PPD_FILE_NAME'=>$name));
						//--END LOG DETAIL--//
				}
			}
			...
			$this->prc_tender_main->updateByPtm($id, array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ));
				//--LOG DETAIL--//
			$this->log_data->detail($LM_ID,'EC_Procurement_pengadaan/update','prc_tender_main','update',array('PTM_STATUS' => $data['0']['PTM_STATUS'] * - 1 ),array('PTM_NUMBER'=>$id));
				//--END LOG DETAIL--//
	...
	public function get_principalList($vendorno){
		$listprincipal = $this->user_principal_model->getPrincipalList($vendorno);

		echo json_encode($listprincipal);
	}
}