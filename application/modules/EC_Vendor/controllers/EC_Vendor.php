...
class EC_Vendor extends  MX_Controller {
    ...
    public function Form_Enofa($id = '') {
        ...
        if ($id == '') {
            ...
            if($this->vendor_no != $data['value'][0]['CREATE_BY']){
                $this->session->set_flashdata('error','Anda Tidak Memiliki Hak untuk mengubah Pendaftaran E-Nofa tersebut');
        ...
        $data['companyCode'] = $this->session->userdata('COMPANYID');
    ...
    public function checkFaktur(){
        ...
        $old = $this->input->post('old_start');
        $ret = array('status' => 1);

        if($no_faktur != $old){
            $cek = $this->nova->get_enofa('WHERE CREATE_BY = '.$this->vendor_no,'');
            //var_dump($cek);die();
            for ($i=0; $i < count($cek); $i++) {
                if(preg_replace('/[^\p{L}\p{N}\s]/u', '', $cek[$i]['START_NO']) == $no_faktur){
                    $ret['status'] = 0;
                    $ret['msg'] = 'No Faktur Sudah Anda Daftarkan, Silahkan Tunggu Proses Verifikasi';
                }
            }
        }

        $this->load->library('sap_invoice');
        $cek = $this->sap_invoice->getRangeFakturNo(array('VENDOR_NO' => $this->vendor_no));

        /*Cek apakah no faktur yang akan diapprove sudah terdaftar*/
        for ($i=0; $i < count($cek); $i++) {
            if($cek[$i]['FPNUML'] == $no_faktur){
                $ret['status'] = 0;
                $ret['msg'] = 'No Faktur Sudah Terdaftar';
            }
        }
