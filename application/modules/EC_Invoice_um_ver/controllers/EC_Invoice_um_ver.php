...
class EC_Invoice_um_ver extends MX_Controller {
	...
	public function index($cheat = false) {
		...
		$this->layout->add_js('pages/EC_inv_um_ver.js?1');
	...
	public function detail($noinvoice = '1234567890') {
		...
		$this->layout->add_js('pages/invoice/EC_common.js');
		$this->layout->add_js('pages/EC_inv_um_ver_detail.js');
		$this->layout->add_js('pages/EC_bootstrap-switch.min.js');
		$this->layout->add_js('pages/EC-bootstrap-datepicker.min.js');
		$this->layout->add_js('pages/EC_jasny-bootstrap.min.js');
		$this->layout->add_js('jquery.alphanum.js');
		...
		$ambilUm = $this->ec_uang_muka->get(array('ID_UM' => $noinvoice));
		$data['detail_um'] = $ambilUm[0];

		$wherePajak = "ID_JENIS IN ('VZ', 'VN') AND STATUS = 1";
		$data['pajak'] = $this->db->where($wherePajak)->get('EC_M_PAJAK_INV')->result_array();

		$venno = $data['detail_um']['VENDOR_NO'];
		$data['listBank'] = $this->listBankVendor($venno);
	...
	public function get_data() {
		...
		$stat = 'VERIFIKASI';
        //	$venno = "0000112709";
		$_dataa = $this->ec_uang_muka->getManUmVer($stat);
	...
	public function update_invoice() {
		...
		if($status==2){
			$this->ec_open_um->setStatus_Invoice_ver34($id_um, $next_action, $this->session->userdata('FULLNAME'), $alasan_reject);
		} else {
			if($status==3){
			...
			$this->ec_open_um->setStatus_Invoice_ver5($id_um, 5);
			$data_tracking = array(
				'ID_UM' => $id_um,
				'DESC' => 'POSTED',
			...
			$this->eti->insertLog($data_tracking);
			$where_poapprove = array('ID_UM' => $id_um);
			$this->eti->update($data_tracking, $where_poapprove);
		}
		
		$this->load->model('invoice/ec_um_header', 'eh');
		$dataInv = $this->eh->get(array('ID_UM' => $ID_INVOICE));
		$pesan = "Uang Muka " . $id_um . ", PO " . $dataInv->NO_SP_PO . " Approved";
		$this->session->set_flashdata('message', $pesan);
		redirect('EC_Invoice_um_ver');
	}

	public function delete_invoice($ID_INVOICE) {
		header('Content-Type: application/json');
        //$json_data = array('data' => $dataa);
		$this->load->model('invoice/ec_um_header', 'eih');
		$this->load->model('ec_open_inv');
		$this->db->trans_begin();
	...
	public function listDokumen() {
		...
		$listDoc = $this->db->select('*')->get('MASTER_DOC_UM')->result_array();
		$this->load->model('ec_uang_muka');
		$ambilUm = $this->ec_uang_muka->get(array('ID_UM' => $invoice));
		$data['alasan_reject'] = '';
		if (!empty($ekspedisi)) {
		...
		$data['proses'] = $proses;
		$data['status'] = $status;
		$this->load->view('EC_Invoice_um/listDokumen', $data);
	}
	...
	public function updatePosisiDokumen() {
		...
		$alasan_reject = $this->input->post('alasan_reject');
		$cat = $this->input->post('catatan_vendor'); 
// Kondisi Ketika Vendor Kirim Ulang Dokumen
		...
		$dataInvoice = $this->eih->get(array('ID_UM' => $invoice));
		$tracking = $this->eti->order_by('DATE', 'DESC')->get(array('ID_UM' => $invoice));
		$data_tracking = array(
			'ID_UM' => $invoice,
			'DESC' => 'APPROVED',
		...
		$bisaKirim = array('BELUM KIRIM', 'RETUR', 'TERIMA');
        ...
        // }
		$this->eti->insertLog($data_tracking);
		$where_poapprove = array('ID_UM' => $invoice);
		$this->eti->update($data_tracking, $where_poapprove);
		$result['status'] = 1;
		$result['message'] = $pesan;
