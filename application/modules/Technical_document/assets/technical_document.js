...
$(document).ready(function(){
	...
	/* Jika Proses ambil data material dari SAP  
	...
	*/
	...
	$('#saveandcont_good_and_service').click(function(){
		window.location.href = "Human_resources_data";
	});
	...
	$('.update_vendor_product').click(function(){
		...
		$.ajax({
			url:base_url+'Technical_document/get_product_edit',
			dataType: "json",
			type:"post",
		...
		$.ajax({
			...
			success:function(data){
				$.each(data, function(i,product){
					...
					setTimeout(function(){$(".select3").select2();}, 2000);
				});
			}
		});
			...
			$.ajax({
				...
				success : function(result)
				{
					swal({
						  ...
						  function(isConfirm) {
							if (isConfirm) {
							  window.location=base_url+'Technical_document/human_resources_data';
							}
						  });
	...
	$('#saveandcont_sdm').click(function(){
		window.location.href = "Certification_data";
	});

	$('#saveandcont_certifications').click(function(){
		window.location.href = "Facility_and_equipment";
	});
	...
	/* CERTIFICATION DATA */
	if ((url[url.length-1].toLowerCase() == "certification_data") || (url[url.length-1].toLowerCase() == "company_experience")) {
			...
			var uploader = new ss.SimpleUpload({
				...
				onComplete: function( filename, response ) {
					...
					if ( response.success === true ) {
						...
						msgBox.html('<a style="color: #666; text-decoration: underline" href="'+base_url+response.upload_dir+response.newFileName+'">File Attachment</a>');
				...
				$.ajax({
					...
					success : function(result)
					{
						...
						swal({
						  ...
						  function(isConfirm) {
							if (isConfirm) {
							  window.location=base_url+'Technical_document/certification_data';
							}
						  });
					}
				})
	...
	if ((url[url.length-1].toLowerCase() == "certification_data") || (url[url.length-1].toLowerCase() == "facility_and_equipment") || (url[url.length-1].toLowerCase() == "company_experience")) {
				...
				$.ajax({
					...
					success : function(result)
					{
						...
						swal({
						  ...
						  function(isConfirm) {
							if (isConfirm) {
							  window.location=base_url+'Technical_document/facility_and_equipment';
							}
						  });
					}
				})
		...
		$('#saveandcont_equipments').click(function(){
			window.location.href = "Company_experience";
		});
				...
				$.ajax({
					...
					success : function(result)
					{
						swal({
						  ...
						  function(isConfirm) {
							if (isConfirm) {
							  window.location=base_url+'Technical_document/company_experience';
							}
						  });
					}
				})
		...
		$('#saveandcont_experiences').click(function(){
			window.location.href = "../Additional_document";
		});
	};
	...
	$('.update_vendor_certifications').click(function(){
		...
		var base_url = $('#base-url').val();
		$.ajax({
			url:base_url+'Technical_document/get_certification_edit',
			dataType: "json",
			type:"post",
...
$('.cek_mandatory_update').on('change', function() {
	...
	$('#update_ahli_data_button').click(function(){
		...
		if(success){
			...
			$.ajax({
				...
				success : function(result)
				{
					$('#updateTenagaPendukungModal').hide();
					swal({
							title: "Ubah Data Berhasil",
							//text: "Data akan Dihapus Dari Sistem !",
							type: "success",
    ...
    $('.uploadAttachment').each(function(event) {
        ...
        var uploader = new ss.SimpleUpload({
            button: btn,
            url: $("#base-url").val() + 'Technical_document/uploadAttachment',
            name: 'uploadfile',
            allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'odt', 'html', 'zip'],
            maxSize: 2048,
            multipart: true,
	...
	$('.delete_upload_file_ja').click(function(){
        $.ajax({
            url: $("#base-url").val() + 'Technical_document/deleteFile',
            type: 'POST',
            data: {
                id : $("#service_id").val(),
...
function delete_dacumum(th){
	var url=$(th).attr('ub_url');
	swal({
