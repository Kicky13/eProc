$(document).ready(function() {
    ...
    $('form').submit(function(e){
        ...
        if (check == 1) {
            bootbox.alert('No Akhir Faktur Harus Lebih Besar dari No Akhir');
        }else if(check == 2){
            bootbox.alert('Persiksa Kembali Tanggal Awal dan Akhir E-Nofa');
        }else{
            var old = hanyaAngka($('input[name=old_start]').val());
            var _url = $('#base-url').val() + 'EC_Vendor/checkFaktur';
            $.post(_url,{no_faktur:f_awal,old_start:old},function(data){
                if(data.status){
                    Form.submit();
                }else{
