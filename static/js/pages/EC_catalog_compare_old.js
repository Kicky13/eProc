		...
		if (data.data.length == 0)
			...
			for (var i = limitMin; i < data.data.length; i++) {
				...
				if (mode == 'list') {
					...
					// teks += ('<a href="' + base_url + 'EC_Ecatalog/detail_prod/' + data.data[i][5] + '/' + data.data[i][1] + '" style="font-size:12px;box-shadow: 1px 1px 1px #ccc"  class="btn btn-primary form-control beli" onclick="buyOneCheck(this,\'' + data.data[i][5] + '\',\'' + data.data[i][1] + '\',\'' + data.data[i][19] + '\')"><i class="glyphicon glyphicon-usd" ></i>&nbsp&nbspBuy</a>' )
					...
					// teks += '<a href="' + base_url + 'EC_Ecatalog/detail_prod/' + data.data[i][5] + '/' + data.data[i][1] + '" style="font-size:12px;margin:5px;width:80%;box-shadow: 2px 2px 2px 2px #ccc"  class="btn btn-primary form-control beli" data-category="'+data.data[i][19]+'"><i class="glyphicon glyphicon-usd" ></i>&nbsp&nbspBuy</a>'
...
function buyOneCheck(elm, contract_no, matno, category) {
	...
	$.ajax({
		...
		// console.log(data.sukses);
		if(data.sukses){
			window.location=$("#base-url").val() + 'EC_Ecatalog/detail_prod/'+contract_no+'/'+matno;
		}else{
			bootbox.alert("You don't have authorized");
		}
		// console.log(cek);
