...
function loadTable_() {
    ...
    mytable = $('#table_inv').DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_Report_cataloging/getReport_material',
        ...
        "columns": [{
            ...
            mRender: function (data, type, full) {
                a = "<div class='col-md-12 text-center'>" +
                    ...
                    '<a href="javascript:void(0)" data-toggle="modal" data-target="#modalDetail" data-cat="' + (full.ID_CAT) + '" data-matno="' + (full.MATNO) + '"><span title="Tracking" class="glyphicon glyphicon-search" aria-hidden="true"></span></a>' +
                    '</div>';
                return a;
            }
        }],
...
$('#modalDetail').on('show.bs.modal', function (event) {
    ...
    console.log(category + ' ' + matno)
    $.ajax({
        url: $("#base-url").val() + 'EC_Report_cataloging/getDetail_material/',
        type: 'POST',
        data:{
            'cat': category,
            'matno': matno
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i]['LOG_ACTIVITY'] == 0){
                teks += "<td class=\"text-center\">Propose</td>"
