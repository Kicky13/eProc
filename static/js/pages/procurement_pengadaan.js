...
function terpilih(e){
    ...
    if($(e).is(":checked")){
        ...
        if (!ada) {
            tr = '<tr>' +
                    '<td class="text-center"><input type="checkbox" name="coba_vnd_terpilih[]" class="vnd_terpilih" onclick="nocentang(this)" value="' + vendorno + '" checked></td>' +
                    '<td>' + vendorno + '</td>' +
...
//    });
    ...
    function populateTable() {
        ...
        template_table = $('#template-table').DataTable( {
            "ajax": $("#base-url").val() + 'Procurement_pengadaan/get_all_template',
            "columnDefs": [{
                "searchable": true,
                "orderable": false,
                "targets": 0
...
//end submit
    ...
    if ($("#just").val() != '5') {
        if($("#jenis_perencanaan").val() == 0){ //barang
            ...
            $.ajax({
                url: $("#base-url").val() + 'Procurement_pengadaan/get_vendor/' + $('#ptm_number').val(),
                type: 'get',
                dataType: 'json',
            })
            .done(function(data) {
            ...
            var tableGrid = $('#table_vendor_jasa').DataTable( {
                ...
                "paging": false,
                "ajax": {
                    url : $("#base-url").val() + "Procurement_pengadaan/get_vendor_jasa",
                    method : 'post',
                    data : {id_ptm},
                },
                "columnDefs": [{
