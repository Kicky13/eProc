...
function cek_hargawarning(e) {
    ...
    $.ajax({
        url: $("#base-url").val() + 'Nego_invitation_principal/cek_toleransi/' + e.parent().parent().find(".tit_id").val() + '/' + dat,
        type: 'post',
        dataType: 'json',
    })
    .done(function(data) {
...
//	$("#form_ub").validationEngine({promptPosition : "topRight", scroll: false});
	...
	$(".delete_file").click(function(){
		swal({
			  ...
			  showCancelButton: true,
				  ...
				  if (isConfirm) {
					$.ajax({
						url:$("#base-url").val() + 'Nego_invitation_principal/delete_file',
						type:'post',
	...
	$('#save_bidding').click(function(dataku){
		...
		if(state){
			...
			$.ajax({
				url:$("#base-url").val() + 'Nego_invitation_principal/cek_toleransi_ub',
				type:'post',
				data:datakirim,
				dataType:'Json',
				success:function(ea){
