...
function loadTable_() {
    ...
    mytable = $('#table_inv').DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_PO_PL_Approval/getPO_pl_approval',
        ...
        }, 
        {
            ...
            mRender: function (data, type, full) {
                a = "<div class='col-md-12 text-center'>" +
                    '<a href="javascript:approve(' + (full.NOMERPO) + ')"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></a>&nbsp;&nbsp;' +
                    '<a href="javascript:reject(' + (full.NOMERPO) + ')"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>&nbsp;&nbsp;' +
...
$('#modalDetil').on('show.bs.modal', function (event) {
    ...
    $.ajax({
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            teks += "<td class=\"text-center\"><a href=\"" + data[i]['FILE_KORIN'] + "\" target=\"_blank\">" + data[i]['FILE_KORIN'] + "</a></td>"
