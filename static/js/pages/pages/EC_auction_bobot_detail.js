...
function showItem(NO_TENDER) {
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot/get_item/' + NO_TENDER,
		data : {
		},
		type : 'POST',
		dataType : 'json'
...
function showListPeserta(NO_TENDER) {
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot/get_Peserta/' + NO_TENDER,
		data : {
		},
		type : 'POST',
		dataType : 'json'
		...
		if (data.data.length == 0)
			...
			for (var i = 0; i < data.data.length; i++) {
				...
				teks += '<td class="text-center">' + $("#CURR").text() + ' ' + numberWithCommas(data.data[i][4]) + '</td>'
				teks += '<td class="text-center">' + data.data[i][6] + ' ' + (parseInt(data.data[i][4]) < parseInt(removeDot($("#hps").text())) ? '<img width="8%" height="8%" src="' + $("#base-url").val() + '/upload/EC_auction/giphy.gif" />' : "") + '</td>'
				teks += '/<tr>'
				$("#Peserta").append(teks);
			}
	}).fail(function() {
...
function showLog(NO_TENDER) {
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot/getLog/' + NO_TENDER,
		data : {
		},
		type : 'POST',
		dataType : 'json'
...
function cetak(NO_TENDER) {
	...
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot/save_note/' + NO_TENDER,
		data : {
			NOTE : $('#Note').val()
		},
		type : 'POST',
		...
		// console.log(data);
		// location.reload();
		window.open($("#base-url").val() + 'EC_Auction_bobot/print_e_auction/' + NO_TENDER, "_blank");
	});
}
...
$(document).ready(function() {
	...
	var timeInterval3 = setInterval(function() {
		console.log('tik')
		$.ajax({
			url : $("#base-url").val() + 'EC_Auction_bobot_negotiation/getTimeServer/',
			type : 'POST',
			dataType : 'json'
		}).always(function(data) {
