...
function loadTable () {
    mytable = $('#tbl_group_akses').DataTable({
        // "dom": '<"toolbar">rtp',
        "ajax" : {'url':base_url + 'Emp/get_datatable_ub','type': 'POST',
		data: function(d){d.nama_company_f=$('#nama_company_f').val(),d.nama_unit_f=$('#nama_unit_f').val()
								,d.nama_jabatan_f=$('#nama_jabatan_f').val(),d.nama_posisi_f=$('#nama_posisi_f').val(),d.fullname_f=$('#fullname_f').val(),d.kode_mkcctr=$('#kode_mkcctr').val()
								},
...
function update(btn){
    $.ajax({
  		type: "post",
		url : base_url + 'Emp/update',
		data : "btn="+btn,
		success: function(data) {
			if(data=='ok'){
				swal("Berhasil!", "Update data "+btn+" berhasil!", "success");
			}else if(data=='error'){
				swal("Gagal!", "Update data "+btn+" gagal!", "error");
			}
			if(btn=='Employee'){
				location.reload();
			}
