        ...
        if (data.data.length == 0)
            ...
            for (var i = limitMin; i < data.data.length; i++) {
                if (mode == 'list') {
                    ...
                    teks += ('<p></p><a href="javascript:void(0)" style="font-size:12px;padding: 5px;box-shadow: 1px 1px 1px #ccc" class="btn btn-info form-control beli"  onclick="addCart(this,\'' + data.data[i].MATNR + '\',\'PL2017\',\'' + data.data[i].KODE_DETAIL_PENAWARAN + '\')"><i class="glyphicon glyphicon-shopping-cart"></i> Add to Cart</a>' )
                    teks += ('<p></p><a href="javascript:addCompare(this,\'' + data.data[i].MATNR + '\',\'PL2017\',' + data.data[i].KODE_DETAIL_PENAWARAN + ')" style="box-shadow: 1px 1px 1px #ccc;font-size:12px;" class="btn btn-warning form-control" style="padding: 5px;"><i class="glyphicon glyphicon-tasks"></i>&nbspCompare</a>' + '</form>' + '</div>' + '</div>' + '</div>')
                } else {
                    teks = ''
                    if (i % 3 == 0 || i == 0) {
                        teks += '<div class="row">'
...
function addCart(elm, matno, contract_no, kode_penawaran) {
    var COSCENTER = '1';
    $.ajax({
...
function addCompare(elm, matno, contract_no, kode_p) {
