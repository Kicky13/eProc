...
function piala(no_tender) {
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot_negotiation/piala/' + no_tender,
		type : 'POST',
		dataType : 'json'
	}).always(function(data) {
		// console.log(data)
...
function bid(no_tender) {
	$.ajax({
		url : $("#base-url").val() + 'EC_Auction_bobot_negotiation/bid/' + no_tender + '/' + removeDot(removeCommas($('#penawaran').val())),
		type : 'POST',
		dataType : 'json'
	}).always(function(data) {
		console.log(data)
