...
$(document).ready(function(){
    if (url[url.length-1].toLowerCase() == "registration_date") {
		var table_registration_date = $('#registration_date_list').DataTable( {
			...
			"serverSide": true,
			"lengthMenu": [ 10, 25, 50 ],
        	"ajax" : {'url':base_url + 'Vendor_registration_management/get_registration_date'},
			// "ajax": { url: location + "Vendor_registration_management/get_registration_date", type: 'POST'},
			...
			// 	}, "sClass": "text-center"},
			{
                mRender : function(data,type,full){
                    ...
                    ans = '<a href="'+base_url+'Vendor_registration_management/update_registration_date/'+full.COMPANYID+'" class="main_button color1 small_btn">Edit</a> ';
                    return ans;
            }, "sClass": "text-center"},
				],
			} );
		...
		var table_vendor_registration_news = $('#vendor_registration_news_list').DataTable( {
        	...
        	"ajax" : {'url':base_url + 'Vendor_registration_management/get_vendor_registration_news'},
			// "ajax": {
			...
			{"data" : "NEWS_TITLE"},
			{
                mRender : function(data,type,full){
                    ...
                    ans = '<a href="'+base_url+'Vendor_registration_management/update_vendor_registration_news/'+full.COMPANYID+'" class="main_button color1 small_btn">Edit</a> ';
                    return ans;
            }, "sClass": "text-center"},
			// {
			// 	mRender : function(data,type,full){
