...
function loadDataList() {
	...
	$.ajax({
		url : $("#base-url").val() + 'EC_Ecatalog' + urlll,
		data : {
			"min" : range_harga[0],
			"max" : range_harga[1],
			"limitMin" : limitMin,
		...
		if (data.data.length == 0)
			...
			for (var i = limitMin; i < data.data.length; i++) {
				...
				if (mode == 'list') {
					...
					teks += ('</div>' + '<div class="col-md-4" style="  padding-left: 35px;">' + '<div class="row" style="font-size:11px">Kategori: ' + data.data[i][16] + '</div>')
					...
					teks += ('<a href="' + base_url + 'EC_Ecatalog/detail_prod/' + data.data[i][5] + '/' + data.data[i][1] + '" style="font-size:12px;box-shadow: 1px 1px 1px #ccc"  class="btn btn-primary form-control beli"><i class="glyphicon glyphicon-usd" ></i>&nbsp&nbspBuy</a>' )
					teks += ('<p></p><a href="javascript:void(0)" style="font-size:12px;padding: 5px;box-shadow: 1px 1px 1px #ccc" class="btn btn-info form-control beli"  onclick="addCart(this,\'' + data.data[i][1] + '\',\'' + data.data[i][5] + '\')"><i class="glyphicon glyphicon-shopping-cart"></i> Add to Cart</a>' )
					teks += ('<p></p><a href="javascript:addCompare(this,\'' + data.data[i][1] + '\',\'' + data.data[i][5] + '\')" style="box-shadow: 1px 1px 1px #ccc;font-size:12px;" class="btn btn-warning form-control" style="padding: 5px;"><i class="glyphicon glyphicon-tasks"></i>&nbspCompare</a>' + '</form>' + '</div>' + '</div>' + '</div>')
				} else {
					teks = ''
					if (i % 3 == 0 || i == 0) {
						teks += '<div class="row">'
					...
					teks += '<div class="row" style="padding:20px;">'
					teks += ('<a href="javascript:void(0)" data-backdrop="true" data-toggle="modal" data-target="#modaldetail" data-produk="' + data.data[i][1] + '">')
					teks += ('<img src="' + $("#base-url").val() + $("#UPLOAD_PATH").val() + "EC_material_strategis/" + data.data[i][12] + '" class="img-responsive"></a>')
					teks += '</div>'
					teks += '<div class="row" style="font-size:11px;padding-left:5px;padding-right:5px;">Kategori: ' + data.data[i][16] + '</div>'
					teks += ('<div class="row" style="font-size:16px;padding-left:5px;padding-right:5px;">')
					teks += ('<strong><a href="javascript:void(0)" data-backdrop="true" data-toggle="modal" data-target="#modaldetail" data-produk="' + data.data[i][1] + '">' + data.data[i][2] + '</a></strong></div>')
					...
					teks += '<a href="' + base_url + 'EC_Ecatalog/detail_prod/' + data.data[i][5] + '/' + data.data[i][1] + '" style="font-size:12px;margin:5px;width:80%;box-shadow: 2px 2px 2px 2px #ccc"  class="btn btn-primary form-control beli"><i class="glyphicon glyphicon-usd" ></i>&nbsp&nbspBuy</a>'
					teks += '<a href="javascript:void(0)" style="font-size:12px;padding: 5px;margin:5px;width:80%;box-shadow: 2px 2px 2px 2px #ccc" class="btn btn-info form-control beli"  onclick="addCart(this,\'' + data.data[i][1] + '\',\'' + data.data[i][5] + '\')"><i class="glyphicon glyphicon-shopping-cart"></i> Add to Cart</a>'
					teks += '<a href="javascript:addCompare(this,\'' + data.data[i][1] + '\',\'' + data.data[i][5] + '\')" style="font-size:12px;margin:5px;width:80%;box-shadow: 2px 2px 2px 2px #ccc" class="btn btn-warning form-control" style="padding: 5px;"><i class="glyphicon glyphicon-tasks"></i>&nbspCompare</a>'
					teks += '</div>'
					teks += '</div>'
				}
...
function loadDataListPricelist() {
	...
	$.ajax({
		url : $("#base-url").val() + 'EC_Ecatalog' + '/get_data_pricelist', //urlll,
		data : {
			"min" : range_harga[0],
			"max" : range_harga[1],
			"limitMin" : limitMin,
	...
	if (confirm("Konfirmasi Order?"))
		if (qty > 0 && $(".qtyy").val().search(/[A-Za-z]/g) == -1)//
			if (qty < $("#avl").val())
				if ($("#sisa").val() > 0)
					$.ajax({
						url : $("#base-url").val() + 'EC_Ecatalog/confirmOne/',
						data : {
							"qty" : $(".qtyy").val(),
						...
						po = (data[0].PO_NO) != po ? (data[0].PO_NO) : ""
						$('#tbodyPO').append('<tr></tr>')
						$('#tbodyPO tr:eq(' + 0 + ')').append('<td>' + (po == "null" ? "gagal" : po) + '</td>')
						$('#tbodyPO tr:eq(' + 0 + ')').append('<td>' + (data[0].MAKTX) + '   (' + (data[0].MATNO) + ')</td>')
						$('#tbodyPO tr:eq(' + 0 + ')').append('<td>' + (data[0].curr) + ' ' + numberWithCommas(data[0].netprice * $(".qtyy").val()) + '</td>')
						// };
...
function addCompare(elm, matno, contract_no) {
	...
	$.ajax({
		url : $("#base-url").val() + 'EC_Ecatalog/addCompare/' + matno,
		data : {
			"contract_no" : contract_no
		},
		type : 'POST',
