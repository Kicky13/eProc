...
$(document).ready(function(){
	...
	$('.nama').autocomplete({
        dropdownParent: $('#tbl_parent_item'),
            // serviceUrl berisi URL ke controller/fungsi yang menangani request kita
        serviceUrl: $("#base-url").val() + 'ET_Procurement_template/search_nama',
        // fungsi ini akan dijalankan ketika user memilih salah satu hasil request
        onSelect: function (suggestion) {        	
        		$("#txt_Nama").val(suggestion.value);
            	$("#ID_USER").val(suggestion.ID_USER+':'+suggestion.value);
        } 
    });
		...
		if (data_child == 'ada') {
	        ...
	        function(isConfirm) {
	            if (isConfirm) {
					...
					$.ajax({
						url : $('#base-url').val() + 'ET_Procurement_template/create',
						method : 'post',
						data : form_data,
						success : function(result)
						{
							if(result=='OK'){ 
								...
								window.location = $('#base-url').val() + 'ET_Procurement_template/index';
	...
	$("#buttonAddItem").click(function() {
		...
		var evt_user = $('#ID_USER').val();
		var evt_prasyarat = $('input[name="optradioprasyarat"]:checked').val();
		// numeral.language('id');
		var user_otorisasi = evt_user.split(':');
		
		$("#firstRow").hide(400);

		console.log(evt_prasyarat);
		if(evt_prasyarat==1){
			if (Name == "" || evt_user == "") return;
			console.log('Create parent item with ID: ' + no);
				...
				rowTag = '<tr style="vertical-align: top;">' +
							...
							'<input type="hidden" class="parent_mode" id="mode_'+no+'" name="evt_teknis['+no+'][user]" value="'+user_otorisasi[0]+'">'+
							'<input type="hidden" class="parent_mode" id="mode_'+no+'" name="evt_teknis['+no+'][prasyarat]" value="'+evt_prasyarat+'">'+
							...
							'</td>' +
							'<td class="" style="vertical-align: top;">'+user_otorisasi[1]+'</td>' +
							'<td class="bobotnya" style="vertical-align: top;">Prasyarat</td>' +
				...
				$("#txt_Nama").val("");
				$('#ID_USER').val("");
				$("#ppd_weight").prop("disabled", false);
				// $("#ppd_mode").prop("disabled", false); 
				$("input[name='optradioprasyarat'][value='0']").prop("checked", true);

				/* Nambah option */
				$("#select_item_det").append('<option value="'+no+'_'+mode+'_'+evt_prasyarat+'">'+Name+'</option>')

				no++;
		}else{
			if (Name == "" || Weight == "" || evt_user == "") return;
	...
	$("#buttonAddDetail").click(function() {
		...
		if (name == "") return;
		var itemType = $('input[name="optradiotype"]:checked').val();
		...
		if (parent != null) {
			if(fields[2]==1){
				...
				inputnya = '<li>'+
					...
					'<td style="text-align:right; width:100px">&nbsp;&nbsp;&nbsp; '+tipe+'</td>'+
					'<input type="hidden" name="pptu_weight['+parent_id+'][]" value="'+pptu_weight+'">'+
					'<input type="hidden" name="pptu_type['+parent_id+'][]" value="'+itemType+'">'+
					'</li>'
				...
				if(pptu_weight == '' && fields[1] == 0) { //jika dinamis
					...
					if(parseInt(bobot_parent) >= parseInt(bbt_chld_akumulasi)){
						...
						inputnya = '<li>'+
							...
							'<input type="hidden" name="pptu_weight['+parent_id+'][]" value="'+pptu_weight+'">'+
							'<input type="hidden" name="pptu_type['+parent_id+'][]" value="'+(itemType==undefined?'0':'')+'">'+
							'</li>'
						listnya.append(inputnya);
					}else{
		...
		$('input[name="optradiotype"]').prop('checked', false);
	});
	...
	$('input[name="optradioprasyarat"]').click(function() {
		console.log($(this).val());
		var pval = $(this).val();
		if(pval==1){
			$('#ppd_weight').val('');
			$('#ppd_weight').prop('disabled', true);
		}else{
			$('#ppd_weight').prop('disabled', false);
		}
	});
