...
function setCode(args, elme) {
	...
	$("#idUpload").val(id_parent)
	$("#kodeUpload").val($(elme).data('kode'))
	...
	countLvl5 = 0
	countLvl6 = 0
	var dataKode = $(elme).data('kode')
	if(isNaN(dataKode)) {
		$("#file_input").attr("disabled", "disabled");
		$("#dis_up").attr("disabled", "disabled");
		$("#dis_dl").attr("disabled", "disabled");
	} else {
		$("#file_input").removeAttr("disabled");
		$("#dis_up").removeAttr("disabled");
		$("#dis_dl").removeAttr("disabled");
	}
}
...
function cek(input) {
	if((input.value).length > 0) {
		$("#nama_file_input").val(input.files[0].name);
	} else {
