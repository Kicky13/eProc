...
$(document).ready(function() {
    console.log($("#ptp_template_evaluasi").val());
    if($("#ptp_template_evaluasi").val()!=''){
        $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_detail_template/' + $("#ptp_template_evaluasi").val(),
            type: 'get',
            dataType: 'json',
        }).done(function(data) {
...
//    });
    ...
    $("#ptp_template_evaluasi").change(function(event) {
        // alert($(this).val());
        $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_detail_template/' + $(this).val(),
            type: 'get',
            dataType: 'json',
        }).done(function(data) {
            $('#tbl_detail_template').empty();
    ...
    $(".tender_type").change(function(event) {
    // $(document).on('change', '.tender_type', function() {
       // alert($(this).attr('id'));
        event.preventDefault();
        var ven = $(this).attr('id');
        var vendorno = ven.split(":");
       $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_principalList/' + vendorno[1],
            type: 'get',
            dataType: 'json',
        }).done(function(data) {
            if(data==false){
                alert('Vendor ini tidak mempunyai Principal');
                document.getElementById(ven).value = vendorno[1]+":1";
                // console.log("#list_principal:"+vendorno[1]);
                // $("#list_principal:"+vendorno[1]).append("<option value='0'>No Principal</option>")
                // document.getElementById("list_principal:"+vendorno[1]).value = "0";
                // document.getElementById("list_principal:"+vendorno[1]).setAttribute("disabled","disabled");
            }
        }).fail(function() {
    ...
    function populateTable() {
        ...
        template_table = $('#template-table').DataTable( {
            "ajax": $("#base-url").val() + 'EC_Procurement_pengadaan/get_all_template',
            "columnDefs": [{
                "searchable": true,
                "orderable": false,
                "targets": 0
    ...
    $('.uploadAttachment').each(function(event) {
        ...
        var uploader = new ss.SimpleUpload({
            button: btn,
            url: $("#base-url").val() + 'EC_Procurement_pratender/uploadAttachment',
            name: 'uploadfile',
    ...
    $('.del_upload_file').click(function(){
        $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pratender/deleteFile',
            type: 'POST',
            data: {
...
//end submit
    ...
    if ($("#just").val() != '5') {
        if($("#jenis_perencanaan").val() == 0){ //barang
            ...
            $.ajax({
                url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_vendor/' + $('#ptm_number').val(),
                type: 'get',
                dataType: 'json',
            })
            .done(function(data) {
            ...
            var tableGrid = $('#table_vendor_jasa').DataTable( {
                ...
                "paging": false,
                "ajax": {
                    url : $("#base-url").val() + "EC_Procurement_pengadaan/get_vendor_jasa",
                    method : 'post',
                    data : {id_ptm},
                },
                "columnDefs": [{
        ...
        $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pratender/get_po_from_ptm/' + $("#ptm_number").val(),
            type: 'POST',
            dataType: 'json',
    ...
    $('#group_jasa_id').change(function(){
        ...
        $.ajax({
            url: $("#base-url").val() + "EC_Procurement_pratender/pilih_child",
            method : 'post',
            data : "id="+id,
    ...
    $('#klasifikasi_jasa_id').change(function(){
        ...
        $('.new_data').remove();
        $.ajax({
            url : $("#base-url").val() + 'EC_Procurement_pratender/pilih_sub_klasifikasi',
            method : 'post',
            data : "id="+id,
    ...
    $('#search').click(function(){
        ...
        var tableGrid = $('#table_vendor_jasa_tambahan').DataTable( {
            ...
            "paging": false,
            "ajax": {
                url : $("#base-url").val() + "EC_Procurement_pratender/get_vendor_jasa",
                method : 'post',
    ...
    $('#generate_vnd_brg_tmbhn').click(function(){
        ...
        var tableGridBarang = $('#table_vendor_barang_tambahan').DataTable( {            
            ...
            "ajax": {
                url : $("#base-url").val() + "EC_Procurement_pengadaan/get_vendor_barang",
                method : 'post',
                data : {vendor},
            },
            "columnDefs": [{
...
function terpilih_vnd_tambahan(e){
    ...
    if($(e).is(":checked")){  
        ...
        if (!ada) {
            $.ajax({
                url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_principalList/' + vendorno,
                type: 'get',
                dataType: 'json',
            }).done(function(data) {
                if(data==false){
                    ...
                    tr = '<tr>' +
                      ...
                      '<td>' + $(e).parent().parent().find('.namavendor_tambahan').html() + '</td>' +
                      '<td>'+
                          '<select class="tender_type" id="tender_type:'+vendorno+'" name="type_tender[]">'+
                              '<option value="'+vendorno+':1">Agen to Principal</option>'+
                              '<option value="'+vendorno+':2">By Principal</option>'+
                          '</select>'+
                      '</td>'+
                      '<td>'+
                            '<select id="list_principal:'+vendorno+'" name="list_principal[]" style="width: 250px;">';                                
                                for (var i = 0; i < data.length; i++) {
                                    tr += '<option value="'+vendorno+':'+data[i]['PC_CODE']+'">'+data[i]['PC_NAME']+'</option>';
