...
function loadTable () {
    ...
    mytable = $('#tbl_group_akses').DataTable({
        ...
        "ajax" : $("#base-url").val() + 'Monitoring_prc_user/get_datatable',
        ...
        "columns":[
            ...
            },
            {
                mRender : function(data,type,full){
                    console.log(full);
                    var button = '<a title="Monitoring Procurement" href="' + $("#base-url").val() + 'Monitoring_prc/detail/'+full.PTM_NUMBER+'" class="btn btn-default btn-sm glyphicon glyphicon-th-list"></a> ' + '<button title="Current job holder" onclick="openmodal('+full.PTM_NUMBER+')" class="btn btn-default btn-sm glyphicon glyphicon-user"></button>';
                    if(full.PROCESS_MASTER_ID=='Tahap_negosiasi/index'){
