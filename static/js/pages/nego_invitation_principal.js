...
$(document).ready(function() {
	if ($('#invitation_lsit').length > 0) {
		var table_invitation_lsit = $('#invitation_lsit').DataTable( {
			...
			"lengthMenu": [ 10, 25, 50 ],
			"ajax": {
				url: base_url + "Nego_invitation_principal/get_invitation_list"
			},
			"columnDefs": [{
				"targets": 0
			}],
			"columns":[
				...
				}, "sClass": "text-center"},
				{
					...
					if (full.PTV_STATUS != 0 && full.PTV_STATUS != 1) {
						return '<a class="btn btn-default" href="'+base_url+'Nego_invitation_principal/detail_invitation/'+full.ECT_PTV_ID+'/'+full.PTV_VENDOR_CODE+'">Process</a>'
					}
				}, "sClass": "text-center"}
			],
		} );
