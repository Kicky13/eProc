...
function loadTable_() {
    ...
    mytable = $('#table_inv').DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_Publish_Approval/getPublish_approval',
        ...
        "columns": [{
            ...
            mRender: function (data, type, full) {
                a = "<div class='col-md-12 text-center'>" +
                    // '<a href="javascript:approve(' + (full.KODE_PROPOSE) + ')"><span title="Approve" class="glyphicon glyphicon-ok" aria-hidden="true"></span></a>&nbsp;&nbsp;' +
                    // '<a href="javascript:reject(' + (full.KODE_PROPOSE) + ')"><span title="Reject" class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>&nbsp;&nbsp;' +
                    '<input type="checkbox" class="actionSelect" data-kode="' + full.KODE_PROPOSE + '">' +
