...
$(document).ready(function(){
	...
	var table_quotation_lsit = $('#quotation_lsit').DataTable( {
		...
		"lengthMenu": [ 10, 25, 50 ],
		"ajax": {
			url: base_url + "Quotation/get_quotation_list"
		},
		"columnDefs": [{
			"targets": 0
		}],
			...
			}, "sClass": "text-center"},
			{
				mRender : function(data,type,full) {
					if (full.PTV_STATUS_EVAL == 1 || full.PTV_STATUS_EVAL == 2) {
						return '<a class="btn btn-default" href="'+base_url+'Quotation/harga/'+full.PTM_NUMBER+'/'+full.PTV_ID+'">Process</a>';
					} else {
						return '<a class="btn btn-default" href="'+base_url+'Quotation/inputQuotation/'+full.PTM_NUMBER+'">Process</a>';
					}
			}, "sClass": "text-center"}
		],
	} );
	...
	var table_quotation_submitted = $('#quotation_submitted').DataTable( {
		...
		"lengthMenu": [ 10, 25, 50 ],
		"ajax": {
			url: base_url + "Quotation/get_quotation_list/"+"submitted"
		},
		"columnDefs": [{
			"searchable": false,
            "orderable": false,
		...
		"columns":[
			...
			}, "sClass": "text-center"},
			{
				mRender : function(data,type,full) {
					if (full.PTV_STATUS_EVAL == 1 || full.PTV_STATUS_EVAL == 2) {
						return '<a title="View" href="'+base_url+'Quotation/harga/'+full.PTM_NUMBER+'/'+full.PTV_ID+'/viewSubmitted'+'" class="btn btn-default btn-sm glyphicon glyphicon-eye-open"></a>';
					} else {
						return '<a title="View" href="'+base_url+'Quotation/inputQuotation/'+full.PTM_NUMBER+'/viewSubmitted'+'" class="btn btn-default btn-sm glyphicon glyphicon-eye-open"></a>';
					}
			}, "sClass": "text-center"}
		],
	} );
