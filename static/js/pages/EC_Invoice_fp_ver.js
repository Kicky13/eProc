	...
	mytable = $('#table_inv').DataTable({
        ...
        if(full.FAKPJK_PIC != ''){
          ...
          a += '<a href="#">'+ full.FAKTUR_PJK+'</a>';
			...
			mRender : function(data, type, full) {
				a = "<div class='col-md-12 text-center'>";
				a += '<a href="#">'+full.FAKTUR_PJK_DATE+'</a>';
				a += "</div>";
				return a;
			}
		}, {
			...
			mRender : function(data, type, full) {
				if (full.STATUS_SETOR == '1') {
					status = "Vendor";
				} else if (full.STATUS_SETOR == '2') {
					status = "Verifikasi";
				} else {
					status = "-";
				}
			...
			mRender : function(data, type, full) {
				...
				if (full.STATUS_SETOR == '') {
					a += '<input type="checkbox" name="STATUS_SETOR[]" value="'+full.FAKTUR_PJK+'">';
				} else {
