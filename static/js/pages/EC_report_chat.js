...
$(document).ready(function () {
    ...
    $('#sendMsg').click(function () {
        ...
        if (msg !== ''){
            $.ajax({
                url: $('#base-url').val() + 'EC_Report_Chat/sendMessage',
                type: 'POST',
                dataType: 'json',
                data: {
                    negoId: negoid,
...
function loadTable_Active() {
    ...
    mytable = $('#table_nego_active').DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_Report_Chat/getActiveNego',
        ...
        "columns": [{
            ...
            mRender: function (data, type, full) {
                ...
                a = "<div class='col-md-12 text-center'>";
                a += "<button type='button' data-toggle='modal' data-target='#modalNego' data-plant='"+ full.PLANT +"' data-plantName='"+ full.PLANT_NAME +"' data-matno='"+ full.MATNO +"' data-maktx='"+ full.MAKTX +"' data-vendorno='"+ full.VENDORNO +"' data-vendorname='"+ full.VENDOR_NAME +"' data-negoid='"+ full.ID +"' title='Nego Harga' style='font-size:12px;box-shadow: 1px 1px 1px #ccc'  class='btn btn-" + colorClass + " nego'><i class='glyphicon glyphicon-comment' ></i> " + unread + "</button>";
                a += "</div>";
                return a;
            }
        }],
...
function loadTable_Archive() {
    ...
    mytable = $('#table_nego_archive').DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_Report_Chat/getArchiveNego',
        ...
        "columns": [{
            ...
            mRender: function (data, type, full) {
                a = "<div class='col-md-12 text-center'>";
                a += "<button type='button' data-toggle='modal' data-target='#modalNegoArchive' data-plant='"+ full.PLANT +"' data-plantName='"+ full.PLANT_NAME +"' data-matno='"+ full.MATNO +"' data-maktx='"+ full.MAKTX +"' data-vendorno='"+ full.VENDORNO +"' data-vendorname='"+ full.VENDOR_NAME +"' data-negoid='"+ full.ID +"' title='Nego Harga' style='font-size:12px;box-shadow: 1px 1px 1px #ccc'  class='btn btn-primary nego'><i class='glyphicon glyphicon-comment' ></i> Negosiasi</button>";
                a += "</div>";
                return a;
            }
        }],
...
function loadChat(negoId) {
    ...
    mytable = $('#table_chat').DataTable({
        ...
        "ajax": {
            url: $("#base-url").val() + 'EC_Report_Chat/openChat',
            type: 'POST',
            dataType: 'json',
            data: {
                negoId: negoId
...
function openLockHarga(vendorno, matno, plant) {
    $.ajax({
        url: $('#base-url').val() + 'EC_Report_Chat/openLock',
        type: 'POST',
        dataType: 'json',
        data: {
            vendorno: vendorno,
