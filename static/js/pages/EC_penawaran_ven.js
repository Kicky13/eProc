...
function loadDataList() {
    ...
    $.ajax({
        url: $("#base-url").val() + 'EC_Penawaran_Vendor_test' + urlll,
        data : {            
            "limitMin" : limitMin,
            "limitMax" : limitMax            
        },
...
//                        teks += ('<label for="deskrip">Deskripsi Item:</label>')
//                        teks += ('<textarea class="form-control" rows="3" id="deskrip'+i+'" readonly>' + data.data[i].LONGTEXT + '</textarea>')
//                    teks += ('</div>')
//                teks += ('</div>')
                teks += ('<div class="row">')
                    teks += ('<form class="form-inline"> <div class="form-group">Stok:&nbsp;&nbsp;')
                ...
                teks += ('<div class="col-md-1" style="text-align: center;">')// + '<form class="form-horizontal">' )

                teks += ('<a href="javascript:updateStock(\'' + data.data[i].MATNO + '\',\'' + data.data[i][13] + '\',' + (data.data[i].STOK_COMMIT == "" ? 0 : data.data[i].STOK_COMMIT) + ',' + i + ',' + (data.data[i].STOK == "" ? 0 : data.data[i].STOK) + ')" style="font-size:12px;"  class="btn btn-info form-control pull-right">Update</a><p></p>')
                ...
                teks += ('</div>' + '</div>')
                teks += ('<div class="row" style="padding: 5px;">')
                teks += ('<div class="col-sm-12 col-md-12 col-lg-12">')
                    teks += ('<div class="panel-group" id="accordion'+i+'" role="tablist" aria-multiselectable="true">')
                        teks += ('<div class="panel panel-default">')
                            teks += ('<div class="panel-heading" role="tab" id="accord-item'+i+'">')
                                teks += ('<h5 class="panel-title">')
                                    teks += ('<a role="button" data-toggle="collapse" data-parent="#accordion'+i+'" href="#item'+i+'" aria-expanded="true" aria-controls="item'+i+'" onclick="getDetail(this,\'table_detail' + i + '\',\'' + i + '\',\'' + data.data[i].MATNO + '\',\'' + data.data[i].VENDORNO + '\',\'' + data.data[i].CURRENCY + '\')"><strong>Quote Pricelist</strong></a>')
                                teks += ('</h5>')
                            teks += ('</div>')
                            teks += ('<div id="item'+i+'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="accord-item'+i+'">')
                                teks += ('<div class="panel-body">')
                                    // teks += ('<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">')
                                        teks += ('<div class="panel-group skrol" id="" role="tablist" aria-multiselectable="true">')
                                            // teks += ('<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">')
                                                teks += ('<table id="table_detail'+i+'" class="table table-striped nowrap" width="100%">')
                                                    teks += ('<thead>')
                                                        teks += ('<tr>')
                                                            teks += ('<th class="text-center ts1-'+i+'"><a href="javascript:void(0)">Destination</a></th>')
                                                            teks += ('<th class="text-center ts2-'+i+'"><a href="javascript:void(0)">Currency</a></th>')
                                                            teks += ('<th class="text-center ts3-'+i+'"><a href="javascript:void(0)">Price</a></th>')
                                                            teks += ('<th class="text-center ts4-'+i+'"><a href="javascript:void(0)">Delivery Time</a></th>')
                                                            teks += ('<th class="text-center ts5-'+i+'"><a href="javascript:void(0)">Last Update</a></th>')
                                                            teks += ('<th class="text-center ts6-'+i+'"><a href="javascript:void(0)">Next Update</a></th>')
                                                            teks += ('<th class="text-center ts7-'+i+'"><a href="javascript:void(0)">Aksi</a></th>')
                                                        teks += ('</tr>')
                                                        teks += ('<tr class="sear'+i+'">')
                                                            teks += ('<th><input type="text" class="col-xs-10 col-xs-offset-1 srch'+i+'" style="margin: 0px"></th>')
                                                            teks += ('<th><input type="text" class="col-xs-10 col-xs-offset-1 srch'+i+'" style="margin: 0px"></th>')
                                                            teks += ('<th></th>')
                                                            teks += ('<th></th>')
                                                            teks += ('<th><input type="text" class="col-xs-10 col-xs-offset-1 srch'+i+'" style="margin: 0px"></th>')
...
function updateStock(matno, kode, stokc, id, stok_awal= '0') {
    ...
    if(stok<stokc){
        ...
        bootbox.confirm('Konfirmasi Update Stok?', function (result) {
            if (result){
                $.ajax({
                    url: $("#base-url").val() + 'EC_Penawaran_Vendor_test/insertStok/' + matno,
                    data: {
                        "matno": matno,
                        "harga": '0',
                        "curr": '0',
...
function getDetail_old(elm, matno, vendorno, currency) {
    ...
    $.ajax({
        url: $("#base-url").val() + 'EC_Penawaran_Vendor_test/getDetail/' + matno,
        type: 'POST',
        dataType: 'json'
    }).done(function (data) {
        // console.log(data);
...
function SaveHarga(matno, plant, harga, deliverytime, currency) {
    if($("#harga"+plant).is( ":disabled" ) == true){
        ...
        bootbox.confirm('Konfirmasi Update Harga?', function (result) {
            if (result){
                $.ajax({
                url: $("#base-url").val() + 'EC_Penawaran_Vendor_test/SaveHarga/' + matno,
                data: {
                            "plant": plant,
                            "curr": currency,
...
function save(element, matno, harga, curr, deliverytime, stok) {
    ...
    if (harga1 == '' || currency == null || time == '') {
        ...
        if (confirm('Apakah anda yakin menyimpan data ini ?'))
            $.ajax({
                url: $("#base-url").val() + 'EC_Penawaran_Vendor_test/insertHarga/' + matno,
                data: {
                    "matno": matno,
                    "harga": $('#' + harga).val(),
                    "curr": $('#' + curr).val(),
...
$('#modaldetail').on('show.bs.modal', function (event) {
    ...
    //var modal = $(this)
    $.ajax({
        url: $("#base-url").val() + 'EC_Penawaran_Vendor_test/getDescItem/' + MATNR,
        type: 'POST',
        dataType: 'json'
    }).done(function (data) {
        // console.log(data);
...
function loadTable_App(tabelname, index, matno, currency) {
    ...
    mytable = $('#'+tabelname).DataTable({
        ...
        "ajax": $("#base-url").val() + 'EC_Penawaran_Vendor_test/getDetail/' + matno,
