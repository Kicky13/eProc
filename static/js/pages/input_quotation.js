...
function cek_hargawarning(e) {
    ...
    $.ajax({
        url: $("#base-url").val() + 'Quotation/cek_toleransi/' + e.parent().parent().find(".tit_id").val() + '/' + dat,
        type: 'post',
        dataType: 'json',
    })
    .done(function(data) {
...
$(document).ready(function(){
    ...
    $('.delete_upload_file').click(function(){
        $.ajax({
            url: $("#base-url").val() + 'Quotation/deleteFile',
            type: 'POST',
            data: {
                id : $("#id_ptv").val(),
                filename: cekFile
    ...
    $('#quoform').submit(function(e) {
        ...
        // gagal = false;
        // $('.submit').find('.quotation_uploadAttachment').each(function() {
    ...
    $(".open-material").click(function() {
        ...
        $.ajax({
            url: $("#base-url").val() + 'Quotation/getlongtext',
            type: 'POST',
            dataType: 'html',
            data: {
                PPI_NOMAT: PPI_NOMAT,
    ...
    $('.quotation_uploadAttachment').each(function(event) {
        ...
        var uploader = new ss.SimpleUpload({
            button: btn,
            url: $("#base-url").val() + 'Quotation/uploadAttachment',
            name: 'uploadfile',
            allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'odt', 'html', 'zip'],
            maxSize: 10048,
            multipart: true,
...
function delete_quotation_upload(tit_id){
    $.ajax({
        url: $("#base-url").val() + 'Quotation/deleteQuoFile',
        type: 'POST',
        data: {
