    ...
    mytable = $('#tableMT').DataTable({
        ...
        "columns": [
        {
            ...
            mRender: function(data, type, full) {
                ...
                a += '<input type="checkbox"class="lot'+full.EBELN+'" onclick="cekGRSelected(this)" class="ckb" value="' + full.EBELN + '" data-amount="' + full.NETPRC + '"data-curr="' + full.WAERS + '"';
                // '
                if (full.STATUS == 1)
                    a += " checked>";
                else
...
function loadTable_invoice() {
    ...
    mytable_inv = $('#table_inv').DataTable({
        ...
        mRender: function(data, type, full) {
            a = "<div class='col-md-12 text-center'>";
            a += full.CURRENCY + " <strong>" + numberWithCommas(full.DP_REQ_AMOUNT) + "</strong>";
            a += "</div>";
            return a;
        }
    }, {
...
$(".format-koma").keyup(function() {
    ...
    bootbox.alert('Tidak ada PO yang dipilih');
}
