...
function cek_hargawarning(e) {
    ...
    $.ajax({
        url: $("#base-url").val() + 'Quotation_principal/cek_toleransi/' + e.parent().parent().find(".tit_id").val() + '/' + dat,
        type: 'post',
        dataType: 'json',
    })
    .done(function(data) {
...
$(document).ready(function(){
    // console.log($("#ptp_evaluasi_teknik").val());
    /*if($("#ptp_evaluasi_teknik").val()!=''){
        $.ajax({
            url: $("#base-url").val() + 'EC_Procurement_pengadaan/get_detail_template/' + $("#ptp_evaluasi_teknik").val(),
            type: 'get',
            dataType: 'json',
        }).done(function(data) {
            ...
            for (var i = 0; i < data.detail.length; i++) {
                ...
                setBody += '<tr style="vertical-align: top;">';
                    setBody += '<td class="text-center" style="vertical-align: top;">'+(no+1)+'</td>';
                    setBody += '<td>';    
                        ...
                        for (var j = 0; j < data.detail[i].uraian.length; j++) { 
                            if(data.detail[i].uraian[j].PPTU_TYPE==1){
                                setBody += '<div class="row" style="padding:5px;">';
                                    setBody += '<div class="col-sm-1 col-md-1 col-lg-1"><input class="eval_cek" type="checkbox" value="'+data.detail[i].uraian[j].PPTU_ID+'" name="cek_eval[]" checked></div>';
                                    setBody += '<div class="col-sm-4 col-md-4 col-lg-4">'+data.detail[i].uraian[j].PPTU_ITEM+'</div>';                                
                                    setBody += '<div class="col-sm-3 col-md-3 col-lg-3">';
                                        setBody += '<label class="radio-inline"><input type="radio" name="optradio'+data.detail[i].uraian[j].PPTU_ID+'" value="Ya">Ya</label>';
                                        setBody += '<label class="radio-inline"><input type="radio" name="optradio'+data.detail[i].uraian[j].PPTU_ID+'" value="Tidak">Tidak</label>';
                                    setBody += '</div>';
                                setBody += '</div>';
                            }else if(data.detail[i].uraian[j].PPTU_TYPE==2){
                                setBody += '<div class="row" style="padding:5px;">';
                                    setBody += '<div class="col-sm-1 col-md-1 col-lg-1"><input class="eval_cek" type="checkbox" value="'+data.detail[i].uraian[j].PPTU_ID+'" name="cek_eval[]" checked></div>';
                                    setBody += '<div class="col-sm-4 col-md-4 col-lg-4">'+data.detail[i].uraian[j].PPTU_ITEM+'</div>';                                
                                    setBody += '<div class="col-sm-4 col-md-4 col-lg-4">';
                                        setBody += '<textarea class="form-control" rows="1" id="" name="optradio'+data.detail[i].uraian[j].PPTU_ID+'"></textarea>';
                                    setBody += '</div>';
                                setBody += '</div>';
                            }
    ...
    $('.delete_upload_file').click(function(){
        $.ajax({
            url: $("#base-url").val() + 'Quotation_principal/deleteFile',
            type: 'POST',
            data: {
                id : $("#id_ptv").val(),
                filename: cekFile
    ...
    $(".open-material").click(function() {
        ...
        $.ajax({
            url: $("#base-url").val() + 'Quotation_principal/getlongtext',
            type: 'POST',
            dataType: 'html',
            data: {
                PPI_NOMAT: PPI_NOMAT,
    ...
    $('.quotation_uploadAttachment').each(function(event) {
        ...
        var uploader = new ss.SimpleUpload({
            button: btn,
            url: $("#base-url").val() + 'Quotation_principal/uploadAttachment',
            name: 'uploadfile',
            allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'odt', 'html', 'zip'],
            maxSize: 2048,
            multipart: true,
...
function delete_quotation_upload(tit_id){
    $.ajax({
        url: $("#base-url").val() + 'Quotation_principal/deleteQuoFile',
        type: 'POST',
        data: {
